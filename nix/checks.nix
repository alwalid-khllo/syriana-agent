...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      syriana-agent = self'.packages.default;
      syrianaVenv = syriana-agent.syrianaVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "syriana-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${syrianaVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "syriana-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "syriana-build-package" { } ''
          echo "PASS: package built at ${syriana-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "syriana-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "syriana-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${syriana-agent}/bin/syriana || (echo "FAIL: syriana binary missing"; exit 1)
          test -x ${syriana-agent}/bin/syriana-agent || (echo "FAIL: syriana-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${syriana-agent}/bin/syriana version 2>&1 | grep -qi "syriana" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "syriana-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in syriana syriana-agent syriana-acp; do
            test -x ${syriana-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "syriana-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking syriana --help ==="
          ${syriana-agent}/bin/syriana --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${syriana-agent}/bin/syriana --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "syriana-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${syriana-agent}/share/syriana-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${syriana-agent}/share/syriana-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "SYRIANA_BUNDLED_SKILLS" ${syriana-agent}/bin/syriana || \
            (echo "FAIL: SYRIANA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: SYRIANA_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "syriana-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${syriana-agent}/share/syriana-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${syriana-agent}/share/syriana-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "SYRIANA_BUNDLED_PLUGINS" ${syriana-agent}/bin/syriana || \
            (echo "FAIL: SYRIANA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: SYRIANA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "syriana-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${syriana-agent}/share/syriana-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${syriana-agent}/share/syriana-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${syriana-agent}/share/syriana-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "SYRIANA_BUNDLED_LOCALES" ${syriana-agent}/bin/syriana || \
            (echo "FAIL: SYRIANA_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: SYRIANA_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (SYRIANA_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && SYRIANA_BUNDLED_LOCALES=${syriana-agent}/share/syriana-agent/locales \
            ${syrianaVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with SYRIANA_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${syrianaVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${syrianaVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without SYRIANA_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "syriana-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${syriana-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${syriana-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "SYRIANA_TUI_DIR" ${syriana-agent}/bin/syriana || \
            (echo "FAIL: SYRIANA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: SYRIANA_TUI_DIR set in wrapper"
        ...
        # Verify SYRIANA_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        syriana-node = pkgs.runCommand "syriana-node-version" { } ''
          set -e
          echo "=== Checking SYRIANA_NODE in wrapper ==="
          grep -q "SYRIANA_NODE" ${syriana-agent}/bin/syriana || \
            (echo "FAIL: SYRIANA_NODE not set in wrapper"; exit 1)
          echo "PASS: SYRIANA_NODE present in wrapper"

          SYRIANA_NODE=$(sed -n "s/^export SYRIANA_NODE='\(.*\)'/\1/p" ${syriana-agent}/bin/syriana)
          test -x "$SYRIANA_NODE" || (echo "FAIL: SYRIANA_NODE=$SYRIANA_NODE not executable"; exit 1)
          echo "PASS: SYRIANA_NODE executable at $SYRIANA_NODE"

          NODE_MAJOR=$("$SYRIANA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All SYRIANA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify SYRIANA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "syriana-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(SYRIANA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking SYRIANA_MANAGED guards ==="
          check_blocked "config set" ${syriana-agent}/bin/syriana config set model foo
          check_blocked "config edit" ${syriana-agent}/bin/syriana config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = syriana-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "syriana-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${hermesWithExtra}/bin/syriana || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${hermesWithExtra}/bin/syriana || \
          ...
          if grep -q "PYTHONPATH" ${syriana-agent}/bin/syriana; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = syriana-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "syriana-extra-dependency-groups" { } ''
          set -e
          ...
          echo "venv: ${hermesWithGroups.syrianaVenv}"
        ...
        messaging-variant = pkgs.runCommand "syriana-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.syrianaVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "syriana-config-roundtrip" {
          ...
          merge_and_load() {
            local syriana_home="$1"
            export SYRIANA_HOME="$syriana_home"
            ${configMergeScript} ${nixSettings} "$syriana_home/config.yaml"
            ${syrianaVenv}/bin/python3 -c '
import json, sys
