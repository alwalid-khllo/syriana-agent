    ...
    in {
      ...
      checks = {
        ...
        package-contents = pkgs.runCommand "hermes-package-contents" { } ''
          ...
          test -x ${hermes-agent}/bin/hermes || (echo "FAIL: syriana binary missing"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "hermes-entry-points-sync" { } ''
          ...
          for bin in syriana hermes-agent hermes-acp; do
        ...
        bundled-skills = pkgs.runCommand "hermes-bundled-skills" { } ''
          ...
          grep -q "SYRIANA_BUNDLED_SKILLS" ${hermes-agent}/bin/hermes || \
        ...
        bundled-plugins = pkgs.runCommand "hermes-bundled-plugins" { } ''
          ...
          grep -q "SYRIANA_BUNDLED_PLUGINS" ${hermes-agent}/bin/hermes || \
        ...
        bundled-locales = pkgs.runCommand "hermes-bundled-locales" { } ''
          ...
          grep -q "SYRIANA_BUNDLED_LOCALES" ${hermes-agent}/bin/hermes || \
          ...
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && SYRIANA_BUNDLED_LOCALES=${hermes-agent}/share/hermes-agent/locales \
        ...
        bundled-tui = pkgs.runCommand "hermes-bundled-tui" { } ''
          ...
          grep -q "SYRIANA_TUI_DIR" ${hermes-agent}/bin/hermes || \
        ...
        hermes-node = pkgs.runCommand "hermes-node-version" { } ''
          ...
          grep -q "SYRIANA_NODE" ${hermes-agent}/bin/hermes || \
            (echo "FAIL: SYRIANA_NODE not set in wrapper"; exit 1)
          echo "PASS: SYRIANA_NODE present in wrapper"

          SYRIANA_NODE=$(sed -n "s/^export SYRIANA_NODE='\(.*\)'/\1/p" ${hermes-agent}/bin/hermes)
        ...
        in pkgs.runCommand "hermes-config-roundtrip" {
          ...
          merge_and_load() {
            local hermes_home="$1"
            export SYRIANA_HOME="$hermes_home"
