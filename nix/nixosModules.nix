...
#
# Container mode: syriana runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $SYRIANA_HOME/.env
# and read by syriana at startup — no container recreation needed for env changes.
#
# Tool resolution: the syriana wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    # SYRIANA_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "hermes-documents" { } (
      ...
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'SYRIANA_DOC_EOF'\n${value}\nHERMES_DOC_EOF"
        ) cfg.documents
      )
    );
    ...
    containerEntrypoint = pkgs.writeShellScript "hermes-container-entrypoint" ''
      ...
      SYRIANA_GID="''${SYRIANA_GID:?SYRIANA_GID must be set}"

      # ── Group: ensure a group with GID=$SYRIANA_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$SYRIANA_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        ...
        if command -v groupadd >/dev/null 2>&1; then
          ...
          addgroup -g "$SYRIANA_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$SYRIANA_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$SYRIANA_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        ...
        if command -v useradd >/dev/null 2>&1; then
          ...
          adduser -u "$SYRIANA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$SYRIANA_UID:$SYRIANA_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure SYRIANA_HOME is owned by the target user.
      ...
      if [ -n "''${SYRIANA_HOME:-}" ] && [ -d "$SYRIANA_HOME" ]; then
        find "$SYRIANA_HOME" \! -user "$SYRIANA_UID" -exec chown "$SYRIANA_UID:$SYRIANA_GID" {} +
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$SYRIANA_UID" --regid="$SYRIANA_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $SYRIANA_HOME/.env.
  ...
  in {
    options.services.hermes-agent = with lib; {
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $SYRIANA_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $SYRIANA_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `syriana gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the syriana user's per-user profile
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          syriana_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as syriana (python312).
        '';
        example = literalExpression ''
          [
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the syriana CLI to environment.systemPackages and export
          SYRIANA_HOME system-wide (via environment.variables) so interactive
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      # Wire extraPackages into the syriana user's per-user profile so the
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.hermes-agent = {
          ...
          serviceConfig = {
            ...
            # so interactive users in the syriana group can read/write them.
            UMask = "0007";
        ...
        systemd.services.hermes-agent = {
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              SYRIANA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              SYRIANA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
