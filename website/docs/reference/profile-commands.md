...
This page covers all commands related to [Syriana profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `syriana profile` without a subcommand shows help.
...
```bash
syriana profile use <name>
```

Sets `<name>` as the active profile. All subsequent `syriana` commands (without `-p`) will use this profile.
...
syriana profile use default
```

## `syriana profile create`

```bash
syriana profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `syriana profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `syriana update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.syriana`), use `syriana skills opt-out` / `syriana skills opt-in`. |
...
syriana profile create work2 --clone-from work

# Clone everything from a specific profile
syriana profile create work2-backup --clone-from work --clone-all
```

## `syriana profile describe`

```bash
syriana profile describe [<name>] [options]
```
...
syriana profile describe researcher

# Set it explicitly
syriana profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
syriana profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
syriana profile describe --all --auto
```

## `syriana profile delete`

```bash
syriana profile delete <name> [options]
```
...
```bash
syriana profile delete mybot
syriana profile delete mybot --yes
```
...
## `syriana profile show`

```bash
syriana profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Syriana home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ syriana profile show work
Profile: work
Path:    ~/.syriana/profiles/work
...
## `syriana profile alias`

```bash
syriana profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Syriana installation.
...
```bash
syriana profile alias work
# Creates/updates ~/.local/bin/work

syriana profile alias work --name mywork
# Creates ~/.local/bin/mywork

syriana profile alias work --remove
# Removes the wrapper script
```

## `syriana profile rename`

```bash
syriana profile rename <old-name> <new-name>
```
...
```bash
syriana profile rename mybot assistant
# ~/.syriana/profiles/mybot → ~/.syriana/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `syriana profile export`

```bash
syriana profile export <name> [options]
```
...
```bash
syriana profile export work
# Creates work.tar.gz in the current directory

syriana profile export work -o ./work-2026-03-29.tar.gz
```

## `syriana profile import`

```bash
syriana profile import <archive> [options]
```
...
```bash
syriana profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

syriana profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`syriana profile export` / `import` are still the right commands for
...
### `syriana profile install`

```bash
syriana profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `syriana -p telemetry`). |
...
syriana profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
syriana profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
syriana profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
syriana profile install ./telemetry/
```

### `syriana profile update`

```bash
syriana profile update <name> [--force-config] [--yes]
```
...
### `syriana profile info`

```bash
syriana profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Syriana version, author, env var requirements, the source URL/path, and
...
`syriana profile list` also shows the distribution name and version in a
`Distribution` column, and `syriana profile show <name>` / `delete <name>`
...
syriana profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
syriana profile install https://github.com/your-org/internal-assistant.git
```
...
`syriana_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Syriana version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Syriana can clone from.
3. Tell recipients to run `syriana profile install <your-repo-url>`.
...
## `syriana -p` / `syriana --profile`

```bash
syriana -p <name> <command> [options]
syriana --profile <name> <command> [options]
```

Global flag to run any Syriana command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
syriana -p work chat -q "Check the server status"
syriana --profile dev gateway start
syriana -p personal skills list
syriana -p work config edit
```

## `syriana completion`

```bash
syriana completion <shell>
```
...
- `syriana profile <TAB>` — subcommands (list, use, create, etc.)
- `syriana profile use <TAB>` — profile names
- `syriana -p <TAB>` — profile names
