...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Syriana new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Syriana how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Syriana installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Syriana to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Syriana but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
syriana skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.syriana/skills/`
...
```bash
# Check it's there
syriana skills list | grep arxiv
...
For how to ship skills in your own plugin, see [Build a Syriana Plugin → Bundle skills](/guides/build-a-syriana-plugin#bundle-skills).
...
When a skill with config is first loaded, Syriana prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
syriana skills config gif-search

# View all skill config
syriana config show | grep '^skills\.config'
```
...
```bash
mkdir -p ~/.syriana/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.syriana/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
syriana chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.syriana/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Syriana may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Syriana will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.syriana/skills/devops/`, `~/.syriana/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Syriana to update the skill with what you learned. Skills that aren't maintained become liabilities.
