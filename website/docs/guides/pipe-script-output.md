...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `syriana send`."
---

# Pipe Script Output to Messaging Platforms

`syriana send` is a small, scriptable CLI that pushes a message to any
messaging platform Syriana is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | syriana send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `syriana
...
syriana send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
syriana send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
syriana send --list

# Filter by platform
syriana send --list telegram
```
...
Any platform Syriana ships adapters for works as a target:
...
`syriana send` resolves the message body in this order:

1. **Positional argument** — `syriana send --to telegram "hi"`
2. **`--file PATH`** — `syriana send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | syriana send --to telegram`

When stdin is a TTY (no pipe), Syriana does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  syriana send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `syriana send` reuses your Syriana config, the same script works on
any host where Syriana is installed — no need to export bot tokens into
...
`syriana send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  syriana send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | syriana send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.syriana/bin/syriana send \
...
```bash
./train.py --epochs 200 && \
  syriana send --to telegram "training done" || \
  syriana send --to telegram "training failed (exit $?)"
```
...
syriana send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(syriana send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `syriana send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `syriana send` calls
the platform's REST endpoint directly using credentials from
`~/.syriana/.env` and `~/.syriana/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `syriana gateway start` and retry.
...
# Machine-readable
syriana send --list --json
```

The listing is built from `~/.syriana/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`syriana gateway
...
| Approach | Multi-platform | Reuses Syriana creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `syriana send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`syriana send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `syriana send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `syriana send` shares with cron delivery.
