---
sidebar_position: 8
title: "Use Voice Mode with Syriana"
description: "A practical guide to setting up and using Syriana voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Syriana
...
- you want Syriana sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Syriana.
...
## Step 1: make sure normal Syriana works first
...
Syriana supports both local and cloud speech stacks.
...
### If you use `syriana setup`

If you choose NeuTTS in the setup wizard, Syriana checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Syriana transcribes and responds
...
- asking Syriana to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Syriana loop.

## Tuning CLI behavior

### Silence threshold

If Syriana starts/stops too aggressively, tune:
...
Syriana stays a normal chat bot, but can speak replies.
...
- you want Syriana to function like a portable research or ops assistant
...
Syriana joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Syriana detects speech boundaries
- transcripts are posted in the associated text channel
- Syriana responds in text and audio
...
1. get text Syriana working
2. install `syriana-agent[voice]`
