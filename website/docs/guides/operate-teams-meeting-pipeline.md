...
```bash
syriana teams-pipeline validate
```
...
```bash
syriana teams-pipeline token-health
syriana teams-pipeline token-health --force-refresh
```
...
```bash
syriana teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
syriana teams-pipeline maintain-subscriptions
syriana teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Syriana cron (recommended if you already run the Syriana gateway)

Syriana ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.syriana/scripts/`. First create the script:

```bash
mkdir -p ~/.syriana/scripts
cat > ~/.syriana/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec syriana teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.syriana/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
syriana cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/syriana-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Syriana Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=syriana
EnvironmentFile=/etc/syriana/env
ExecStart=/usr/local/bin/syriana teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/syriana-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Syriana Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now syriana-teams-pipeline-maintain.timer
systemctl list-timers syriana-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/syriana teams-pipeline maintain-subscriptions >> /var/log/syriana/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.syriana/.env` at the top of a wrapper script that crontab calls.
...
```bash
syriana teams-pipeline subscriptions   # should show expirationDateTime advanced
syriana teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
syriana teams-pipeline list
syriana teams-pipeline list --status failed
syriana teams-pipeline show <job-id>
```

### Replay a stored job

```bash
syriana teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
syriana teams-pipeline fetch --meeting-id <meeting-id>
syriana teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `syriana teams-pipeline maintain-subscriptions --dry-run`
- inspect `syriana teams-pipeline list --status failed`
...
- run `syriana teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `syriana teams-pipeline run`
...
- [ ] `syriana teams-pipeline validate` returns an OK snapshot
- [ ] `syriana teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Syriana cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
