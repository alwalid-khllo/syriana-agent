...
The ACP adapter wraps Syriana' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
syriana acp / syriana-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.syriana/.env
  -> configure stderr logging
  -> construct SyrianaACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'syriana-agent[acp]==<version>' syriana-acp`, pointed at the `syriana-agent` PyPI release.
...
- `allow_once` -> Syriana `once`
- `allow_always` -> Syriana `always`
- reject options -> Syriana `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Syriana tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["syriana-acp"])
...
Instead it reuses Syriana' runtime resolver:

- `acp_adapter/auth.py`
- `syriana_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Syriana provider/credentials. It also always advertises a terminal setup auth method (`syriana-setup`, args `--setup`) so first-run registry clients can open Syriana' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.syriana/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `syriana-acp` toolset definition
- `syriana_cli/main.py` — `syriana acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `syriana-acp` script
