...
description: "How Syriana stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `syriana_cli/cron.py` | CLI `syriana cron` subcommands |
...
Jobs are stored in `~/.syriana/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `syriana cron` commands are run or during active CLI sessions.
...
```python
# ~/.syriana/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.syriana/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `syriana cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `syriana cron` CLI provides direct job management:

```bash
syriana cron list                    # Show all jobs
syriana cron create                  # Interactive job creation (alias: add)
syriana cron edit <job_id>           # Edit job configuration
syriana cron pause <job_id>          # Pause a running job
syriana cron resume <job_id>         # Resume a paused job
syriana cron run <job_id>            # Trigger immediate execution
syriana cron remove <job_id>         # Delete a job
```
