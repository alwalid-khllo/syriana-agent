...
description: "How to add a new inference provider to Syriana Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Syriana can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `syriana model` menu entries
...
1. `syriana_cli/auth.py` decides how credentials are found.
2. `syriana_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `syriana_cli/models.py` and `syriana_cli/main.py` make the provider show up in the CLI. (`syriana_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `syriana_cli/auth.py`
2. `syriana_cli/models.py`
3. `syriana_cli/runtime_provider.py`
4. `syriana_cli/main.py`
...
:::tip
`syriana_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `syriana setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$SYRIANA_HOME/plugins/model-providers/` both get picked up.
...
7. `syriana model` menu includes the provider
8. `syriana setup` wizard delegates to `main.py` automatically
...
User plugins at `$SYRIANA_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `syriana model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `syriana_cli/auth.py`
- `_PROVIDER_LABELS` in `syriana_cli/models.py`
- `_PROVIDER_ALIASES` in both `syriana_cli/auth.py` and `syriana_cli/models.py`
- CLI `--provider` choices in `syriana_cli/main.py`
...
## Step 2: Add auth metadata in `syriana_cli/auth.py`
...
- What env vars should Syriana check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `syriana_cli/models.py`
...
## Step 4: Resolve runtime data in `syriana_cli/runtime_provider.py`
...
Be careful with API-key precedence. Syriana already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `syriana_cli/main.py`

A provider is not discoverable until it shows up in the interactive `syriana model` flow.

Update these in `syriana_cli/main.py`:
...
:::tip
`syriana_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `syriana model` and `syriana setup` automatically.
:::
...
When you add a native provider, double-check that Syriana is only sending fields that provider actually understands.
...
- `tests/syriana_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/syriana_cli/test_model_switch_custom_providers.py` (and adjacent `tests/syriana_cli/test_model_switch_*.py`)
- `tests/syriana_cli/test_setup_model_provider.py`
...
python -m pytest tests/syriana_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/syriana_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m syriana_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m syriana_cli.main model
...
- [ ] `ProviderConfig` added in `syriana_cli/auth.py`
- [ ] aliases added in `syriana_cli/auth.py` and `syriana_cli/models.py`
- [ ] model catalog added in `syriana_cli/models.py`
- [ ] runtime branch added in `syriana_cli/runtime_provider.py`
- [ ] CLI wiring added in `syriana_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `syriana model` but not `syriana setup`
