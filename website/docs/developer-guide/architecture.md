...
sidebar_position: 1
title: "Architecture"
description: "Syriana Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Syriana Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ syriana_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # SyrianaCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── syriana_state.py           # SQLite session/state database with FTS5
├── syriana_constants.py       # SYRIANA_HOME, profile-aware paths
...
│
├── syriana_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `syriana` subcommands (large file)
...
│   ├── skills_config.py      # syriana skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # syriana tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # syriana gateway start/stop
│
...
```text
User input → SyrianaCLI.process_input()
...
Three discovery sources: `~/.syriana/plugins/` (user), `.syriana/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `syriana plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-syriana-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Syriana as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`syriana -p <name>`) gets its own SYRIANA_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
