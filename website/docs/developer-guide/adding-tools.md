...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Syriana Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Syriana tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Syriana core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Syriana Plugin](/guides/build-a-syriana-plugin)
...
2. **`toolsets.py`** — add tool name to `_SYRIANA_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `syriana_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `syriana_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `syriana chat -q "Use the weather tool for London"`
