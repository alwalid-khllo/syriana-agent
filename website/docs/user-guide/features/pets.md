...
Syriana can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<SYRIANA_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Syriana renders the sprite at full fidelity when your
...
syriana pets list
syriana pets list cat

# Install a pet and make it active in one step
syriana pets install boba --select
...
## `syriana pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `syriana pets list [query] [--limit N]` |
| List installed pets | `syriana pets list --installed` |
| Install a pet | `syriana pets install <slug> [--select] [--force]` |
...
| Preview/animate | `syriana pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `syriana pets off` |
| Remove an installed pet | `syriana pets remove <slug>` |
| Diagnose setup | `syriana pets doctor` |

`syriana pets show` flags:
...
Beyond installing pre-made pets from the gallery, Syriana can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `syriana pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `syriana tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `SYRIANA_PET_IMAGE_PROVIDER` env var (e.g. `SYRIANA_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Syriana is
...
Run `syriana pets doctor` — it reports:
