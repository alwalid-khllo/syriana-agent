...
description: "Customize Syriana Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Syriana Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `SYRIANA_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Syriana is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Syriana now seeds a default `SOUL.md` automatically in:

```text
~/.syriana/SOUL.md
```

More precisely, it uses the current instance's `SYRIANA_HOME`, so if you run Syriana with a custom home directory, it will use:

```text
$SYRIANA_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Syriana creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Syriana loads `SOUL.md` only from `SYRIANA_HOME`
- Syriana does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Syriana falls back to a built-in default identity
...
If Syriana loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `SYRIANA_HOME`, the personality belongs to the Syriana instance itself.

That also makes it easier to teach users:
- "Edit `~/.syriana/SOUL.md` to change Syriana' default personality."
...
- how Syriana should handle uncertainty, disagreement, or ambiguity
...
## What Syriana injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Syriana falls back to a built-in default identity ("You are Syriana Agent, an intelligent AI assistant created by syriana ai Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Syriana ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Syriana, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Syriana its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.syriana/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.syriana/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-syriana)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Syriana speaks
- `display.skin` and `/skin` affect how Syriana looks in the terminal
