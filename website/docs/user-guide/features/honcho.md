...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Syriana's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Syriana instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
syriana memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `syriana` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Syriana syncs them back at session init.
...
The `syriana honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `syriana memory setup honcho` (or run `syriana memory setup` and pick it from the list); the `syriana honcho` subcommand then appears on the next invocation.

```bash
syriana memory setup honcho    # Configure Honcho directly (works before activation)
syriana honcho status          # Connection status, config, and key settings
syriana honcho setup           # Redirects to `syriana memory setup` (post-activation alias)
syriana honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
syriana honcho peer            # Show or update peer names + dialectic reasoning level
syriana honcho mode            # Show or set recall mode (hybrid/context/tools)
syriana honcho tokens          # Show or set token budget for context and dialectic
syriana honcho identity        # Seed or show the AI peer's Honcho identity
syriana honcho sync            # Sync Honcho config to all existing profiles
syriana honcho peers           # Show peer identities across all profiles
syriana honcho sessions        # List known Honcho session mappings
syriana honcho map             # Map current directory to a Honcho session name
syriana honcho enable          # Enable Honcho for the active profile
syriana honcho disable         # Disable Honcho for the active profile
syriana honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `syriana honcho`

If you previously used the standalone `syriana honcho setup`:
...
No re-login or re-setup needed. Run `syriana memory setup` and select "honcho" — the wizard detects your existing config.
