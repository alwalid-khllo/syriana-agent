...
**Tool Search** is Syriana' opt-in progressive-disclosure layer for that
...
:::info Built-in Syriana tools never defer
The tools that make up Syriana' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_SYRIANA_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Syriana **unwraps the bridge** and
...
- **No JS sandbox.** Syriana uses the simpler "structured tools" mode
