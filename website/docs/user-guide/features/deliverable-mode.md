...
When Syriana Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.syriana/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.syriana/config.yaml` (switchable per session
via `/personality`).
...
If you use Syriana' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.syriana/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Syriana Agent's deliverable
