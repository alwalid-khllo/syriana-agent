...
chat completions — use your Syriana-managed provider subscription as their
...
| Use case | "Use Syriana as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Syriana stores
the refresh token in `~/.syriana/auth.json` — the same place all Syriana
...
Model:      Syriana-4-70B    # or Syriana-4.3-36B, Syriana-4-405B
```
...
interface in `syriana_cli/proxy/adapters/`.
...
If you see `not logged in`, run `syriana portal`. If you see
...
supports; check `portal.syriana.ai/models`.
...
[portal.syriana.ai](https://portal.syriana.ai).
...
`syriana_cli/proxy/adapters/<provider>.py` and registering it in
