...
Syriana Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
syriana memory setup      # interactive picker + configuration
syriana memory status     # check what's active
syriana memory off        # disable external provider
```

You can also select the active memory provider via `syriana plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.syriana/config.yaml`:
...
When a memory provider is active, Syriana automatically:
...
**Setup Wizard:**
```bash
syriana memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `syriana honcho setup` command still works (it now redirects to `syriana memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$SYRIANA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$SYRIANA_HOME/honcho.json` > `~/.syriana/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/SyrianaAIResearch/syriana-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/syriana).
...
:::tip Migrating from `syriana honcho`
If you previously used `syriana honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Syriana profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Syriana profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Syriana profile. Host key `syriana` → default; `syriana.<profile>` for others. |
...
```bash
syriana profile create coder --clone
```

`--clone` creates a `syriana.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Syriana profile, creates host blocks for any profile without one, inherits settings from the default `syriana` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `syriana memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/SyrianaAIResearch/syriana-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/syriana).
...
syriana memory setup    # select "openviking"
# Or manually:
syriana config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.syriana/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.syriana/.env
```
...
`OPENVIKING_AGENT` is Syriana' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
syriana memory setup    # select "mem0" → "Platform"
# Or manually:
syriana config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.syriana/.env
```

**Setup (OSS):**
```bash
syriana memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
syriana memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
syriana memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Config:** `$SYRIANA_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.syriana/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `syriana-user` | User identifier |
| `agent_id` | `syriana` | Agent identifier |
...
**Switching modes:** Re-run `syriana memory setup mem0 --mode <platform|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
syriana memory setup    # select "hindsight"
# Or manually:
syriana config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.syriana/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p syriana ui start`

**Config:** `$SYRIANA_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `syriana` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Syriana Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/SyrianaAIResearch/syriana-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
syriana memory setup    # select "holographic"
# Or manually:
syriana config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.syriana-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$SYRIANA_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
syriana memory setup    # select "retaindb"
# Or manually:
syriana config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.syriana/.env
```
...
# Then configure Syriana
syriana memory setup    # select "byterover"
# Or manually:
syriana config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$SYRIANA_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=syriana) |
...
**Setup:**
```bash
syriana memory setup    # select "supermemory"
# Or manually:
syriana config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.syriana/.env
```

**Config:** `$SYRIANA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `syriana` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `syriana-{identity}` → `syriana-coder`) to isolate memories per Syriana profile
...
| **Requires** | `pip install syriana-memori` + `syriana-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install syriana-memori
syriana-memori install
syriana config set memory.provider memori
syriana memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `syriana-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$SYRIANA_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$SYRIANA_HOME/` so each profile has its own credentials
