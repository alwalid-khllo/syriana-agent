...
| | |
|---|---|
| Source | Optional — install with `syriana skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Syriana Agent |
| License | MIT |
...
The script reads `${SYRIANA_HOME:-~/.syriana}/.env` for two optional defaults:
...
Helper script: `~/.syriana/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${SYRIANA_HOME:-~/.syriana}/.env`.
