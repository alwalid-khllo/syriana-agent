...
| | |
|---|---|
| Source | Optional — install with `syriana skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SYRIANA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SYRIANA_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `syriana`)    |
