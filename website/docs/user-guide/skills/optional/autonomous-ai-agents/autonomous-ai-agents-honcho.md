...
description: "Configure and use Honcho memory with Syriana -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Syriana -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `syriana skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Syriana

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Syriana profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/syriana#running-honcho-locally-with-syriana

### Verify

```bash
syriana honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Syriana profile's AI peer
...
Honcho models conversations as interactions between **peers**. Syriana creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Syriana instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Syriana run |
| `global` | Single session across all directories |

Manual override: `syriana honcho map my-project-name`
...
Each Syriana profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block syriana.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `syriana.coder` host block in `honcho.json`
...
```bash
syriana honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Syriana when Honcho memory is active.
...
Config file: `$SYRIANA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `syriana honcho setup`. Ensure `memory.provider: honcho` is in `~/.syriana/config.yaml`.

### Memory not persisting across sessions
Check `syriana honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `syriana profile create <name> --clone`. For existing profiles: `syriana honcho sync`.
...
| Command | Description |
|---------|-------------|
| `syriana honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `syriana honcho status` | Show resolved config, connection test, peer info for active profile |
| `syriana honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `syriana honcho disable` | Disable Honcho for the active profile |
| `syriana honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `syriana honcho peers` | Show peer identities across all profiles |
| `syriana honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `syriana honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `syriana honcho sessions` | List known directory-to-session-name mappings |
| `syriana honcho map <name>` | Map current working directory to a Honcho session name |
| `syriana honcho identity` | Seed AI peer identity or show both peer representations |
| `syriana honcho sync` | Create host blocks for all Syriana profiles that don't have one yet |
| `syriana honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Syriana + Honcho |
| `syriana memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `syriana memory status` | Show active memory provider and config |
| `syriana memory off` | Disable external memory provider |
