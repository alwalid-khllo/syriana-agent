...
| | |
|---|---|
| Source | Optional — install with `syriana skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Syriana or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Syriana. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.syriana/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.syriana/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Syriana integration, either:

- configure the server in `~/.syriana/config.yaml` using the `native-mcp` skill, or
...
### Syriana cannot see the deployed server

The server-building part may be correct while the Syriana config is not. Load the `native-mcp` skill and configure the server in `~/.syriana/config.yaml`, then restart Syriana.
