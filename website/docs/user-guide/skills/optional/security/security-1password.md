...
| | |
|---|---|
| Source | Optional — install with `syriana skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Syriana Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Syriana terminal calls (desktop app flow only)
...
### Service Account (recommended for Syriana)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${SYRIANA_HOME:-~/.syriana}/.env` (the skill will prompt for this on first load).
...
## Syriana Execution Pattern (desktop app flow)

Syriana terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/syriana-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/syriana-op.sock"
