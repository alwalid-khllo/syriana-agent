...
`~/.syriana/config.yaml`, `~/.syriana/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/syriana`:

```text
/etc/syriana/
├── config.yaml     # managed config layer (wins over ~/.syriana/config.yaml)
└── .env            # managed env layer (wins over ~/.syriana/.env + shell)
```
...
The location can be relocated with the `SYRIANA_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `SYRIANA_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Syriana.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export SYRIANA_MANAGED_DIR=/opt/org/syriana-policy
```

:::warning
A user who can set `SYRIANA_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `syriana doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/syriana/config.yaml` (managed) | `/etc/syriana/.env` (managed) |
| 2 | `~/.syriana/config.yaml` (user) | `~/.syriana/.env` (user) |
...
```bash
syriana config        # shows a header naming the managed source + the pinned keys
syriana doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Syriana refuses and names the source:

```bash
$ syriana config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/syriana/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `syriana config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/syriana

# Pin some config values for every user on this machine
sudo tee /etc/syriana/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/syriana/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/syriana
sudo chmod 0644 /etc/syriana/config.yaml /etc/syriana/.env
```

Changes take effect on the next Syriana start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`syriana doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Syriana as `root`), managed scope is advisory.
