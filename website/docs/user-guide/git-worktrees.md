...
title: "Git Worktrees"
description: "Run multiple Syriana agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Syriana Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Syriana so each session has a clean, isolated working directory.

## Why Use Worktrees with Syriana?

Syriana treats the **current working directory** as the project root:

- CLI: the directory where you run `syriana` or `syriana chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.syriana/config.yaml`
...
git worktree add ../repo-feature feature/syriana-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/syriana-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Syriana there:

```bash
cd ../repo-feature

# Start Syriana in the worktree
syriana
```
...
git worktree add ../repo-experiment-a feature/syriana-a
git worktree add ../repo-experiment-b feature/syriana-b
```
...
Each Syriana process:

- Works on its own branch (`feature/syriana-a` vs `feature/syriana-b`).
...
- Syriana checkpoint data under `~/.syriana/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Syriana experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/syriana-checkpoints-docs`, `feature/syriana-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Syriana from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `syriana -w` (Automatic Worktree Mode)

Syriana has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `syriana/syriana-<hash>`).
...
```bash
syriana -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `syriana -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Syriana session its own clean checkout.
