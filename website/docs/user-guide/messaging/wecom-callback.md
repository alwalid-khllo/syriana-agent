...
Connect Syriana to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Syriana supports two WeCom integration modes:
...
> Run `syriana gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Syriana decrypts the message, queues it for the agent
...
(Use `syriana gateway start` only after `syriana gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Syriana and the token the
...
in `~/.syriana/.env` values around `=` will also break signature checks. After
fixing, restart `syriana gateway run`.
...
Check `syriana gateway run` logs for the bound host/port. If the adapter bound to
