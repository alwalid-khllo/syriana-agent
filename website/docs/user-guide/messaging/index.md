...
description: "Chat with Syriana from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Syriana from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Syriana](/guides/use-voice-mode-with-syriana).
...
```mermaid
flowchart TB
    subgraph Gateway["Syriana Gateway"]
...
For group chats, hooks, and automation flows, Syriana supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Syriana keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Syriana does not hide failures just because the text resembles a silence token.
...
```bash
syriana gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
syriana gateway              # Run in foreground
syriana gateway setup        # Configure messaging platforms interactively
syriana gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo syriana gateway install --system   # Linux only: install a boot-time system service
syriana gateway start        # Start the default service
syriana gateway stop         # Stop the default service
syriana gateway status       # Check default service status
syriana gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Syriana Agent to the latest version |
...
Configure per-platform overrides in `~/.syriana/gateway.json`:
...
syriana pairing approve telegram XKGH5N7P

# Other pairing commands:
syriana pairing list          # View pending + approved users
syriana pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Syriana appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.syriana/config.yaml`:
...
Off by default. When enabled, Syriana prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.syriana/config.yaml`:
...
```bash
SYRIANA_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
syriana gateway install               # Install as user service
syriana gateway start                 # Start the service
syriana gateway stop                  # Stop the service
syriana gateway status                # Check status
journalctl --user -u syriana-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo syriana gateway install --system
sudo syriana gateway start --system
sudo syriana gateway status --system
journalctl -u syriana-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Syriana installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit syriana-gateway` (or `sudo systemctl edit syriana-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `syriana update`. When `syriana update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart syriana-gateway` command (it never blocks on an interactive password prompt).
...
```bash
syriana gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `syriana update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo syriana update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/syriana-gateway`:

```
syriana ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed syriana-gateway*, /usr/bin/systemctl --no-ask-password start syriana-gateway*, /usr/bin/systemctl --no-ask-password restart syriana-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Syriana will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Syriana installations on the same machine (with different `SYRIANA_HOME` directories), each gets its own systemd service name. The default `~/.syriana` uses `syriana-gateway`; other installations use `syriana-gateway-<hash>`. The `syriana gateway` commands automatically target the correct service for your current `SYRIANA_HOME`.
:::

### macOS (launchd)

```bash
syriana gateway install               # Install as launchd agent
syriana gateway start                 # Start the service
syriana gateway stop                  # Stop the service
syriana gateway status                # Check status
tail -f ~/.syriana/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.syriana.gateway.plist`. It includes three environment variables:
...
- **SYRIANA_HOME** — scopes the gateway to your Syriana installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `syriana gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `SYRIANA_HOME` directory gets its own launchd label. The default `~/.syriana` uses `ai.syriana.gateway`; other installations use `ai.syriana.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `syriana-cli` | Full access |
| Telegram | `syriana-telegram` | Full tools including terminal |
| Discord | `syriana-discord` | Full tools including terminal |
| WhatsApp | `syriana-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `syriana-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `syriana-slack` | Full tools including terminal |
| Google Chat | `syriana-google_chat` | Full tools including terminal |
| Signal | `syriana-signal` | Full tools including terminal |
| SMS | `syriana-sms` | Full tools including terminal |
| Email | `syriana-email` | Full tools including terminal |
| Home Assistant | `syriana-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `syriana-mattermost` | Full tools including terminal |
| Matrix | `syriana-matrix` | Full tools including terminal |
| DingTalk | `syriana-dingtalk` | Full tools including terminal |
| Feishu/Lark | `syriana-feishu` | Full tools including terminal |
| WeCom | `syriana-wecom` | Full tools including terminal |
| WeCom Callback | `syriana-wecom-callback` | Full tools including terminal |
| Weixin | `syriana-weixin` | Full tools including terminal |
| BlueBubbles | `syriana-bluebubbles` | Full tools including terminal |
| QQBot | `syriana-qqbot` | Full tools including terminal |
| Yuanbao | `syriana-yuanbao` | Full tools including terminal |
| Microsoft Teams | `syriana-teams` | Full tools including terminal |
| API Server | `syriana-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `syriana-webhook` | Full tools including terminal |
| Raft | `syriana-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.syriana/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
