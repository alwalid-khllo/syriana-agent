...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Syriana Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Syriana to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.syriana/.env`:
...
All standard Syriana commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.syriana/logs/gateway.log`
...
4. Consider enabling verbose logging: `SYRIANA_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Syriana automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
SYRIANA_LOG_LEVEL=debug syriana gateway
```
...
```bash
syriana chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
