...
Connect Syriana to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Syriana — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Syriana runs it in a
...
syriana photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Syriana Agent` project on your account.
...
Runtime credentials are written to `~/.syriana/.env`
...
(device token, dashboard project id) lives in `~/.syriana/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Syriana
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Syriana replies with a pairing code. Approve it with:

```bash
syriana pairing approve photon <CODE>
```

Use `syriana pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.syriana/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.syriana/.env`):
...
By default Syriana responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Syriana` and
`@Syriana agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Syriana will reply.

## Status & troubleshooting

```bash
syriana photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Syriana uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run syriana photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `syriana photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `syriana photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `syriana photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Syriana sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Syriana wake words  | JSON list / comma / newline regex patterns for group mentions |
