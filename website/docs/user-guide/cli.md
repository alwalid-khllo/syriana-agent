...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Syriana Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Syriana Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `syriana setup --portal` — and you're ready to `syriana chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Syriana also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `syriana --tui` — see the [TUI](tui.md) guide.
:::
...
syriana chat --model "anthropic/claude-sonnet-4"

# With a specific provider
syriana chat --provider nous        # Use Nous Portal
syriana chat --provider openrouter  # Force OpenRouter

# With specific toolsets
syriana chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
syriana -s syriana-agent-dev,github-auth
syriana chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
syriana --continue             # Resume the most recent CLI session (-c)
syriana --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
syriana chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
syriana -w                         # Interactive mode in worktree
syriana -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Syriana CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Syriana CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `SYRIANA_YOLO_MODE` is on (either `syriana --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`syriana -c` or `syriana --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Syriana to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Syriana supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Syriana replies |
...
```bash
syriana -s syriana-agent-dev,github-auth
syriana chat -s github-pr-workflow -s github-auth
```

Syriana loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.syriana/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.syriana/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Syriana..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Syriana recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Syriana — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Syriana is working, Syriana prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Syriana to the background — just like any terminal process. The shell prints a confirmation:

```
Syriana Agent has been suspended. Run `fg` to bring Syriana Agent back.
```
...
Resume this session with:
  syriana --resume 20260225_143052_a1b2c3
...
```bash
syriana --continue                          # Resume the most recent CLI session
syriana -c                                  # Short form
syriana -c "my project"                     # Resume a named session (latest in lineage)
syriana --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
syriana --resume "refactoring auth"         # Resume by title
syriana -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `syriana sessions rename <id> <title>` from the command line. Use `syriana sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Syriana's SQLite state database under `~/.syriana/state.db`. The database keeps:
...
Syriana immediately confirms the task and gives you back the prompt:
