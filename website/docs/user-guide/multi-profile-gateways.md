...
If you only run one Syriana agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Syriana agents that should all
...
(`ai.syriana.gateway-<name>.plist`) or systemd user service
(`syriana-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
syriana profile create coder
syriana profile create personal-bot
syriana profile create research
...
```bash
syriana config set gateway.multiplex_profiles true
syriana gateway restart
```

Equivalently, in the default profile's `~/.syriana/config.yaml`:
...
You do **not** run `syriana gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `syriana gateway start` / `run` is a
...
home). `syriana status` reports the multiplexer and the profiles it serves;
`syriana status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/syriana-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: syriana-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    syriana gateway "$action"
  else
    syriana -p "$profile" gateway "$action"
  fi
}
...
```bash
syriana-gateways start      # start every configured profile
syriana-gateways stop       # stop every configured profile
syriana-gateways restart    # restart all
syriana-gateways status     # status across all
syriana-gateways list       # delegates to `syriana gateway list`
```

:::tip
The `default` profile is targeted with `syriana gateway <action>` (no `-p`),
not `syriana -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `syriana -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.syriana.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/syriana-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.syriana.gateway.plist` /
`syriana-gateway.service`.
...
```bash
# Default profile
tail -f ~/.syriana/logs/gateway.log
tail -f ~/.syriana/logs/gateway.error.log

# Named profile
tail -f ~/.syriana/profiles/<name>/logs/gateway.log
tail -f ~/.syriana/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.syriana/logs/gateway.log ~/.syriana/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
syriana logs -f                  # follow default profile
syriana -p coder logs -f         # follow one profile
syriana logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
syriana profile list             # profiles + model + gateway state
syriana-gateways status          # full status across every profile
launchctl list | grep syriana    # macOS — PIDs and labels
systemctl --user list-units 'syriana-gateway-*'   # Linux — units
```
...
```
~/.syriana/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.syriana/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
syriana config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.syriana/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=syriana --why="gateways running" \
  sleep infinity &
...
`syriana-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.syriana/.env ~/.syriana/profiles/*/.env
```

## Updating the code

`syriana update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `syriana gateway start` after a previous `syriana gateway stop`. The
...
```bash
ps -ef | grep "syriana_cli.*-p <profile>"
cat ~/.syriana/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.syriana.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.syriana.gateway-<profile>.plist

# Linux
systemctl --user restart syriana-gateway-<profile>.service
```

### Health check

```bash
syriana doctor                  # default profile
syriana -p <profile> doctor     # one profile
```
