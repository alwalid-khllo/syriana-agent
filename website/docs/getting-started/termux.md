...
description: "Run Syriana Agent directly on an Android phone with Termux"
---

# Syriana on Android with Termux
...
Syriana Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Syriana from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `syriana` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `syriana` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/syriana" "$PREFIX/bin/syriana"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `syriana` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.syriana/.env`.
...
### `syriana doctor` says ripgrep or Node is missing
