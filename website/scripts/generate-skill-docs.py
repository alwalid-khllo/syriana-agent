...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/SyrianaAIResearch/syriana-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    syriana_meta = (fm.get("metadata") or {}).get("syriana") or {}
    tags = syriana_meta.get("tags") or []
    related = syriana_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `syriana skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Syriana Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Syriana ships with a large built-in skill library copied into `~/.syriana/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Syriana also syncs bundled skills on `syriana update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.syriana/skills/` tree, it is still shipped with Syriana; restore it with `syriana skills reset <name> --restore`.",
        "",
