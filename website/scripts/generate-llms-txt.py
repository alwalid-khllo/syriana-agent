#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Syriana docs site.
...
Both publish at:
  https://syriana-agent.fixology.dev/docs/llms.txt
  https://syriana-agent.fixology.dev/docs/llms-full.txt
...
SITE_BASE = "https://syriana-agent.fixology.dev/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Syriana as a Python Library", None),
        ("guides/use-mcp-with-syriana", "Use MCP with Syriana", None),
        ("guides/use-voice-mode-with-syriana", "Use Voice Mode with Syriana", None),
        ("guides/use-soul-with-syriana", "Use SOUL.md with Syriana", None),
        ("guides/build-a-syriana-plugin", "Build a Syriana Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Syriana"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Syriana Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by syriana ai Research. A terminal-native "
    ...
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/SyrianaAIResearch/"
        "syriana-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/SyrianaAIResearch/syriana-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Syriana Agent — Full Documentation\n",
        (
            "This file is the entire Syriana Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Syriana, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://syriana-agent.fixology.dev/docs\n",
        "Short index: https://syriana-agent.fixology.dev/docs/llms.txt\n",
        "\n---\n\n",
    ]
