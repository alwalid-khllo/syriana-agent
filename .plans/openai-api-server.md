# OpenAI-Compatible API Server for Syriana Agent
...
SSE streaming. By exposing this endpoint, syriana-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  syriana-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.syriana/.env`
2. Run `syriana gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with syriana-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns syriana-agent as a model) |
...
Authorization: Bearer syriana-api-key-here
...
But syriana-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "syriana-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured syriana-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of syriana-agent's core prompt
...
Once implemented, syriana-agent works as a drop-in backend for:
