...
# Python-driven tool session (e.g. Syriana terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:alwalid-khllo/syriana-agent.git"
REPO_URL_HTTPS="https://github.com/alwalid-khllo/syriana-agent.git"
SYRIANA_HOME="${SYRIANA_HOME:-$HOME/.syriana}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${SYRIANA_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$SYRIANA_INSTALL_DIR"
...
#   code at /usr/local/lib/syriana-agent, command at /usr/local/bin/syriana,
#   data still at /root/.syriana (SYRIANA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$SYRIANA_HOME/.no-bundled-skills so future"
            echo "                   'syriana update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Syriana.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.syriana/syriana-agent"
            echo "                   default (root, Linux): /usr/local/lib/syriana-agent"
            echo "  --syriana-home PATH  Data directory (default: ~/.syriana, or \$SYRIANA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Syriana installs the code under"
            echo "  /usr/local/lib/syriana-agent and links the command into"
            echo "  /usr/local/bin/syriana (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SYRIANA_HOME"
            echo "  (default /root/.syriana).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SYRIANA_HOME/syriana-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs syriana setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Syriana Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by syriana ai Research.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `syriana update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Syriana-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Syriana Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install syriana command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `syriana` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SYRIANA_HOME/syriana-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SYRIANA_HOME/syriana-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/syriana-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SYRIANA_HOME/syriana-agent — then preserve it)
#
# Always no-op when the user set --dir or $SYRIANA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SYRIANA_HOME.
    if is_termux; then
        INSTALL_DIR="$SYRIANA_HOME/syriana-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SYRIANA_HOME/syriana-agent/.git" ]; then
            INSTALL_DIR="$SYRIANA_HOME/syriana-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/syriana-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/syriana-agent"
        ...
        # /usr/local/bin/syriana wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/syriana"
        log_info "  Data:    $SYRIANA_HOME (unchanged)"
...
# Point a Syriana-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $SYRIANA_HOME/node/bin — which is NOT on
...
# Syriana's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Syriana-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$SYRIANA_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$SYRIANA_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$SYRIANA_HOME/node/etc/npmrc"
}

get_syriana_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/syriana" ]; then
        echo "$link_dir/syriana"
    else
        echo "syriana"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/alwalid-khllo/syriana-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Syriana owns its own uv at $SYRIANA_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (syriana_cli/managed_uv.py) looks in the same
    # place, so install.sh and `syriana update` stay in sync.
    local _managed_uv="$SYRIANA_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $SYRIANA_HOME/bin ..."
    mkdir -p "$SYRIANA_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/syriana-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/syriana-uv-installer.$$.sh")"
    ...
    # directly into $SYRIANA_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$SYRIANA_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Syriana-
...
check_node() {
    ...
    # Repair pre-existing Syriana-managed installs where `npm install -g` lands
    ...
    if [ -x "$SYRIANA_HOME/node/bin/node" ] && node_satisfies_build "$("$SYRIANA_HOME/node/bin/node" --version)"; then
        export PATH="$SYRIANA_HOME/node/bin:$PATH"
        log_success "Node.js $("$SYRIANA_HOME/node/bin/node" --version) found (Syriana-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Syriana-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.syriana/node/..."
    ...
    # Place into ~/.syriana/node/ and symlink binaries into the same bin dir
    # the syriana command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$SYRIANA_HOME/node"
    mkdir -p "$SYRIANA_HOME"
    mv "$extracted_dir" "$SYRIANA_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$SYRIANA_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$SYRIANA_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$SYRIANA_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$SYRIANA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.syriana/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Syriana install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Syriana Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `syriana update` path
                # (#4735).
                ...
                local stash_name
                stash_name="syriana-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``syriana update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Syriana behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up syriana command..."

    if [ "$USE_VENV" = true ]; then
        SYRIANA_BIN="$INSTALL_DIR/venv/bin/syriana"
    else
        SYRIANA_BIN="$(which syriana 2>/dev/null || echo "")"
        if [ -z "$SYRIANA_BIN" ]; then
            log_warn "syriana not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$SYRIANA_BIN" ]; then
        log_warn "syriana entry point not found at $SYRIANA_BIN"
    ...
    # Create a user-facing shim for the syriana command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $SYRIANA_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$SYRIANA_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/syriana"
    cat > "$command_link_dir/syriana" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SYRIANA_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/syriana"
    log_success "Installed syriana launcher → $command_link_display_dir/syriana"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "syriana command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v syriana' \
                >/dev/null 2>&1; then
        ...
        log_info "syriana not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Syriana Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Syriana Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Syriana Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so syriana works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.syriana directory structure (config at top level, code in subdir)
    mkdir -p "$SYRIANA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.syriana/.env (top level, easy to find)
    if [ ! -f "$SYRIANA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SYRIANA_HOME/.env"
            log_success "Created ~/.syriana/.env from template"
        else
            touch "$SYRIANA_HOME/.env"
            log_success "Created ~/.syriana/.env"
        fi
    else
        log_info "~/.syriana/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$SYRIANA_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.syriana/config.yaml (top level, easy to find)
    if [ ! -f "$SYRIANA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SYRIANA_HOME/config.yaml"
            log_success "Created ~/.syriana/config.yaml from template"
        fi
    else
        log_info "~/.syriana/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in syriana_cli/default_soul.py — the
    ...
    if [ ! -f "$SYRIANA_HOME/SOUL.md" ]; then
        cat > "$SYRIANA_HOME/SOUL.md" << 'SOUL_EOF'
You are Syriana Agent, an intelligent AI assistant created by syriana ai Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.syriana/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.syriana/"

    # Seed bundled skills into ~/.syriana/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `syriana update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'syriana update'." \
            > "$SYRIANA_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $SYRIANA_HOME/.no-bundled-skills"
        log_info "  Future 'syriana update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.syriana/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.syriana/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SYRIANA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$SYRIANA_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.syriana/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$SYRIANA_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Syriana Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Syriana will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Syriana Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (syriana --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `syriana update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'syriana setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m syriana_cli.main setup < /dev/tty
    else
        python -m syriana_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$SYRIANA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Syriana to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SYRIANA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'syriana whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                SYRIANA_CMD="$(get_syriana_command_path)"
                $SYRIANA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'syriana whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'syriana gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SYRIANA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SYRIANA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: syriana gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: syriana gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SYRIANA_CMD gateway > "$SYRIANA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.syriana/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: syriana gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: syriana gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SYRIANA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SYRIANA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SYRIANA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}syriana${NC}              Start chatting"
    echo -e "   ${GREEN}syriana setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}syriana config${NC}       View/edit configuration"
    echo -e "   ${GREEN}syriana config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}syriana gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}syriana update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'syriana' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'syriana' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'syriana' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$SYRIANA_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$SYRIANA_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$SYRIANA_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$SYRIANA_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$SYRIANA_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Syriana for pip install"
    ...
    SYRIANA_CMD="$(command -v syriana 2>/dev/null || echo "")"
    if [ -n "$SYRIANA_CMD" ]; then
        log_info "Running syriana setup..."
        "$SYRIANA_CMD" setup
    else
        log_warn "syriana command not found on PATH"
        log_info "Try: python -m syriana_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Syriana` rename
...
# _purge_electron_build_cache() used by `syriana desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Syriana-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `syriana desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Syriana" ]; then
            app="$desktop_dir/release/linux-unpacked/Syriana"
        elif [ -x "$desktop_dir/release/linux-unpacked/syriana" ]; then
            app="$desktop_dir/release/linux-unpacked/syriana"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Syriana.app" \
            "$desktop_dir/release/mac/Syriana.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Syriana is damaged and can't be opened").
    ...
    # checkout stays clean for the next `syriana update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Syriana-managed Node
            # provisioned during prerequisites/node-deps (at $SYRIANA_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $SYRIANA_HOME.
    # $SYRIANA_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'syriana update' on this host install.
