# ============================================================================
# Syriana Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/alwalid-khllo/syriana-agent/main/scripts/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$SyrianaHome = $(if ($env:SYRIANA_HOME) { $env:SYRIANA_HOME } else { "$env:LOCALAPPDATA\syriana" }),
    [string]$InstallDir = $(if ($env:SYRIANA_HOME) { "$env:SYRIANA_HOME\syriana-agent" } else { "$env:LOCALAPPDATA\syriana\syriana-agent" }),
    ...
    # builds apps/desktop into a launchable Syriana.exe.
    #
    # Why opt-in:
    #   * Syriana-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Syriana.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Syriana.exe
    ...
    #     `syriana desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:alwalid-khllo/syriana-agent.git"
$RepoUrlHttps = "https://github.com/alwalid-khllo/syriana-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Syriana Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by syriana ai Research.       |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $SyrianaHome)) {
        New-Item -ItemType Directory -Force -Path $SyrianaHome | Out-Null
    }
    $envFile = Join-Path $SyrianaHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $SyrianaHome "node"
...
function Install-Uv {
    # Syriana owns its own uv at $SyrianaHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (syriana_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `syriana update` stay in sync.
    $managedUv = Join-Path $SyrianaHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $SyrianaHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $SyrianaHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $SyrianaHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $SyrianaHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and SYRIANA_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Syriana-Setup.exe
...
function Install-Git {
    ...
    which Syriana uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\syriana\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Syriana needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Syriana copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\syriana\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SYRIANA_GIT_BASH_PATH`` (User scope) so Syriana can find it in a fresh
    ...
    # Download PortableGit into $SyrianaHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $SyrianaHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Syriana features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Syriana needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SYRIANA_GIT_BASH_PATH`` (User env scope) so Syriana can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $SyrianaHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $SyrianaHome\git\usr\bin\bash.exe
    $candidates += "$SyrianaHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$SyrianaHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SYRIANA_GIT_BASH_PATH", $candidate, "User")
            $env:SYRIANA_GIT_BASH_PATH = $candidate
            Write-Info "Set SYRIANA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Syriana may not find Git Bash."
    Write-Info "If needed, set SYRIANA_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    # Prefer a Syriana-managed Node from a previous run over a too-old system one.
    $managedNode = "$SyrianaHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$SyrianaHome\node;$env:Path"
        Write-Success "Node.js $version found (Syriana-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Syriana-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $SyrianaHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $SyrianaHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\syriana-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$SyrianaHome\node") { Remove-Item -Recurse -Force "$SyrianaHome\node" }
                Move-Item $extractedDir.FullName "$SyrianaHome\node"
                ...
                $version = & "$SyrianaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $SyrianaHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\syriana-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `syriana
                    # update` path (#4735).
                    ...
                    $stashName = "syriana-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``syriana update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Syriana behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running syriana processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/alwalid-khllo/syriana-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/alwalid-khllo/syriana-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/alwalid-khllo/syriana-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\syriana-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\syriana-agent-extract"
    ...
    # next `syriana update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Syriana-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running syriana process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running syriana processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Syriana_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (syriana.exe) plus its child tree.
            & taskkill /F /T /IM syriana.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM syriana.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m syriana_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not syriana.exe.
            ...
            # user (see syriana_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `syriana update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `syriana.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'syriana-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install syriana-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and syriana.exe broken with
    ...
    # here, not 30 seconds later when the user runs `syriana`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register syriana.exe in dist-info/RECORD but fail to
        ...
        # `syriana` command while syriana-agent.exe / syriana-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m syriana_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `syriana dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `syriana dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `syriana dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up syriana command..."
    
    if ($NoVenv) {
        $syrianaBin = "$InstallDir"
    } else {
        $syrianaBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so syriana is globally available
    # On Windows, the syriana.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$syrianaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$syrianaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $syrianaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SYRIANA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\\syriana instead
    # of the Unix default ~/.syriana
    $currentSyrianaHome = [Environment]::GetEnvironmentVariable("SYRIANA_HOME", "User")
    if (-not $currentSyrianaHome -or $currentSyrianaHome -ne $SyrianaHome) {
        [Environment]::SetEnvironmentVariable("SYRIANA_HOME", $SyrianaHome, "User")
        Write-Success "Set SYRIANA_HOME=$SyrianaHome"
    }
    $env:SYRIANA_HOME = $SyrianaHome
    
    # Update current session
    $env:Path = "$syrianaBin;$env:Path"
    
    Write-Success "syriana command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.syriana-bootstrap-complete which tells the Syriana
    ...
    # Syriana-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `syriana update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".syriana-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the SYRIANA_HOME directory structure ($SyrianaHome, default %LOCALAPPDATA%\\syriana)
    New-Item -ItemType Directory -Force -Path "$SyrianaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SyrianaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SyrianaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SyrianaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SyrianaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SyrianaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SyrianaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SyrianaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SyrianaHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$SyrianaHome\.env"
    ...
    # Create config.yaml
    $configPath = "$SyrianaHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Syriana's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$SyrianaHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in syriana_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $SyrianaHome"
    
    # Seed bundled skills into $SyrianaHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $SyrianaHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $SyrianaHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$SyrianaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $SyrianaHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Syriana-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'syriana setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\syriana-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\syriana-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\syriana-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Syriana` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Syriana.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_syriana_desktop command
    # resolves apps/desktop/release/win-unpacked/Syriana.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Syriana-managed
    ...
    # The --dir mode produces an unpacked Syriana.exe in
    ...
    # are NOT trying to work around). The Syriana icon + product name are
    # stamped onto Syriana.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\syriana-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Syriana.exe",
        "$desktopDir\release\win-arm64-unpacked\Syriana.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Syriana.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Syriana icon + identity are stamped onto Syriana.exe by the
    ...
    #    Syriana.exe. We deliberately do NOT point them at `syriana desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Syriana.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Syriana.lnk')
        )
        ...
        # with the Syriana icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.syriana/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'syriana setup'."
        return
    }
    ...
    # Run syriana setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m syriana_cli.main setup
    } else {
        python -m syriana_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $syrianaCmd = "$InstallDir\venv\Scripts\syriana.exe"
    if (-not (Test-Path $syrianaCmd)) {
        $syrianaCmd = "syriana"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$SyrianaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'syriana whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: syriana gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$SyrianaHome\logs\gateway.log"
            Start-Process -FilePath $syrianaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$SyrianaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: syriana gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: syriana gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$SyrianaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$SyrianaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SyrianaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SyrianaHome\syriana-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   syriana              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   syriana setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   syriana config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   syriana config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   syriana gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   syriana update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Syriana repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Syriana-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Syriana to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/alwalid-khllo/syriana-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
