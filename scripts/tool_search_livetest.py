#!/usr/bin/env python3
"""Live test harness for Syriana Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any syriana imports.
ORIGINAL_HOME = os.environ.get("SYRIANA_HOME")
ORIGINAL_AUTH = Path.home() / ".syriana" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.syriana/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.syriana/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="syriana_ts_live_"))
    syriana_home = home_dir / ".syriana"
    syriana_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, syriana_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".syriana" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, syriana_home / ".env")
        ...
        load_syriana_dotenv(syriana_home=str(Path.home() / ".syriana"))
    ...
    (syriana_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return syriana_home
...
def reset_module_state():
    """Drop cached modules so the new SYRIANA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "syriana_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["SYRIANA_HOME"] = str(home)
...
def main():
    ...
    # Restore original SYRIANA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SYRIANA_HOME"] = ORIGINAL_HOME
    else:
