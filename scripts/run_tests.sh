#!/usr/bin/env bash
# Canonical test runner for syriana-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.syriana/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.syriana/syriana-agent/venv"; do
...
if [ -f "$HOME/.syriana/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.syriana"
...
exec env -i \
  ...
  ${SYRIANA_RUN_SLOW_PET_TESTS:+SYRIANA_RUN_SLOW_PET_TESTS="$SYRIANA_RUN_SLOW_PET_TESTS"} \
