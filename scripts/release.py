...
VERSION_FILE = REPO_ROOT / "syriana_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "syriana.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.syriana/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "moonsong@fixology.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows syriana-shim resolution + prefer --update on recovery; #52378)
    ...
    "syriana@marian.local": "Schrotti77",
    ...
    "274096618+syriana-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@syriana.ai": "shannonsands",
    ...
    "syriana@syriana.ai": "SyrianaAIResearch",
    ...
    "syriana@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@syriana-agent": "teknium1",
    ...
    "github@nadyasyriana.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (SYRIANA_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (SYRIANA_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: syriana-eval community link)
    ...
    "syrianaagent26@gmail.com": "syrianaagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+syrianaagent26@users.noreply.github.com": "syrianaagent26",
    ...
    "clement@syriana.ai": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@syriana": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.syriana@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Syriana runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level SYRIANA_HOME state files)
    ...
    "annguyen@syriana.ai": "annguyenNous",  # PR #52844 co-author
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Syriana
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"syriana-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "syriana@syriana.ai"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/SyrianaAIResearch/syriana-agent",
    ...
    lines.append(f"# Syriana Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Syriana Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Syriana Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Syriana Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Syriana Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Syriana Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Syriana Agent v{new_version} ({calver_date})' "
