...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _syriana_env="${SYRIANA_HOME:-$HOME/.syriana}/.env"; [ -f "$_syriana_env" ] && grep -q "^GITHUB_TOKEN=" "$_syriana_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_syriana_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
