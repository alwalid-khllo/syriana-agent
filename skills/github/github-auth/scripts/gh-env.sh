#!/usr/bin/env bash
# GitHub environment detection helper for Syriana Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _syriana_env="${SYRIANA_HOME:-$HOME/.syriana}/.env"; [ -f "$_syriana_env" ] && grep -q "^GITHUB_TOKEN=" "$_syriana_env" 2>/dev/null; then
