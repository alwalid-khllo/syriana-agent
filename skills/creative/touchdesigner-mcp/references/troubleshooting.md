...
## 5. Syriana Configuration

### Config location

`$SYRIANA_HOME/config.yaml` (defaults to `~/.syriana/config.yaml` when `SYRIANA_HOME` is unset)
...
Restart the Syriana session for changes to take effect. The MCP connection is
