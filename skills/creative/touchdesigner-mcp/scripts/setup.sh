...
SYRIANA_HOME_DIR="${SYRIANA_HOME:-$HOME/.syriana}"
SYRIANA_CFG="${SYRIANA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Syriana config has twozero_td MCP entry ──
if [[ ! -f "$SYRIANA_CFG" ]]; then
    echo -e " ${FAIL} Syriana config not found at ${SYRIANA_CFG}"
    manual_steps+=("Create ${SYRIANA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$SYRIANA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Syriana config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Syriana config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$SYRIANA_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${SYRIANA_CFG} manually"); }
    manual_steps+=("Restart Syriana session to pick up config change")
fi
