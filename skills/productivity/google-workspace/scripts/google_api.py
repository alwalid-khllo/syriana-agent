#!/usr/bin/env python3
"""Google Workspace API CLI for Syriana Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Syriana-facing JSON contract and falls back to the Python client
...
from _syriana_home import get_syriana_home
...
def _gws_binary() -> str | None:
    override = os.getenv("SYRIANA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Syriana Agent")
