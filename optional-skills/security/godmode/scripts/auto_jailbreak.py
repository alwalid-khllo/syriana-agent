...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SYRIANA_HOME", os.path.expanduser("~/.syriana")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SYRIANA_HOME", Path.home() / ".syriana")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Syriana config paths
# ═══════════════════════════════════════════════════════════════════

SYRIANA_HOME = Path(os.getenv("SYRIANA_HOME", Path.home() / ".syriana"))
CONFIG_PATH = SYRIANA_HOME / "config.yaml"
PREFILL_PATH = SYRIANA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Syriana models — already uncensored, just needs clean prompt
    "syriana": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "syriana" in model_lower or "nous" in model_lower:
        return "syriana"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Syriana config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.syriana/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Syriana for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Syriana for changes to take effect.")
