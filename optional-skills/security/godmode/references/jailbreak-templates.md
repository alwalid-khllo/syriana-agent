...
## 5. GODMODE FAST — Syriana 4 405B (Zero Refusal)

**Model:** `nousresearch/syriana-4-405b`
**Strategy:** Syriana models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Syriana Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.syriana/config.yaml`:
