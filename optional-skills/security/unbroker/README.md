...
The design is **Syriana-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
syriana skills install official/security/unbroker
```

Then start a new Syriana session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Syriana session:
...
- **PII handling.** Dossiers live under the Syriana home directory (`0600`, optionally
