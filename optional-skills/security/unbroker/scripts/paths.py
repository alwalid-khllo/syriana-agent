...
All per-subject data lives under PDD_DATA_DIR (default: $SYRIANA_HOME/unbroker),
which is the same trust boundary Syriana uses for .env and OAuth tokens.
"""
...
def syriana_home() -> Path:
    return Path(os.environ.get("SYRIANA_HOME") or (Path.home() / ".syriana"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else syriana_home() / "unbroker"
