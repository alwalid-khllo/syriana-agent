#!/usr/bin/env python3
"""Canvas LMS API CLI for Syriana Agent.
...
def _check_config():
    ...
    if missing:
        syriana_env = os.path.join(
            os.environ.get("SYRIANA_HOME", os.path.expanduser("~/.syriana")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {syriana_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Syriana Agent"
    )
