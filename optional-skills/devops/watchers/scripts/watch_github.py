...
Usage (via cron with --no-agent):
    ...
    syriana cron create syriana-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$SYRIANA_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name syriana-issues --repo SyrianaAIResearch/syriana-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Syriana .env file
(``${SYRIANA_HOME:-~/.syriana}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Syriana-Watcher/1.0",
    }
