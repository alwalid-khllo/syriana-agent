...
Usage (via cron with --no-agent):
    ...
    syriana cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SYRIANA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Syriana-Watcher/1.0"})
