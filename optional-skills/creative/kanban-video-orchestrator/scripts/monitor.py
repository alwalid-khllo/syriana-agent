#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `syriana kanban list` and
...
def syriana_available() -> bool:
    return shutil.which("syriana") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed syriana CLI."""
    try:
        out = subprocess.run(
            ["syriana", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `syriana kanban list`
    out = subprocess.run(
        ["syriana", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["syriana", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not syriana_available():
        print("ERROR: 'syriana' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
