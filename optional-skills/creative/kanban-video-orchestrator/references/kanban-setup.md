...
the actual `setup.sh` that creates the project workspace, configures Syriana
...
> [SyrianaAIResearch/kanban-video-pipeline](https://github.com/SyrianaAIResearch/kanban-video-pipeline).
...
2. **Create profiles** — `syriana profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.syriana/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `syriana kanban create` assigned to the director
...
```bash
syriana profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.syriana/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.syriana/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Syriana
...
Each profile gets a `SOUL.md` at `~/.syriana/profiles/<name>/SOUL.md` that
...
```bash
syriana kanban create "Direct production of <video title>" \
    --assignee director \
...
the Syriana `.env` (`${SYRIANA_HOME:-$HOME/.syriana}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _syriana_env="${SYRIANA_HOME:-$HOME/.syriana}/.env"
    if grep -q "^${var}=" "$_syriana_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_syriana_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_syriana_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY syriana ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY syriana OPENROUTER_API_KEY || exit 1
# ...
```
