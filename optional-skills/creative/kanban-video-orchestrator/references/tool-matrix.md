...
Maps each role archetype to the Syriana skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public syriana-agent
...
## Syriana skills relevant to video production

### Visual / rendering skills (`syriana-agent/skills/creative/`)
...
### Design / pre-production skills (`syriana-agent/skills/creative/`)
...
### Audio / media skills (`syriana-agent/skills/creative/` + `skills/media/`)
...
These are **not** Syriana skills but external CLIs / APIs that profiles invoke.
...
## Built-in Syriana tools for media review

These are native Syriana tools — not invoked via terminal but through their own
...
key is present in `${SYRIANA_HOME:-~/.syriana}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Syriana profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${SYRIANA_HOME:-~/.syriana}/.env` → environment variable.
...
When a new Syriana-public video skill ships:
