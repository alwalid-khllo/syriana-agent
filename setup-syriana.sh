...
# 5. Symlinks the 'syriana' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Syriana Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/syriana-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink syriana into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up syriana command..."

SYRIANA_BIN="$SCRIPT_DIR/venv/bin/syriana"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SYRIANA_BIN" "$COMMAND_LINK_DIR/syriana"
echo -e "${GREEN}✓${NC} Symlinked syriana → $COMMAND_LINK_DISPLAY_DIR/syriana"
...
# ============================================================================
# Seed bundled skills into ~/.syriana/skills/
# ============================================================================

SYRIANA_SKILLS_DIR="${SYRIANA_HOME:-$HOME/.syriana}/skills"
mkdir -p "$SYRIANA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.syriana/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SYRIANA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  syriana status        # Check configuration"
if is_termux; then
    echo "  syriana gateway       # Run gateway in foreground"
else
    echo "  syriana gateway install # Install gateway service (messaging + cron)"
fi
echo "  syriana cron list     # View scheduled jobs"
echo "  syriana doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m syriana_cli.main setup
fi
