...
The PID file lives at ``{SYRIANA_HOME}/gateway.pid``.  SYRIANA_HOME defaults to
``~/.syriana`` but can be overridden via the environment variable.  This means
separate SYRIANA_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting SYRIANA_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("SYRIANA_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "syriana" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Syriana gateway lifecycle subcommand from a command line.
    ...
    test also matched ``syriana_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\syriana-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Syriana's
    ...
    for token in tokens:
        ...
        if basename in ("syriana-gateway", "syriana-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "syriana_cli.main" in joined
        or "syriana_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("syriana", "syriana.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `syriana gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Syriana-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Syriana gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a SYRIANA_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.syriana`` or ``$SYRIANA_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``syriana_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``SYRIANA_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"syriana_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit SYRIANA_HOME= on the argv. SYRIANA_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "syriana_home=" in command_lc and f"syriana_home={home_lc}" not in command_lc:
        return False
    return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `syriana status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``SYRIANA_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    SYRIANA_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different SYRIANA_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``syriana.service`` + ``syriana-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different SYRIANA_HOME. When two profile gateway
    # services share the same default ~/.syriana (SYRIANA_HOME not set
    ...
    # field is absent in markers written by older Syriana versions; treat
    ...
    replacer_home = record.get("replacer_syriana_home")
    if replacer_home is not None and replacer_home != str(get_syriana_home()):
        return False
    ...
    # ``syriana gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_syriana_home": str(get_syriana_home()),
