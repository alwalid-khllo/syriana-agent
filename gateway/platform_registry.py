...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``syriana setup`` display).
    ...
    # built-ins).  Used by ``syriana gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `syriana`
        # invocation -- including plain `syriana chat`, which never touches any
        ...
        # `syriana setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``syriana setup``/``gateway status``, channel
