...
Defenses 1 and 2 (the ``_SYRIANA_GATEWAY`` guard on ``syriana gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.syriana.gateway")``,
...
State lives in ``<SYRIANA_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_syriana_home() / "gateway" / "restart_loop.json"
