...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{SYRIANA_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``SYRIANA_HOME`` is a **durable** store — on Syriana
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``syriana gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting SYRIANA_HOME."""
    base = home if home is not None else get_syriana_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable SYRIANA_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (SYRIANA_HOME is a
    durable Fly volume on Syriana Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable SYRIANA_HOME volume is
