...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Syriana-Session-Id header; opt-in long-term memory scoping via X-Syriana-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Syriana-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists syriana-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Syriana sessions
- POST /api/sessions               — create an empty Syriana session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to syriana-agent
...
def _syriana_version() -> str:
    """Return the syriana-agent version string, or "dev" if it can't be resolved.

    Tries the installed package metadata first (authoritative for a pip/uv
    install), then the in-tree ``syriana_cli.__version__`` (covers editable /
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_syriana_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted SYRIANA_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # syriana_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same Syriana session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through syriana-agent's AIAgent.
    """
    ...
    def _resolve_max_concurrent_runs() -> int:
        ...
        default = 10
        try:
            from syriana_cli.config import cfg_get, load_config
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Syriana-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Syriana-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Syriana-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Syriana-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Syriana-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``syriana sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the syriana-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Syriana-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "syriana-agent", "version": _syriana_version()}
        )
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "status": "ok",
            "platform": "syriana-agent",
            "version": _syriana_version(),
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list syriana-agent and any configured model_routes aliases."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Syriana version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "syriana.api_server.capabilities",
            "platform": "syriana-agent",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Syriana AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Syriana-Session-Id",
                "session_key_header": "X-Syriana-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Syriana sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty Syriana session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({"object": "syriana.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "syriana.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "syriana.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "syriana.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Syriana-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Syriana-Session-Key"] = gateway_session_key
        return web.json_response(
            {
                "object": "syriana.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Syriana-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Syriana-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Syriana-Session-Key.  This
        # is independent of X-Syriana-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Syriana-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Syriana-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Syriana-Session-Id rejected: "
            ...
            # conversation map to the same Syriana session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``syriana.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Syriana-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Syriana-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["syriana"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-Syriana-Completed"] = "false"
            response_headers["X-Syriana-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Syriana-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Syriana-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Syriana-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Syriana-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Syriana-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: syriana.tool.progress`` SSE event so
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Syriana-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Syriana-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
        ...
        *route* is an optional ``model_routes`` entry (resolved from the
        ...
        def _run():
            ...
            try:
                ...
                # (e.g. X-Syriana-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Syriana-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "syriana.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Syriana-Relay
            ...
            # that is the exact surface the syriana-0day campaign abused to write
            # ~/.syriana/config.yaml and plant persistence. Sandboxing (Docker /
