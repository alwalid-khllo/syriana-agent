...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Syriana Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to SYRIANA_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/syriana), this function
    mirrors the bridge source to a writable SYRIANA_HOME location and returns that
    ...
    # Try SYRIANA_HOME location first
    syriana_home = get_syriana_home()
    syriana_home_bridge = syriana_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to SYRIANA_HOME if needed
    if syriana_home_bridge.exists():
        return syriana_home_bridge

    # Mirror the bridge source to SYRIANA_HOME
    try:
        syriana_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            syriana_home_bridge,
            dirs_exist_ok=False,
