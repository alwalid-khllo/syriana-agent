...
There are two ways to add a platform to the Syriana gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.syriana/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Syriana code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `syriana gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `syriana_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Syriana core.
...
And add it to the `syriana-gateway` composite:

```python
"syriana-gateway": {
    "includes": [..., "syriana-your-platform"]
}
```
...
## 12. Status Display (`syriana_cli/status.py`)
...
## 13. Gateway Setup Wizard (`syriana_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ syriana_cli/ toolsets.py \
  --include="*.py" -l | sort -u
