...
const logDir = join(process.env.SYRIANA_HOME?.trim() || join(homedir(), '.syriana'), 'logs')
...
// a real ~/.syriana (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('syriana-tui: parent lifecycle log unavailable\n')
    }
  }
}
