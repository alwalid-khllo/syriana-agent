...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SYRIANA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Syriana on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SYRIANA_TUI_TRUECOLOR=1.
