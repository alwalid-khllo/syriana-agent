...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because SYRIANA_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.SYRIANA_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.syriana', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `syriana-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.SYRIANA_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.SYRIANA_HEAPDUMP_MAX_BYTES?.trim()
