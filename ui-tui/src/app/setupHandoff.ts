import type { RunExternalProcess } from '@syriana/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`syriana ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching syriana: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`syriana ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
