...
React + Ink terminal UI for Syriana. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SYRIANA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `syriana setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.syriana/.syriana_history` or under `SYRIANA_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `syriana setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/syriana-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external syriana setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.syriana/tui-parent.log
    ...
    types/
      syriana-ink.d.ts            type declarations for @syriana/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent SyrianaCLI subprocess for slash commands
```