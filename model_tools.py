...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_syriana_home() under the active profile.
...
#   - cli.py, syriana_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("SYRIANA_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("SYRIANA_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by SYRIANA_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like syriana-cli)
    # is enabled, any tools belonging to a disabled toolset are strictly
    # stripped out. See issue #17309.
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                if toolset_name.startswith("syriana-"):
                    # Platform bundles (syriana-*) include _SYRIANA_CORE_TOOLS, so
    ...
    # window). Core Syriana tools (toolsets._SYRIANA_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from syriana_cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from syriana_cli.plugins import has_hook, invoke_hook
