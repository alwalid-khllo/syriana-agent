...
def _make_agent():
    syriana_home = Path(tempfile.mkdtemp(prefix="syriana-test-home-"))
    (syriana_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._syriana_home", syriana_home),
