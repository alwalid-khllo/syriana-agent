...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    syriana_state = ModuleType("syriana_state")
    syriana_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "syriana_state", syriana_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Syriana"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Syriana"
