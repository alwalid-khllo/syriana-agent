...
AlexKucera reported on Discord (2026-04-16) that after ``syriana update`` pulled
...
Opt-in — not part of default CI:
    SYRIANA_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.syriana/.env).
"""
...
LIVE = os.environ.get("SYRIANA_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set SYRIANA_LIVE_TESTS=1"),
