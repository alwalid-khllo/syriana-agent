...
closure the PR changed, against a real temp ``SYRIANA_HOME``.
"""
...
def _fake_switch_result():
    ...
    from syriana_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "syriana_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_syriana_home() / config.yaml`` — point it here.
    monkeypatch.setattr("syriana_constants.get_syriana_home", lambda: syriana_home)
    monkeypatch.setattr("syriana_cli.config.get_syriana_home", lambda: syriana_home)
    return cfg_path
