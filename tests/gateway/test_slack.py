...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.syriana."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /syriana, /btw, /stop,
        ...
        for expected in ("/syriana", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@syriana`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@syriana"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@syriana")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /syriana subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /syriana btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_syriana_prefix_still_works(self, adapter):
        """Backward compat: /syriana btw foo must still route to /btw foo.

        Old workspace manifests only declared /syriana as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_syriana_freeform_question(self, adapter):
        """/syriana <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/syriana",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_syriana_slash_stashes_context(self, adapter):
        """Legacy /syriana <subcommand> also stashes context."""
        command = {
            "command": "/syriana",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_syriana_question_does_not_stash_context(self, adapter):
        """Free-form /syriana <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/syriana",
