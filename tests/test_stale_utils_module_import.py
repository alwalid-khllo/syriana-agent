...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.syriana/syriana-agent/utils.py)
...
2. ``syriana update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``syriana_cli/main.py`` (the ``syriana update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``syriana_constants`` after the
