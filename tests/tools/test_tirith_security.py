...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._syriana_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(syriana_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(syriana_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_syriana_bin(self):
        """After _INSTALL_FAILED, manual install in SYRIANA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        syriana_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(syriana_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(syriana_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._syriana_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == syriana_bin
            assert _tirith_mod._resolved_path == syriana_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# SYRIANA_HOME isolation
# ---------------------------------------------------------------------------

class TestSyrianaHomeIsolation:
    def test_syriana_bin_dir_respects_syriana_home(self):
        """_syriana_bin_dir must use SYRIANA_HOME, not hardcoded ~/.syriana."""
        from tools.tirith_security import _syriana_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"SYRIANA_HOME": tmpdir}):
            result = _syriana_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_syriana_home(self):
        """_failure_marker_path must use SYRIANA_HOME, not hardcoded ~/.syriana."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"SYRIANA_HOME": "/custom/syriana"}):
            result = _failure_marker_path()
        assert result == "/custom/syriana/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets SYRIANA_HOME; verify it's active."""
        syriana_home = os.getenv("SYRIANA_HOME")
        assert syriana_home is not None, "SYRIANA_HOME should be set by conftest"
        assert "syriana_test" in syriana_home, "Should point to test temp dir"

    def test_get_syriana_home_fallback(self):
        """Without SYRIANA_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_syriana_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove SYRIANA_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".syriana")
            result = _get_syriana_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._syriana_bin_dir",
