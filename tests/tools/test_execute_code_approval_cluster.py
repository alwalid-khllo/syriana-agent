...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad SYRIANA_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SYRIANA_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("SYRIANA_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("SYRIANA_CRON_SESSION", "1")
    monkeypatch.delenv("SYRIANA_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_syriana_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SYRIANA_HOME": "/h", "SYRIANA_PROFILE": "p",
        "SYRIANA_CONFIG": "/c.yaml", "SYRIANA_ENV": "/e",
        # other SYRIANA_* → dropped (broad prefix removed)
        "SYRIANA_BASE_URL": "https://x", "SYRIANA_INTERACTIVE": "1",
        "SYRIANA_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("SYRIANA_HOME", "SYRIANA_PROFILE", "SYRIANA_CONFIG", "SYRIANA_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SYRIANA_BASE_URL", "SYRIANA_INTERACTIVE", "SYRIANA_KANBAN_DB",
...
def test_env_scrub_logs_dropped_syriana_vars(caplog):
    """Dropping a non-allowlisted, non-secret SYRIANA_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset SYRIANA_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "SYRIANA_HOME": "/h",          # allowlisted → kept, not logged
        "SYRIANA_BASE_URL": "https://x",   # dropped → logged
        "SYRIANA_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "SYRIANA_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "SYRIANA_HOME" in out and "PATH" in out
    assert "SYRIANA_BASE_URL" not in out and "SYRIANA_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "SYRIANA_BASE_URL" in msgs and "SYRIANA_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "SYRIANA_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped SYRIANA_* vars."""
    import logging
