...
def test_syriana_provider_subclass_exists():
    """SyrianaMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _SYRIANA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _SYRIANA_PROVIDER_CLS is not None
    assert issubclass(_SYRIANA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_syriana_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns SyrianaMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _SYRIANA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("SYRIANA_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _SYRIANA_PROVIDER_CLS is not None
    assert isinstance(provider, _SYRIANA_PROVIDER_CLS)
    assert provider._syriana_server_name == "srv"
