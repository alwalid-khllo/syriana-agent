"""Regression tests for SYRIANA_HOME override propagation onto the MCP loop.
...
get_syriana_home() inside the coroutine, most visibly OAuth token-store
...
def test_override_propagates_to_mcp_loop(tmp_path, monkeypatch, mcp_loop):
    ...
    process_home.mkdir()
    profile_home.mkdir()
    monkeypatch.setenv("SYRIANA_HOME", str(process_home))
    ...
    token = set_syriana_home_override(str(profile_home))
    try:
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: SyrianaTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(SyrianaTokenStorage("probe-srv")._tokens_path())
