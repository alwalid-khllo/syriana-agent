...
def _clean_state():
    ...
    saved = {}
    for k in ("SYRIANA_INTERACTIVE", "SYRIANA_GATEWAY_SESSION", "SYRIANA_EXEC_ASK", "SYRIANA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SYRIANA_INTERACTIVE"] = "1"
        session_key = os.getenv("SYRIANA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SYRIANA_INTERACTIVE or SYRIANA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
