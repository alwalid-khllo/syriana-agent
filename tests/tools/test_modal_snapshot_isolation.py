...
def _restore_tool_modules():
    original_syriana_home = os.environ.get("SYRIANA_HOME")
    original_modules = {
        name: module
        ...
        or name == "syriana_cli"
        or name.startswith("syriana_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_syriana_home is None:
            os.environ.pop("SYRIANA_HOME", None)
        else:
            os.environ["SYRIANA_HOME"] = original_syriana_home
        _reset_modules(("tools", "syriana_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    syriana_cli = types.ModuleType("syriana_cli")
    syriana_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["syriana_cli"] = syriana_cli
    syriana_home = tmp_path / "syriana-home"
    os.environ["SYRIANA_HOME"] = str(syriana_home)
    sys.modules["syriana_cli.config"] = types.SimpleNamespace(
        get_syriana_home=lambda: syriana_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="syriana-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": syriana_home / "modal_snapshots.json",
