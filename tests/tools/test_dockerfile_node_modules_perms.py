...
Older images made /opt/syriana/ui-tui and /opt/syriana/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV SYRIANA_TUI_DIR=/opt/syriana/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R syriana:syriana /opt/syriana/ui-tui" not in text
    assert "chown -R syriana:syriana /opt/syriana/node_modules" not in text
