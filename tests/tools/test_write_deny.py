...
class TestWriteDenyExactPaths:
    ...
    def test_syriana_env(self):
        # ``.env`` under the active SYRIANA_HOME (profile-aware, not just
        # ``~/.syriana``) must be write-denied. The hermetic test conftest
        # points SYRIANA_HOME at a tempdir — resolve via get_syriana_home()
        # to match the denylist.
        from syriana_constants import get_syriana_home
        path = str(get_syriana_home() / ".env")
        assert _is_write_denied(path) is True

    def test_syriana_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.syriana/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "syriana_root"
        ...
        # Sanity check: SYRIANA_HOME does point to the profile dir, not the root.
        from syriana_constants import get_syriana_home, get_default_syriana_root
        assert get_syriana_home() == profile_home
        assert get_default_syriana_root() == root
...
class TestWriteAllowed:
    ...
    def test_syriana_control_files_requested_writable(self):
        from syriana_constants import get_syriana_home
