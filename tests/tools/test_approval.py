...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("syriana_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("syriana_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestWindowsShellDestructiveCommands:
    def test_cmd_del_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd /c del /f /q C:\tmp\syriana-victim\file.txt"
        )
    ...
    def test_cmd_rmdir_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd.exe /k rmdir /s /q C:\tmp\syriana-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows cmd destructive delete"
    ...
    def test_powershell_remove_item_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"powershell -NoProfile -Command Remove-Item -Recurse -Force C:\tmp\syriana-victim"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_pwsh_rm_alias_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"pwsh -c rm -Recurse -Force C:\tmp\syriana-victim"
        )
        assert dangerous is True
        assert key is not None
        assert "delete" in desc.lower()
    ...
    def test_powershell_bare_remove_item_requires_approval(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            r"powershell Remove-Item -Recurse -Force C:\tmp\syriana-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows PowerShell destructive delete"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"SYRIANA_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_syriana_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.syriana/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_syriana_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $SYRIANA_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_syriana_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$SYRIANA_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSyrianaConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.syriana/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.syriana/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.syriana/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.syriana/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.syriana/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.syriana/config.yaml")
        assert dangerous is True
        assert "syriana config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.syriana/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_syriana_home_config(self):
    ...
    def test_sed_in_place_absolute_syriana_home_env(self):
        env_path = get_syriana_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_syriana_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $SYRIANA_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.syriana/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_syriana_home_config(self):
        config_path = get_syriana_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.syriana/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_syriana_home_env(self):
        env_path = get_syriana_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.syriana/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.syriana/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.syriana/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.syriana/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.syriana/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Syriana config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_syriana_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $SYRIANA_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.syriana/config.yaml/.env must require approval — the
    ...
    def test_cp_to_syriana_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.syriana/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Syriana-home prefixes must fold to ~/ and
    ~/.syriana/ in dangerous-command detection.
    ...
    SSH, and Syriana config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_syriana_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/SYRIANA_HOME so the fold is verified on the POSIX CI runner too."""
    ...
    def test_windows_syriana_home_config_folds(self, monkeypatch):
        # Syriana home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("SYRIANA_HOME", r"C:\Users\tester\.syriana")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.syriana\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.syriana/syriana-agent && source venv/bin/activate && python -m syriana_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m syriana_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m syriana_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "syriana_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart syriana-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_syriana_gateway_stop_detected(self):
        cmd = "syriana gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_syriana_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `syriana` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "syriana -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_syriana_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "syriana --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_syriana_gateway_multiple_flags_detected(self):
        cmd = "syriana -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_syriana_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "syriana -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_syriana_gateway_start_not_flagged(self):
        cmd = "syriana gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_syriana_detected(self):
        """pkill targeting syriana/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_syriana_detected(self):
        cmd = "killall syriana"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_sed_config_dangerous(self):
        """In-place edit of the Syriana security config via IFS must be caught."""
        cmd = "sed${IFS}-i ~/.syriana/config.yaml"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep syriana) bypasses the pkill/killall name-matching
    ...
    def test_pkill_syriana_still_detected(self):
    ...
    def test_kill_dollar_pidof_detected(self):
        """`kill $(pidof syriana)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        cmd = "kill -TERM $(pidof syriana_cli.main)"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()

    def test_kill_backtick_pidof_detected(self):
        cmd = "kill -9 `pidof syriana`"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True


class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Syriana service
    label achieves the same effect as `syriana gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_stop_syriana_detected(self):
        cmd = "launchctl stop ai.syriana.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "syriana" in desc.lower()

    def test_launchctl_kickstart_syriana_detected(self):
        cmd = "launchctl kickstart -k system/ai.syriana.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_syriana_detected(self):
        cmd = "launchctl bootout system/ai.syriana.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_syriana_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.syriana.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-Syriana label is out of scope for the
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("SYRIANA_GATEWAY_SESSION", "SYRIANA_CRON_SESSION",
                      "SYRIANA_YOLO_MODE",
                      "SYRIANA_SESSION_KEY", "SYRIANA_INTERACTIVE")
        }
        os.environ.pop("SYRIANA_YOLO_MODE", None)
        os.environ.pop("SYRIANA_INTERACTIVE", None)
        # SYRIANA_CRON_SESSION takes priority over SYRIANA_GATEWAY_SESSION in
        ...
        os.environ.pop("SYRIANA_CRON_SESSION", None)
        os.environ["SYRIANA_GATEWAY_SESSION"] = "1"
        os.environ["SYRIANA_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("syriana_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"SYRIANA_INTERACTIVE": "1"}, clear=False):
