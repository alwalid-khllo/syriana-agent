...
The ``syriana update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m syriana_cli.main gateway run`` — image name
    ``pythonw``, not ``syriana.exe`` — so the ``taskkill /IM syriana.exe`` guard
    ...
    # The syriana.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM syriana.exe' in text
