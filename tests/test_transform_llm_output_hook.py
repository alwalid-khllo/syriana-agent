...
from syriana_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(syriana_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <syriana_home>/plugins/<name> and opt it in."""
    plugin_dir = syriana_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    syriana_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        syriana_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SYRIANA_HOME", str(tmp_path / "syriana_empty"))
