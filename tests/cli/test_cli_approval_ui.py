...
class TestCliApprovalUi:
    ...
    def test_background_task_registers_thread_local_approval_callbacks(self):
        ...
        assert seen["approval"].__func__ is SyrianaCLI._approval_callback
        assert seen["sudo"].__self__ is cli
        assert seen["sudo"].__func__ is SyrianaCLI._sudo_password_callback
...
def _make_real_paint_cli_stub():
    ...
    cli._paint_now = SyrianaCLI._paint_now.__get__(cli, SyrianaCLI)
    cli._invalidate = SyrianaCLI._invalidate.__get__(cli, SyrianaCLI)
