...
Uses a throwaway SYRIANA_HOME so it never touches ~/.syriana. No real names.
"""
...
def _fresh_home():
    """Point SYRIANA_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_syriana_home() at import time)."""
...
# Import AFTER SYRIANA_HOME is set.
...
# a different get_syriana_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"SYRIANA_HOME (throwaway): {HOME}")
