...
Override the image with ``SYRIANA_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("SYRIANA_TEST_IMAGE", "syriana-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``SYRIANA_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("SYRIANA_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged syriana user
# ---------------------------------------------------------------------------
#
# Background: every Syriana runtime path inside the container drops to UID
# 10000 (the ``syriana`` user) via ``s6-setuidgid syriana``. ``docker exec``
...
# fails (PermissionError swallowed) for syriana, so a test that ran as root
...
# helpers below run every probe under ``-u syriana`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: syriana).
