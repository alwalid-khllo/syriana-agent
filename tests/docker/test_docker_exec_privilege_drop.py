...
The shim (docker/syriana-exec-shim.sh, installed at /opt/syriana/bin/syriana)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> syriana login` would write /opt/data/auth.json as
...
failed: Syriana is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> syriana …`` (defaulting to root) gets dropped to the
   syriana user before the real binary runs.
2. ``docker exec --user syriana <c> syriana …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $SYRIANA_HOME from a ``docker exec`` session land
   as syriana:syriana — the actual user-visible invariant.
4. The SYRIANA_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-syriana-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown syriana:syriana`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to syriana:syriana,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$SYRIANA_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (syriana_cli.container_boot), which s6 runs
    *strictly after* ``01-syriana-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_syriana_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `syriana` with a Python-style `-c` shim equivalent — there's no
    pure-syriana "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `syriana` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `syriana`
    do anything that writes to disk, then check the file's owner.

    Use `syriana config set` which writes config.yaml under SYRIANA_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "syriana", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by syriana, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "syriana:syriana", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected syriana:syriana. "
        "The shim did not drop privileges before invoking syriana."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user syriana already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "syriana", sleep_container,
         "syriana", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user syriana failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up syriana:syriana — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """SYRIANA_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "SYRIANA_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
         "syriana", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "root:root", (
        f"With SYRIANA_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``SYRIANA_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``SYRIANA_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"SYRIANA_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
         "syriana", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "syriana:syriana", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected syriana:syriana"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/syriana/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid syriana syriana <args>` which resolves `syriana`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> syriana login`` (root) wrote
    ...
    "Syriana is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `syriana config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is syriana-readable.

    Specifically: pretend the operator ran `syriana login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the syriana UID. We use `syriana auth list` since that touches the
    ...
    # `syriana auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `syriana config set` first to
    # provoke a write into SYRIANA_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "syriana", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # SYRIANA_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "syriana", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the syriana user "
