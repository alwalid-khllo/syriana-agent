"""Runtime smoke test for Docker $SYRIANA_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the syriana user after container boot.
...
registered later runs its log service as the dropped syriana user and
...
def test_logs_gateways_seeded_and_syriana_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by syriana after boot."""
    ...
    assert "logs=syriana" in r.stdout, (
        f"logs/ not owned by syriana: {r.stdout}"
    )
    assert "gateways=syriana" in r.stdout, (
        f"logs/gateways/ not owned by syriana: {r.stdout}"
    )
