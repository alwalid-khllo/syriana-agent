"""Tests for syriana_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Syriana entry point
(syriana, syriana-agent, syriana-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Syriana entry point imports syriana_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported syriana_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("syriana_bootstrap", None)
    import syriana_bootstrap  # noqa: WPS433
    return syriana_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Syriana CLI
...
class TestEntryPointsImportBootstrap:
    """Every Syriana entry point must import syriana_bootstrap as its
    ...
    # Entry points that invoke Syriana as a process.  Each one must
    # import syriana_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "syriana_cli/main.py",   # syriana CLI (console_script)
        "run_agent.py",          # syriana-agent (console_script)
        "acp_adapter/entry.py",  # syriana-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import syriana_bootstrap' and that
        ...
        half-finished ``syriana update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``syriana_bootstrap`` as a top-level module) leaves syriana
        ...
        """
        # Resolve relative to the syriana-agent repo root.  Tests live
        # at tests/test_syriana_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # syriana start even when syriana_bootstrap hasn't been
        ...
        assert first_import_name == "syriana_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'syriana_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import syriana_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Syriana's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("SYRIANA_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["SYRIANA_PYTHON_SRC_ROOT"] = env
            elif "SYRIANA_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["SYRIANA_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/syriana")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SYRIANA_PYTHON_SRC_ROOT", None)
            else:
                os.environ["SYRIANA_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/syriana", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Syriana root.  The guard must relocate Syriana to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/syriana"])
        assert result[0] == "/opt/syriana"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Syriana root.
        assert result.index("/opt/syriana") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/syriana", "/opt/syriana", ""])
        assert result.count("/opt/syriana") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["SYRIANA_PYTHON_SRC_ROOT"] = "/env/syriana"
            hb.harden_import_path()
            assert sys.path[0] == "/env/syriana"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SYRIANA_PYTHON_SRC_ROOT", None)
