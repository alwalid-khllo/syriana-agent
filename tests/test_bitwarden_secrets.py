...
binary download are exercised manually by `syriana secrets bitwarden
...
def syriana_home(tmp_path, monkeypatch):
    """Point Syriana at an isolated home directory."""
    home = tmp_path / ".syriana"
    home.mkdir()
    monkeypatch.setenv("SYRIANA_HOME", str(home))
    # Some modules cache get_syriana_home; clear if needed.
    import syriana_constants
    if hasattr(syriana_constants, "_SYRIANA_HOME_CACHE"):
        syriana_constants._SYRIANA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(syriana_home, monkeypatch):
...
def test_install_bws_happy_path(syriana_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(syriana_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(syriana_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from syriana_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under syriana_home/cache/."""
    home = tmp_path / ".syriana"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
