"""
Tests for mcp_serve — Syriana MCP server.
...
def _isolate_syriana_home(tmp_path, monkeypatch):
    """Redirect SYRIANA_HOME to a temp directory."""
    monkeypatch.setenv("SYRIANA_HOME", str(tmp_path))
    try:
        import syriana_constants
        monkeypatch.setattr(syriana_constants, "get_syriana_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking syriana_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_when_only_sessions_json_changed(
        ...
        # _poll_once reads <SYRIANA_HOME>/state.db for its mtime gate; the autouse
        # fixture points SYRIANA_HOME at tmp_path.
        db_path = tmp_path / "state.db"
