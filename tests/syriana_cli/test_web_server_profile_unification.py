...
profile switcher can target any profile's SYRIANA_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via SYRIANA_HOME.
"""
import pytest
import yaml
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_syriana_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_syriana_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import syriana_cli.mcp_config as mcp_config
        from syriana_constants import get_syriana_home
        ...
        def fake_probe(name, config, connect_timeout=30, details=None):
            seen["home"] = str(get_syriana_home())
            return [("tool-a", "desc")]
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same SYRIANA_HOME the rest
        ...
        monkeypatch.setattr(
            web_server,
            "_spawn_syriana_action",
            lambda subcommand, name: calls.append(list(subcommand)) or _FakeProc(),
        )
        monkeypatch.setattr(
            "syriana_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid():
            seen_homes.append(str(get_syriana_home()))
            return None
        ...
        assert resp.json()["syriana_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_syriana_home(self, isolated_profiles, monkeypatch):
        import syriana_cli.web_server as web_server

        monkeypatch.setattr(
            "syriana_cli.main._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = web_server._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["SYRIANA_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "SYRIANA_TUI_GATEWAY_URL" not in env
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("SYRIANA_HOME") != str(isolated_profiles["worker_beta"])
