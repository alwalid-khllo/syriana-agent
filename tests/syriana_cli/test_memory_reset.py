"""Tests for the `syriana memory reset` CLI command.
...
- Profile-scoped reset (uses SYRIANA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SYRIANA_HOME with memory files."""
    syriana_home = tmp_path / ".syriana"
    memories = syriana_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("SYRIANA_HOME", str(syriana_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nSyriana repo is at ~/.syriana/syriana-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `syriana memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        syriana_home = tmp_path / ".syriana"
        (syriana_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("SYRIANA_HOME", str(syriana_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's SYRIANA_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_syriana_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
