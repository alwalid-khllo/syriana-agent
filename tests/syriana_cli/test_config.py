"""Tests for syriana_cli configuration management."""
...
class TestGetSyrianaHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert home == Path.home() / ".syriana"

    def test_env_override(self):
        with patch.dict(os.environ, {"SYRIANA_HOME": "/custom/path"}):
            home = get_syriana_home()
            assert home == Path("/custom/path")


class TestEnsureSyrianaHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"SYRIANA_HOME": str(tmp_path)}):
            ensure_syriana_home()
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from syriana_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from syriana_cli.default_soul import _LEGACY_TEMPLATE_SOULS
    ...
    def test_existing_named_profile_still_bootstraps_subdirs(self, tmp_path):
        profile_home = tmp_path / ".syriana" / "profiles" / "coder"
        profile_home.mkdir(parents=True)
        with patch.dict(os.environ, {"SYRIANA_HOME": str(profile_home)}):
            ensure_syriana_home()
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``syriana logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from syriana_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"SYRIANA_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="syriana_cli.config"):
                config = load_config()
            ...
            # stands out at syriana startup before logging is configured)
            captured = capsys.readouterr()
            assert "syriana config:" in captured.err
    ...
    def test_dedup_on_repeated_load_same_file(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"SYRIANA_HOME": str(tmp_path)}):
            ...
            assert "syriana config:" in first
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"SYRIANA_HOME": str(tmp_path)}):
            ...
            assert "syriana config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"SYRIANA_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SYRIANA_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SYRIANA_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"SYRIANA_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SYRIANA_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SYRIANA_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from syriana_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """SYRIANA_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `syriana setup`) lets a
        ...
        via config.yaml; SYRIANA_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from syriana_cli.config import OPTIONAL_ENV_VARS
        assert "SYRIANA_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `syriana setup` surface it (previously only Honcho was listed, leaving
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``SYRIANA_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Syriana
    process on next startup via the dotenv → ``os.environ`` chain in
    ``syriana_cli/env_loader.py``.
    ...
    def _syriana_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "SYRIANA_SPOTIFY_CLIENT_ID",
            "SYRIANA_QWEN_BASE_URL",
            "SYRIANA_MAX_ITERATIONS",
        ],
    )
    def test_syriana_integration_keys_still_writable(self, allowed_key):
        """``SYRIANA_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``SYRIANA_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``SYRIANA_*``) keep working. The denylist
