...
When ``SYRIANA_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.syriana/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.syriana            (has its own auth.json fixture)
    * Profile     -> tmp_path/.syriana/profiles/coder   (active, SYRIANA_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Syriana is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from syriana_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    syriana_home = tmp_path / "classic"
    syriana_home.mkdir()
    monkeypatch.setenv("SYRIANA_HOME", str(syriana_home))

    _write(syriana_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (SYRIANA_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.syriana as the "real home" — which is NOT equal
    # to the SYRIANA_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    syriana_home = tmp_path / "classic"
    ...
    _write(syriana_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from syriana_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: SYRIANA_HOME is set to a custom path that is NOT under
    # ~/.syriana/profiles/ — get_default_syriana_root() returns SYRIANA_HOME
