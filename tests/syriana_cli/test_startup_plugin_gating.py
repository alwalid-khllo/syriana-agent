...
``syriana_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``syriana --help``, ``syriana version``,
``syriana logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``syriana --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from syriana_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["syriana", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["syriana", "-w", "--tui"], None),
        (["syriana", "version"], "version"),
        (["syriana", "--tui", "chat"], "chat"),
        (["syriana", "-w", "logs"], "logs"),
        (["syriana", "chat", "hello world"], "chat"),
        (["syriana", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["syriana", "-m", "gpt5", "chat"], "chat"),
        (["syriana", "--model", "gpt5", "chat", "hi"], "chat"),
        (["syriana", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["syriana", "-z", "hello world"], None),
        (["syriana", "-z", "hello", "chat"], "chat"),
        (["syriana", "--model=gpt5", "chat"], "chat"),     # inline form
        (["syriana", "--", "chat"], "chat"),               # -- terminator
        (["syriana", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["syriana", "some-plugin-cmd"], "some-plugin-cmd"),
        (["syriana", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["syriana"],                          # bare → chat
        ["syriana", "--help"],                # top-level help
        ["syriana", "-h"],
        ["syriana", "version"],               # known built-in
        ["syriana", "logs"],
        ["syriana", "gateway", "run"],
        ["syriana", "--tui"],
        ["syriana", "-w", "--tui"],
        ["syriana", "chat", "hi"],
        ["syriana", "help"],                  # accepted built-in-ish
        ["syriana", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["syriana", "meet", "join"],          # potential google_meet plugin
        ["syriana", "honcho", "status"],      # potential memory plugin
        ["syriana", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``syriana help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"syriana_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
