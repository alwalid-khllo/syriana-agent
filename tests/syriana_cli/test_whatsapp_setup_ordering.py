...
Before the fix, ``syriana whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``syriana gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    syriana = home / ".syriana"
    syriana.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("SYRIANA_HOME", str(syriana))
...
def _env_value(syriana_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from syriana_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("syriana_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``syriana whatsapp`` with an existing paired session and
