"""Tests for IRC gateway configuration via `syriana setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Syriana install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Syriana install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#syriana")
            monkeypatch.setenv("IRC_NICKNAME", "syriana-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `syriana setup gateway` experience with IRC present."""
