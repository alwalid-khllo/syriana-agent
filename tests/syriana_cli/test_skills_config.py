"""Tests for syriana_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("syriana_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from syriana_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("syriana_cli.config.load_config")
    ...
    @patch("syriana_cli.config.load_config")
    @patch.dict("os.environ", {"SYRIANA_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("SYRIANA_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """SYRIANA_SESSION_PLATFORM should be used when SYRIANA_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_syriana_platform_takes_precedence(self, tmp_path, monkeypatch):
        """SYRIANA_PLATFORM should win over SYRIANA_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("SYRIANA_HOME", str(tmp_path))
        monkeypatch.setenv("SYRIANA_PLATFORM", "telegram")
        monkeypatch.setenv("SYRIANA_SESSION_PLATFORM", "discord")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from syriana_cli.skills_config import _get_categories
        skills = [
