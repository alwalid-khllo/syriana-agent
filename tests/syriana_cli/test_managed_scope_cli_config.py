...
syriana_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `syriana config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_syriana_home = get_syriana_home()`` at import time (module
    singleton), so monkeypatching SYRIANA_HOME after import doesn't move it.
    ...
    this only matters for tests that swap SYRIANA_HOME.)
    """
    import cli

    cli._syriana_home = home
