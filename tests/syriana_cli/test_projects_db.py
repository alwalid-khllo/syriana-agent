"""Tests for the per-profile Projects store (syriana_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Syriana Agent", folders=["/tmp/syriana"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "syriana-agent"
    assert proj.name == "Syriana Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/syriana"
    assert [f.path for f in proj.folders] == ["/tmp/syriana"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "syriana-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Syriana Agent")
    pdb.create_project(conn, name="Syriana Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["syriana-agent", "syriana-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' SYRIANA_HOME.
...
def test_db_path_under_syriana_home():
    # Resolves under SYRIANA_HOME (set by the autouse isolation fixture).
