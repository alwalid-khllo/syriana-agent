...
the SAME provider universe as ``syriana model`` (the CLI/TUI picker).
...
def test_every_syriana_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `syriana model` universe."""
    ...
    assert not missing, (
        "providers shown in `syriana model` but not configurable in the desktop "
