...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (SYRIANA_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via SYRIANA_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in SYRIANA_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from syriana_cli.providers import SYRIANA_OVERLAYS
        assert "bedrock" in SYRIANA_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from syriana_cli.providers import SYRIANA_OVERLAYS
        assert SYRIANA_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from syriana_cli.providers import SYRIANA_OVERLAYS
        assert SYRIANA_OVERLAYS["bedrock"].auth_type == "aws_sdk"
