...
The cache exists to keep `syriana tools` → "All Platforms" fast: every
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from syriana_cli import config as config_mod
    from syriana_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_syriana_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    nonexistent = Path(tempfile.gettempdir()) / "syriana-test-no-such-env-xyz123.env"
    nonexistent.unlink(missing_ok=True)

    try:
        with patch("syriana_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
