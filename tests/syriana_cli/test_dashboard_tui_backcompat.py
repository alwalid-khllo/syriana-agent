"""Regression test: `syriana dashboard --tui` must not hard-crash.

Older Syriana desktop app shells (<= 0.15.x) spawn the backend as::

    syriana dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Syriana couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real syriana_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "syriana_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
