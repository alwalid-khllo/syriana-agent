...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SYRIANA_HOME with .container-mode file."""
    syriana_home = tmp_path / ".syriana"
    syriana_home.mkdir()
    monkeypatch.setenv("SYRIANA_HOME", str(syriana_home))
    monkeypatch.delenv("SYRIANA_DEV", raising=False)

    container_mode = syriana_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=syriana-agent\n"
        "exec_user=syriana\n"
        "syriana_bin=/data/current-package/bin/syriana\n"
    )
    return syriana_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("syriana_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "syriana-agent"
    assert info["exec_user"] == "syriana"
    assert info["syriana_bin"] == "/data/current-package/bin/syriana"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("syriana_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_syriana_dev(container_env, monkeypatch):
    """Returns None when SYRIANA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("SYRIANA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_syriana_dev_zero(container_env, monkeypatch):
    """SYRIANA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("SYRIANA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        syriana_home = Path(tmpdir) / ".syriana"
        syriana_home.mkdir()
        (syriana_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("syriana_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_syriana_home": lambda: syriana_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SYRIANA_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=syriana-custom\n"
        "exec_user=myuser\n"
        "syriana_bin=/opt/syriana/bin/syriana\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "syriana-custom"
    assert info["exec_user"] == "myuser"
    assert info["syriana_bin"] == "/opt/syriana/bin/syriana"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from syriana_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "syriana"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "syriana-agent" in cmd
    assert "/data/current-package/bin/syriana" in cmd
    assert "chat" in cmd
