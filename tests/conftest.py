"""Shared fixtures for the syriana-agent test suite.
...
2. **Isolated SYRIANA_HOME.** SYRIANA_HOME points to a per-test tempdir so
   code reading ``~/.syriana/*`` via ``get_syriana_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".syriana"`` instead of the canonical
   ``get_syriana_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SYRIANA_SESSION_* inheritance** — the agent's current gateway
...
# SYRIANA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SYRIANA_BEHAVIORAL_VARS = frozenset({
    "SYRIANA_YOLO_MODE",
    "SYRIANA_INTERACTIVE",
    "SYRIANA_QUIET",
    "SYRIANA_TOOL_PROGRESS",
    "SYRIANA_TOOL_PROGRESS_MODE",
    "SYRIANA_MAX_ITERATIONS",
    "SYRIANA_SESSION_PLATFORM",
    "SYRIANA_SESSION_CHAT_ID",
    "SYRIANA_SESSION_CHAT_NAME",
    "SYRIANA_SESSION_THREAD_ID",
    "SYRIANA_SESSION_SOURCE",
    "SYRIANA_SESSION_KEY",
    "SYRIANA_GATEWAY_SESSION",
    "SYRIANA_CRON_SESSION",
    "_SYRIANA_GATEWAY",
    "SYRIANA_PLATFORM",
    "SYRIANA_MODEL",
    "SYRIANA_INFERENCE_MODEL",
    "SYRIANA_INFERENCE_PROVIDER",
    "SYRIANA_TUI_PROVIDER",
    "SYRIANA_MANAGED",
    "SYRIANA_MANAGED_DIR",
    "SYRIANA_DEV",
    "SYRIANA_CONTAINER",
    ...
    "SYRIANA_TIMEZONE",
    "SYRIANA_REDACT_SECRETS",
    "SYRIANA_BACKGROUND_NOTIFICATIONS",
    "SYRIANA_EXEC_ASK",
    "SYRIANA_HOME_MODE",
    "SYRIANA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.syriana/kanban.db instead of the per-test SYRIANA_HOME.
    "SYRIANA_KANBAN_DB",
    "SYRIANA_KANBAN_BOARD",
    "SYRIANA_KANBAN_HOME",
    "SYRIANA_KANBAN_WORKSPACES_ROOT",
    "SYRIANA_KANBAN_LOGS_ROOT",
    "SYRIANA_KANBAN_TASK",
    "SYRIANA_KANBAN_WORKSPACE",
    "SYRIANA_KANBAN_RUN_ID",
    "SYRIANA_KANBAN_CLAIM_LOCK",
    "SYRIANA_KANBAN_DISPATCH_IN_GATEWAY",
    "SYRIANA_TENANT",
    ...
    "SYRIANA_DASHBOARD_OAUTH_CLIENT_ID",
    "SYRIANA_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SYRIANA_HOME to per-test tempdirs so code that
    reads ``~/.syriana/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral SYRIANA_* vars that could change test semantics.
    for name in _SYRIANA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect SYRIANA_HOME to a per-test tempdir. Code that reads
    #    ``~/.syriana/*`` via ``get_syriana_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.syriana/*`` via
    #    ``Path.home() / ".syriana"`` instead of ``get_syriana_home()``
    #    is a bug to fix at the callsite.
    fake_syriana_home = tmp_path / "syriana_test"
    fake_syriana_home.mkdir()
    (fake_syriana_home / "sessions").mkdir()
    (fake_syriana_home / "cron").mkdir()
    (fake_syriana_home / "memories").mkdir()
    (fake_syriana_home / "skills").mkdir()
    monkeypatch.setenv("SYRIANA_HOME", str(fake_syriana_home))
    ...
    #    ~/.syriana/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import syriana_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_syriana_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal syriana config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``syriana-gateway`` process
...
# correlated with a ``tests/syriana_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> syriana-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart syriana-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting syriana/python patterns are also blocked.
    """
    ...
    def _matches_syriana_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SYRIANA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_syriana_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch syriana-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m syriana_cli.main".
                if (
                    "syriana" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live syriana-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting syriana/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `syriana update` (or the
        # equivalent `python -m syriana_cli.main update`).  These commands
        ...
        if "update" in low and (
            # syriana update / syriana update --gateway / setsid bash -c ... syriana update
            ("syriana" in low and "update" in low.split())
            or
            # python -m syriana_cli.main update --gateway
            ("syriana_cli" in low and "update" in low.split())
            or
            # venv/bin/syriana update  (absolute path variant used in tests)
            (".venv/bin/syriana" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`syriana update` against the real checkout, fetching "
