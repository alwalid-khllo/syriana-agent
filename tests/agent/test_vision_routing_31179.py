...
def isolated_home(monkeypatch):
    """Temp SYRIANA_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="syriana_test_31179_")
    syriana_home = os.path.join(test_home, ".syriana")
    os.makedirs(syriana_home)
...
def _fresh_modules():
    """Drop cached syriana modules so each test reloads against current env."""
