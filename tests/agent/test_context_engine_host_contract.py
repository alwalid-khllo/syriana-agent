...
engine plugins (e.g. syriana-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``syriana plugins`` picker."""
    from syriana_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "syriana_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "syriana_cli.plugins.get_plugin_context_engine",
