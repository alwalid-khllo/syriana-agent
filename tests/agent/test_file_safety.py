...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_syriana_env(self):
        """Syriana' own .env inside SYRIANA_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: syriana internal .env is in ~/.syriana/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.syriana/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.syriana/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Syriana cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        syriana_home = tmp_path / ".syriana"
        cache = syriana_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._syriana_home_path", return_value=syriana_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Syriana cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        syriana_home = tmp_path / ".syriana"
        hub = syriana_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_syriana_home(self, tmp_path):
        """The env basename guard does not depend on SYRIANA_HOME resolution."""
        syriana_home = tmp_path / ".syriana"
        syriana_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        syriana_home = tmp_path / ".syriana"
        cache = syriana_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
