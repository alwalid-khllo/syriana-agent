...
def test_syriana_client_tag_includes_current_version():
    """The client tag must reflect syriana_cli.__version__ verbatim."""
    from syriana_cli import __version__
    from agent.portal_tags import syriana_client_tag

    assert syriana_client_tag() == f"client=syriana-client-v{__version__}"


def test_syriana_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import syriana_client_tag

    tag = syriana_client_tag()
    assert tag.startswith("client=syriana-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import syriana_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=syriana-agent" in tags
    assert syriana_client_tag() in tags
    assert len(tags) == 2
