...
""",
        ...
        from syriana_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        syriana_home = tmp_path / ".syriana"
        syriana_home.mkdir()
        (syriana_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("SYRIANA_HOME", str(syriana_home))
        from syriana_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from syriana_cli.plugins import PluginContext, PluginManifest, PluginManager
