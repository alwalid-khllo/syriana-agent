...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated SYRIANA_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="syriana_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".syriana"))
    prev_home = os.environ.get("SYRIANA_HOME")
    os.environ["SYRIANA_HOME"] = os.path.join(test_home, ".syriana")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("syriana_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["SYRIANA_HOME"] = prev_home
