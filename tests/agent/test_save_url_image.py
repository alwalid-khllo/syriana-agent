...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SYRIANA_HOME under tmp_path."""
    monkeypatch.setenv("SYRIANA_HOME", str(tmp_path / ".syriana"))
    (tmp_path / ".syriana").mkdir()

    # Force the constants/image cache helpers to re-read SYRIANA_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("syriana_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_syriana_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under SYRIANA_HOME — gateway cleanup
