...
path via ``syriana chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp SYRIANA_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point SYRIANA_HOME at a temp dir and turn moa.save_traces on."""
    syriana_home = tmp_path / ".syriana"
    syriana_home.mkdir()
    monkeypatch.setenv("SYRIANA_HOME", str(syriana_home))

    # save_moa_turn reads config via syriana_cli.config.load_config; stub it to
    ...
    return syriana_home / "moa-traces"
