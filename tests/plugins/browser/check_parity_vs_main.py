...
Run from the PR worktree:
    ...
    cd ~/.syriana/syriana-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.syriana/syriana-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.syriana/syriana-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a syriana-agent checkout"
)
...
# Isolated SYRIANA_HOME for the config write.
