...
class TestToolsetConsistency:
    ...
    def test_syriana_platforms_share_core_tools(self):
        """All syriana-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on syriana-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["syriana-cli", "syriana-telegram", "syriana-discord", "syriana-whatsapp", "syriana-slack", "syriana-signal", "syriana-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_syriana_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("syriana-whatsapp")

    def test_syriana_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("syriana-api-server")
