...
These exercise the real store against a temp SYRIANA_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated SYRIANA_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("SYRIANA_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_syriana_home() reads the env live.
    yield tmp_path
