...
the shared /blueprint command handler (syriana_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated SYRIANA_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("syriana://blueprint/morning-brief?")
    ...
    def test_catalog_entry_has_all_surfaces(self):
        ...
        assert entry["appUrl"].startswith("syriana://")
...
class TestCommandHandler:
    ...
    def test_name_match_is_forgiving(self, isolated_home):
        from syriana_cli.blueprint_cmd import handle_blueprint_command, match_blueprint
