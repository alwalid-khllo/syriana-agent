"""Tests for the secret-source tracking in ``syriana_cli.env_loader``.

These cover the small public surface that lets `syriana model` / `syriana setup`
...
from syriana_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_syriana_dotenv()`` is called at module-import time from several
    hot modules (cli.py, syriana_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per SYRIANA_HOME per process.
    """
    ...
    # cli.py, syriana_cli/main.py, run_agent.py, trajectory_compressor.py,
