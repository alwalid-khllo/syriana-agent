...
def server():
    with patch.dict(
        "sys.modules",
        {
            "syriana_constants": MagicMock(
                get_syriana_home=MagicMock(return_value="/tmp/syriana_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'syriana-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'syriana-release' patched"
    }
