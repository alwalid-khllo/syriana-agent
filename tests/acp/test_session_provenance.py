...
def test_root_session_no_compression(db):
    ...
    assert prov["currentSyrianaSessionId"] == "root1"
    assert prov["rootSyrianaSessionId"] == "root1"
    assert prov["parentSyrianaSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_syriana_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentSyrianaSessionId"] == "old"
    assert prov["rootSyrianaSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousSyrianaSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootSyrianaSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootSyrianaSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_syriana_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousSyrianaSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"syriana"}
    assert "sessionProvenance" in meta["syriana"]
    assert meta["syriana"]["sessionProvenance"]["currentSyrianaSessionId"] == "root1"
