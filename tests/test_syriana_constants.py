"""Tests for syriana_constants module."""
...
from syriana_constants import (
    ...
    find_syriana_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_syriana_root,
    get_syriana_dir,
    get_syriana_home,
    heal_syriana_managed_node,
    syriana_managed_node_tree_present,
    iter_syriana_node_dirs,
    is_container,
    node_tool_runnable,
    parse_reasoning_effort,
    secure_parent_dir,
    with_syriana_node_path,
)


class TestGetDefaultSyrianaRoot:
    """Tests for get_default_syriana_root() — Docker/custom deployment awareness."""

    def test_no_syriana_home_returns_native(self, tmp_path, monkeypatch):
        """When SYRIANA_HOME is not set, returns ~/.syriana."""
        monkeypatch.delenv("SYRIANA_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_syriana_root() == tmp_path / ".syriana"

    def test_syriana_home_is_native(self, tmp_path, monkeypatch):
        """When SYRIANA_HOME = ~/.syriana, returns ~/.syriana."""
        native = tmp_path / ".syriana"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SYRIANA_HOME", str(native))
        assert get_default_syriana_root() == native

    def test_syriana_home_is_profile(self, tmp_path, monkeypatch):
        """When SYRIANA_HOME is a profile under ~/.syriana, returns ~/.syriana."""
        native = tmp_path / ".syriana"
    ...
    def test_syriana_home_is_docker(self, tmp_path, monkeypatch):
        """When SYRIANA_HOME points outside ~/.syriana (Docker), returns SYRIANA_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SYRIANA_HOME", str(docker_home))
        assert get_default_syriana_root() == docker_home

    def test_syriana_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any SYRIANA_HOME outside ~/.syriana is treated as the root."""
        custom = tmp_path / "my-syriana-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SYRIANA_HOME", str(custom))
        assert get_default_syriana_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (SYRIANA_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SYRIANA_HOME", str(profile))
        assert get_default_syriana_root() == docker_root

    def test_no_syriana_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\syriana, not ~/.syriana."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("SYRIANA_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(syriana_constants.sys, "platform", "win32")

        assert get_default_syriana_root() == local_appdata / "syriana"

    def test_no_syriana_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/syriana without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("SYRIANA_HOME", raising=False)
        ...
        assert get_default_syriana_root() == home / "AppData" / "Local" / "syriana"


class TestGetSyrianaHome:
    """Tests for get_syriana_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When SYRIANA_HOME is unset on Windows, use %LOCALAPPDATA%\\syriana."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("SYRIANA_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(syriana_constants.sys, "platform", "win32")
        monkeypatch.setattr(syriana_constants, "_profile_fallback_warned", False)

        assert get_syriana_home() == local_appdata / "syriana"


class TestSyrianaManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_syriana_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(syriana_constants.sys, "platform", "win32")
        monkeypatch.setenv("SYRIANA_HOME", str(home))
        monkeypatch.setattr(syriana_constants, "node_tool_runnable", lambda path: True)

        assert find_syriana_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SYRIANA_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(syriana_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(syriana_constants, "heal_syriana_managed_node", lambda: False)
        monkeypatch.setattr(
            syriana_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert syriana_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_syriana_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "syriana"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_syriana_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Syriana-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(syriana_constants, "heal_syriana_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_syriana_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(syriana_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(syriana_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".syriana"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import syriana_cli._subprocess_compat as subprocess_compat
...
class TestGetSyrianaDir:
    """Tests for ``get_syriana_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_syriana_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_syriana_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_syriana_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_syriana_dir("cache/audio", "audio_cache")
