...
const { canImportSyrianaCli, verifySyrianaCli } = require('./backend-probes.cjs')
const {
...
const { buildDesktopBackendEnv, normalizeSyrianaHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.SYRIANA_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.SYRIANA_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with SYRIANA_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[syriana] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[syriana] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('syriana:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[syriana] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[syriana] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[syriana] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// SYRIANA_HOME — the user-facing root for everything Syriana-related. Mirrors
// scripts/install.ps1's $SyrianaHome and scripts/install.sh's $SYRIANA_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\syriana (matches install.ps1)
//   macOS / Linux: ~/.syriana (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.syriana directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\syriana yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// SYRIANA_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// SYRIANA_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.syriana / %LOCALAPPDATA%\syriana.
function resolveSyrianaHome() {
  if (process.env.SYRIANA_HOME) return normalizeSyrianaHomeRoot(process.env.SYRIANA_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'syriana-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a SYRIANA_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\syriana and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('SYRIANA_HOME')
    if (fromRegistry) return normalizeSyrianaHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'syriana')
    const legacy = path.join(app.getPath('home'), '.syriana')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.syriana setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.syriana')
}

const SYRIANA_HOME = resolveSyrianaHome()

function syrianaManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_syriana_node_dirs() in
  // syriana_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(SYRIANA_HOME, 'node')
  const bin = path.join(root, 'bin')
  const entries = IS_WINDOWS ? [root, bin] : [bin, root]
  return entries.filter(directoryExists)
}

function pathWithSyrianaManagedNode(...entries) {
  return [...syrianaManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_SYRIANA_ROOT — the canonical mutable Syriana install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_SYRIANA_ROOT = path.join(SYRIANA_HOME, 'syriana-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_SYRIANA_ROOT, 'venv')
...
// boots, going straight to `resolveSyrianaBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_SYRIANA_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_SYRIANA_ROOT, '.syriana-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Syriana profile the desktop launches its
// local backend as. When set, startSyriana() passes `syriana --profile <name>
// dashboard …`, which deterministically pins SYRIANA_HOME (see
// _apply_profile_override in syriana_cli/main.py) and bypasses the sticky
// ~/.syriana/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors syriana_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// syrianaDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under SYRIANA_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by syriana_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(SYRIANA_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (syriana_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.SYRIANA_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.SYRIANA_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
  if (!Number.isFinite(raw) || raw <= 0) return 650
  return Math.max(120, raw)
})()
const APP_NAME = 'Syriana'
...
// 'syriana:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.syriana) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.syriana')
}
// Seed the native About panel with the live Syriana version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `syriana update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveSyrianaVersion(),
  copyright: 'Copyright © 2026 syriana ai Research'
})
...
const MEDIA_PROTOCOL = 'syriana-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(syriana-media://…)`.
...
let mainWindow = null
let syrianaProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by syrianaProcess +
// connectionPromise + startSyriana(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.SYRIANA_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.SYRIANA_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startSyriana() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning syriana serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Syriana backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[syriana] ${line}`)
  syrianaLog.push(...lines)
  if (syrianaLog.length > 300) {
    syrianaLog.splice(0, syrianaLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-syriana-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('syriana:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the syriana:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('syriana:bootstrap:event', ev)
}
...
// The Tauri updater writes SYRIANA_HOME/.syriana-update-in-progress for the whole
...
// (`force_kill_other_syriana`, taskkill /IM syriana.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(SYRIANA_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Syriana will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(SYRIANA_HOME)
  }
  if (marker) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `syriana` — must not shadow `syriana.cmd`/`syriana.exe`.
...
function unwrapWindowsVenvSyrianaCommand(command, backendArgs) {
  if (!IS_WINDOWS || !command || isCommandScript(command)) return null

  const resolved = path.resolve(String(command))
  if (!/^syriana(?:\.exe)?$/i.test(path.basename(resolved))) return null
  ...
  return {
    label: `existing Syriana Python at ${python}`,
    command: python,
    args: ['-m', 'syriana_cli.main', ...backendArgs],
    bootstrap: false,
    env: buildDesktopBackendEnv({
      syrianaHome: SYRIANA_HOME,
...
// spawns `syriana serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `syriana` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'syriana_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, SYRIANA_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isSyrianaSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'syriana_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.SYRIANA_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Syriana deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Syriana' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\syriana\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'syriana', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'syriana', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal SYRIANA_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\syriana\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'syriana', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'syriana', 'git', 'bin', 'git.exe'))
  }
...
function recentSyrianaLog() {
  return syrianaLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's syriana root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_SYRIANA_ROOT.
// SYRIANA_DESKTOP_SYRIANA_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.SYRIANA_DESKTOP_SYRIANA_ROOT && path.resolve(process.env.SYRIANA_DESKTOP_SYRIANA_ROOT),
    !IS_PACKAGED && isSyrianaSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isSyrianaSourceRoot(ACTIVE_SYRIANA_ROOT) ? ACTIVE_SYRIANA_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_SYRIANA_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('syriana:updates:progress', merged)
  }
}
...
// SYRIANA_HOME/syriana-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_syriana_home). That binary owns
// ALL repo mutation — running `syriana update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'syriana-setup.exe' : 'syriana-setup'
  const candidate = path.join(SYRIANA_HOME, name)
...
// Path to the venv shim whose lock decides whether `syriana update` can write
// fresh entry points. On Windows this is the file the running backend
// `syriana.exe` holds open; on POSIX it's never mandatory-locked.
function venvSyrianaShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'syriana.exe')
    : path.join(updateRoot, 'venv', 'bin', 'syriana')
}
...
// only signals the direct child, so on Windows a backend `syriana.exe` that
// spawned its own grandchildren (a `syriana` REPL, a pty terminal session, the
...
// `syrianaProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `syriana.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a syriana
// REPL, a pty terminal, the gateway) keep `syriana.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (syrianaProcess && Number.isInteger(syrianaProcess.pid)) pids.push(syrianaProcess.pid)
  ...
  if (syrianaProcess && !syrianaProcess.killed) {
    try {
      syrianaProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvSyrianaShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const stragglers = []
    if (syrianaProcess && Number.isInteger(syrianaProcess.pid)) stragglers.push(syrianaProcess.pid)
    for (const entry of backendPool.values()) {
...
// `syriana update`). Instead we spawn the staged Syriana-Setup binary with
// --update and quit, so it can run `syriana update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri syriana-setup. Unlike Windows
      ...
      // whole update itself: `syriana update` (backend) + `syriana desktop
    ...
    if (!updater) {
      ...
      // `syriana desktop`, never the Tauri installer that self-copies
      // syriana-setup.exe into SYRIANA_HOME). They DO have a working `syriana`
      ...
      // checkout they're on — bare `syriana update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'syriana update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `syriana update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `syriana update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, syrianaRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Syriana — this window will close and the updater will open. Don’t reopen Syriana yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // syriana.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running syriana, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Syriana install open ' +
        '(a second Syriana window or a terminal running syriana?). Close it and retry.'
    ...
    // `syriana update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: SYRIANA_HOME,
      env: {
        ...process.env,
        SYRIANA_HOME,
        PATH: pathWithSyrianaManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvSyriana = path.join(venvBin, IS_WINDOWS ? 'syriana.exe' : 'syriana')
  ...
  // not just the `syriana.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvSyriana) || fileExists(path.join(updateRoot, '.syriana-bootstrap-complete'))
...
// Resolve the syriana CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `syriana` on PATH.
function resolveSyrianaCliBinary(updateRoot) {
  const venvSyriana = path.join(updateRoot, 'venv', 'bin', 'syriana')
  if (fileExists(venvSyriana)) return venvSyriana
  return findOnPath('syriana') || null
}
...
// macOS/Linux in-app update: backend (`syriana update`) + OS-aware GUI rebuild
// (`syriana desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const syriana = resolveSyrianaCliBinary(updateRoot)
  if (!syriana) {
    emitUpdateProgress({ stage: 'manual', message: 'syriana update', percent: null })
    return { ok: true, manual: true, command: 'syriana update', syrianaRoot: updateRoot }
  }

  // Put the Syriana-managed Node and the venv on PATH so `syriana desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\syriana\node, not node\bin.
  const env = {
    SYRIANA_HOME,
    PATH: pathWithSyrianaManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `syriana update` reaps stale `syriana serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads SYRIANA_DESKTOP_CHILD_PID and excludes
  ...
  if (syrianaProcess && Number.isInteger(syrianaProcess.pid)) {
    desktopChildPids.push(syrianaProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.SYRIANA_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Syriana (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(syriana, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'syriana update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'syriana update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(syriana, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Syriana to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `syriana desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Syriana…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `syriana-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Syriana to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Syriana desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Syriana to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Syriana.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Syriana.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Syriana to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.syriana-update-new"; then
    rm -rf "$DST.syriana-update-old" 2>/dev/null || true
    mv "$DST" "$DST.syriana-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.syriana-update-new" "$DST"
    rm -rf "$DST.syriana-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Syriana to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_SYRIANA_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isSyrianaSourceRoot(ACTIVE_SYRIANA_ROOT) &&
    fileExists(venvPython) &&
    canImportSyrianaCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_SYRIANA_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `syriana
...
function resolveWebDist() {
  const override = process.env.SYRIANA_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set SYRIANA_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: syriana desktop --force-build`
  )
  return candidates[0]
}
...
function resolveSyrianaCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Syriana.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.SYRIANA_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveSyrianaCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_SYRIANA_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Syriana at ${ACTIVE_SYRIANA_ROOT}`,
    command,
    args: ['-m', 'syriana_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      syrianaHome: SYRIANA_HOME,
      pythonPathEntries: [ACTIVE_SYRIANA_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_SYRIANA_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveSyrianaBackend(backendArgs) {
  // 1. Explicit override -- SYRIANA_DESKTOP_SYRIANA_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.SYRIANA_DESKTOP_SYRIANA_ROOT && path.resolve(process.env.SYRIANA_DESKTOP_SYRIANA_ROOT)
  if (overrideRoot && isSyrianaSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Syriana source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `syriana` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isSyrianaSourceRoot.)
  if (!IS_PACKAGED && isSyrianaSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Syriana source at ${SOURCE_REPO_ROOT}`, backendArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_SYRIANA_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\syriana\syriana-agent (Windows) or ~/.syriana/syriana-agent.
  ...
  //    to spawning syriana. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `syriana update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `syriana` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    SYRIANA_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.SYRIANA_DESKTOP_IGNORE_EXISTING !== '1') {
    let syrianaCommand = null
    const syrianaOverride = process.env.SYRIANA_DESKTOP_SYRIANA

    if (syrianaOverride) {
      const resolvedOverride = findOnPath(syrianaOverride)
      if (resolvedOverride) {
        syrianaCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(syrianaOverride, { isWsl: IS_WSL })) {
        syrianaCommand = syrianaOverride
      } else {
        rememberLog(`Ignoring Windows Syriana override under WSL: ${syrianaOverride}`)
      }
    } else {
      syrianaCommand = findOnPath('syriana')
    }

    if (syrianaCommand) {
      if (looksLikeDesktopAppBinary(syrianaCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Syriana CLI: ${syrianaCommand}`)
        syrianaCommand = null
      }
    }

    if (syrianaCommand) {
      const unwrapped = unwrapWindowsVenvSyrianaCommand(syrianaCommand, backendArgs)
      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `syriana` shim
      ...
      const shellForProbe = isCommandScript(syrianaCommand)
      if (verifySyrianaCli(syrianaCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvSyrianaCommand(syrianaCommand, backendArgs) || {
            label: `existing Syriana CLI at ${syrianaCommand}`,
            command: syrianaCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Syriana CLI at ${syrianaCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed syriana_cli module via system Python.
  ...
  if (python) {
    ...
    // having syriana_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\syriana\syriana-agent\venv.
    if (canImportSyrianaCli(python)) {
      return {
        kind: 'python',
        label: `installed syriana_cli module via ${python}`,
        command: python,
        args: ['-m', 'syriana_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: syriana_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveSyrianaBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Syriana Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_SYRIANA_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveSyrianaBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Syriana install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error(
        'Syriana recovery was handed off to Syriana Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      syrianaHome: SYRIANA_HOME,
      logRoot: path.join(SYRIANA_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Syriana install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Syriana bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(SYRIANA_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startSyriana() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // syriana:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveSyrianaBackend(backend.args))
  }
  ...
  if (!isSyrianaSourceRoot(ACTIVE_SYRIANA_ROOT)) {
    throw new Error(
      `Syriana install at ${ACTIVE_SYRIANA_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Syriana' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\syriana\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `syriana` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Syriana on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Syriana.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isSyrianaSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Syriana venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Syriana at ${ACTIVE_SYRIANA_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Syriana runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Syriana backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Syriana backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Syriana backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Syriana-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('syriana:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveSyrianaCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('syriana:preview-file-changed', payload)
}
...
async function waitForSyriana(baseUrl, token) {
  ...
  throw new Error(`Syriana backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('syriana:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('syriana:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Syriana...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('syriana:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('syriana:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('syriana:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'syriana:desktop:zoomLevel'
...
//
// Hosted Syriana gateways gate the dashboard behind an OAuth provider (e.g.
// syriana ai Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``syriana_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / syriana #37247).
//     Both are set as HttpOnly cookies (``syriana_session_at`` ~15 min,
//     ``syriana_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:syriana-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Syriana gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Syriana backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root SYRIANA_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.SYRIANA_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.SYRIANA_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by SYRIANA_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (syriana #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Syriana gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Syriana gateway is selected, but no session token is saved. ' +
...
//   2. env override (SYRIANA_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.SYRIANA_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.SYRIANA_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'SYRIANA_DESKTOP_REMOTE_URL is set but SYRIANA_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Syriana backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.SYRIANA_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Syriana
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with syriana ai Research" instead of a generic
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startSyriana())
  ...
  // connect to Syriana gateway". Mirror the renderer's connect here so the test
...
function resetSyrianaConnection() {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(syrianaProcess)
...
// startSyriana() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetSyrianaConnection() nulls syrianaProcess.
  const dying = syrianaProcess && !syrianaProcess.killed ? syrianaProcess : null
  resetSyrianaConnection()
...
// profile to startSyriana() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startSyriana() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForSyriana(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: syrianaLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited SYRIANA_HOME env (see _apply_profile_override
  // step 3 in syriana_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveSyrianaBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const syrianaCwd = resolveSyrianaCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Syriana backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: syrianaCwd,
        SYRIANA_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        SYRIANA_DESKTOP: '1',
        SYRIANA_WEB_DIST: webDist,
        ...(readyFile ? { SYRIANA_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Syriana backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Syriana backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Syriana backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForSyriana(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Syriana backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_syriana_home() recreates the deleted profile directory.
...
async function startSyriana() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startSyriana() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Syriana backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Syriana backend at ${remote.baseUrl}`, 24)
      await waitForSyriana(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Syriana backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.syriana/active_profile file) and
    // resolves SYRIANA_HOME the same way `syriana -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Syriana runtime', 28)
    const backend = await ensureRuntime(resolveSyrianaBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const syrianaCwd = resolveSyrianaCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Syriana backend via ${backend.label}`, 84)
    rememberLog(`Starting Syriana backend via ${backend.label}`)

    syrianaProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: syrianaCwd,
        env: {
          ...process.env,
          // Explicitly pin SYRIANA_HOME for the child so Python's get_syriana_home()
          // resolves to the SAME location our resolveSyrianaHome() picked. Without
          // this pin, Python falls back to ~/.syriana on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\syriana, which differs from C:\Users\<u>\.syriana.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets SYRIANA_HOME via setx; the desktop
    ...
    syrianaProcess.stdout.on('data', rememberLog)
    syrianaProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    syrianaProcess.once('error', error => {
      rememberLog(`Syriana backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Syriana backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    syrianaProcess.once('exit', (code, signal) => {
      rememberLog(`Syriana backend exited (${signal || code})`)
      syrianaProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Syriana backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Syriana backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentSyrianaLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Syriana backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(syrianaProcess, { readyFile }),
      backendStartFailed
    ])
    if (readyFile) {
      fs.unlink(readyFile, () => {})
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Syriana backend to become ready', 90)
    await Promise.race([waitForSyriana(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null syrianaProcess when the child dies.
      childAlive: () => syrianaProcess !== null && syrianaProcess.exitCode === null && !syrianaProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Syriana backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Syriana is
...
// pushes pet state over IPC (syriana:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via syriana:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Syriana app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Syriana icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see syriana:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Syriana in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('syriana:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startSyriana().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('syriana:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Syriana…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('syriana:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetSyrianaConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Syriana backend failed liveness probe; dropping stale connection.')
    resetSyrianaConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('syriana:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('syriana:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('syriana:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('syriana:window:openNewSession', async () => {
...
ipcMain.handle('syriana:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('syriana:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('syriana:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('syriana:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('syriana:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('syriana:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('syriana:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('syriana:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('syriana:pet-overlay:control', payload)
})
ipcMain.handle('syriana:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startSyriana() call restarts the
...
ipcMain.handle('syriana:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startSyriana() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('syriana:bootstrap:cancel', async () => {
...
ipcMain.handle('syriana:boot-progress:get', async () => bootProgressState)
ipcMain.handle('syriana:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('syriana:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('syriana:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('syriana:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('syriana:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('syriana:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('syriana:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('syriana:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('syriana:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('syriana:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new SYRIANA_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('syriana:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('syriana:requestMicrophoneAccess', async () => {
...
ipcMain.handle('syriana:api', async (_event, request) => {
  ...
  // backend calls ensure_syriana_home() which recreates the profile directory,
...
ipcMain.handle('syriana:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('syriana:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('syriana:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('syriana:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('syriana:readFileText', async (_event, filePath) => {
...
ipcMain.handle('syriana:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('syriana:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('syriana:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('syriana:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('syriana:saveClipboardImage', async () => {
...
ipcMain.handle('syriana:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('syriana:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('syriana:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('syriana:titlebar-theme', (_event, payload) => {
...
ipcMain.on('syriana:native-theme', (_event, mode) => {
...
ipcMain.on('syriana:translucency', (_event, payload) => {
...
ipcMain.handle('syriana:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('syriana:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveSyrianaCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('syriana:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveSyrianaCwd()
}))

ipcMain.handle('syriana:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('syriana:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('syriana:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('syriana:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('syriana:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('syriana:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: syrianaLog.slice(-200) }))
...
function terminalShellCommand() {
  // SYRIANA_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.SYRIANA_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Syriana' TUI into
  ...
  env.TERM_PROGRAM = 'Syriana'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a syriana/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from SYRIANA_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.SYRIANA_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `syriana:terminal:${id}:${suffix}`
}
...
ipcMain.handle('syriana:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('syriana:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('syriana:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('syriana:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('syriana:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('syriana:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('syriana:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('syriana:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('syriana:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('syriana:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('syriana:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('syriana:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('syriana:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('syriana:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('syriana:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('syriana:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('syriana:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('syriana:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('syriana:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('syriana:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('syriana:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('syriana:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('syriana:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('syriana:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('syriana:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('syriana:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Syriana.')
  }
...
ipcMain.handle('syriana:terminal:write', (_event, id, data) => {
...
ipcMain.handle('syriana:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('syriana:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('syriana:updates:check', async () =>
...
ipcMain.handle('syriana:updates:apply', async (_event, payload) =>
...
ipcMain.handle('syriana:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('syriana:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Syriana version (the one `release.py` bumps in
// syriana_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Syriana version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveSyrianaVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'syriana_cli', '__init__.py')
...
// Re-resolve the live Syriana version and push it into the native About panel
// just before showing it, so an in-place `syriana update` is reflected without
...
ipcMain.handle('syriana:version', async () => ({
  appVersion: resolveSyrianaVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  syrianaRoot: resolveUpdateRoot()
}))
...
// the actual removal via `syriana uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (syriana_cli/uninstall.py + syriana_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_SYRIANA_ROOT
  ...
  const fallback = () => ({
    syriana_home: SYRIANA_HOME,
    agent_installed: isSyrianaSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'syriana_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, SYRIANA_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Syriana agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import syriana_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_SYRIANA_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_SYRIANA_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `syriana-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `syriana-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('syriana:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('syriana:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('syriana:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('syriana:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// syriana:// deep links (e.g. syriana://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const SYRIANA_PROTOCOL = 'syriana'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${SYRIANA_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // syriana://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('syriana:deep-link', payload)
...
ipcMain.handle('syriana:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${SYRIANA_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SYRIANA_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(SYRIANA_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `syriana://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching syriana:// URL is in our own argv.
