 ...
 * the three user-facing uninstall modes to the `syriana uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m syriana_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `syriana_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'syriana_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Syriana.app/Contents/MacOS/Syriana  → …/Syriana.app
 *   Windows: …\Syriana\Syriana.exe                 → …\Syriana  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/Syriana.app/Contents/MacOS/Syriana → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Syriana.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs Syriana.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]Syriana$/i.test(dir) || /[\\/]syriana-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/syriana
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import syriana_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, syrianaHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export SYRIANA_HOME=${q(syrianaHome)}`
  ]
  if (pythonPath) {
 ...
 * modes + `pythonPath`=agentRoot so `import syriana_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Syriana install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "SYRIANA_HOME=${String(syrianaHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
  if (pythonPath) {
