...
// Regression guards for Windows `syriana` resolution in main.cjs.
//
...
//      Git-Bash `syriana` shim shadowed the real syriana.cmd/syriana.exe; the
...
//      --repair by checking ONLY venv\Scripts\syriana.exe (the console-script
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow syriana.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\syriana.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvSyriana\) \?/,
    'recovery regressed to gating only on the syriana.exe shim, which forces destructive --repair'
  )
})
