...
function markerPath(syrianaHome) {
  return path.join(syrianaHome, '.syriana-update-in-progress')
}
 ...
 * clock for tests.
 */
function readLiveUpdateMarker(syrianaHome, { kill, now = Date.now, maxAgeMs = UPDATE_MARKER_MAX_AGE_MS } = {}) {
  const file = markerPath(syrianaHome)
  let raw
  try {
