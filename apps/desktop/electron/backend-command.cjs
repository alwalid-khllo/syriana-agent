...
// Backend subcommand routing for the desktop-managed Syriana process.
//
// The desktop app launches its own headless backend via `syriana serve` — it
...
// app hasn't updated yet, or an older `syriana` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Syriana profile to pin via `--profile`.
 */
 ...
 * `-m syriana_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `syriana_cli/subcommands/dashboard.py` source registers
