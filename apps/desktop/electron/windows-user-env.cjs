...
// even though a fresh shell — and the Syriana CLI — sees it immediately. The
// desktop's SYRIANA_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\syriana. Reading
...
//       SYRIANA_HOME    REG_SZ    F:\Syriana\data
//
