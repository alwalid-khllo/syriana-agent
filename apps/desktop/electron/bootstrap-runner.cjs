 ...
 *
 * Drives apps/desktop's first-launch install of Syriana Agent by spawning
...
function bootstrapCacheDir(syrianaHome) {
  return path.join(syrianaHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.syriana/syriana-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(syrianaHome) {
  if (!syrianaHome) return null
  const candidate = path.join(syrianaHome, 'syriana-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(syrianaHome, commit) {
  return path.join(bootstrapCacheDir(syrianaHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/SyrianaAIResearch/syriana-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(syrianaHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(syrianaHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, syrianaHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass SYRIANA_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          SYRIANA_HOME: syrianaHome || process.env.SYRIANA_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, syrianaHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, syrianaHome }) {
  const args = ['--dir', activeRoot, '--syriana-home', syrianaHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, syrianaHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, syrianaHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, syrianaHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, syrianaHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(syrianaHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, syrianaHome, emit })
