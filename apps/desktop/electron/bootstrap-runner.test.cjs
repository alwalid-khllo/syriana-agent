...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'syriana-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/syriana-runner-test',
    sourceRepoRoot: null,
    syrianaHome: '/tmp/syriana-runner-test',
    logRoot: '/tmp/syriana-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'syriana-agent', 'scripts')
