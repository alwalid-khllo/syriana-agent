 ...
 * Background
 * ----------
 * After `syriana update` + `syriana desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything SYRIANA_DESKTOP_* is preserved
// plus SYRIANA_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['SYRIANA_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['SYRIANA_DESKTOP_']
