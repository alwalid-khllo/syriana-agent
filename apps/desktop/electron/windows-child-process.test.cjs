...
test('desktop background child processes opt into hidden Windows consoles', () => {
  ...
  requireHiddenChildOptions(source, /syrianaProcess = spawn\(\s*backend\.command,\s*backend\.args/)
  requireHiddenChildOptions(source, /spawn\(\s*py,\s*\['-m', 'syriana_cli\.main', 'uninstall', '--gui-summary'\]/)

  assert.match(source, /function unwrapWindowsVenvSyrianaCommand\(command, backendArgs\)/)
  assert.match(source, /function getVenvSitePackagesEntries\(venvRoot\)/)
  assert.match(source, /path\.join\(venvRoot, 'Lib', 'site-packages'\)/)
  assert.match(source, /args: \['-m', 'syriana_cli\.main', \.\.\.backendArgs\]/)
})
...
test('desktop backend launches console python so child consoles are inherited, not pythonw', () => {
  ...
  // SYRIANA_DASHBOARD_READY stdout line — no ready-file side channel is set.
...
test('desktop backend teardown tree-kills Windows backend descendants', () => {
  ...
  const resetIndex = source.indexOf('function resetSyrianaConnection()')
  assert.notEqual(resetIndex, -1, 'missing resetSyrianaConnection')
  const resetSnippet = source.slice(resetIndex, resetIndex + 300)
  assert.match(resetSnippet, /stopBackendChild\(syrianaProcess\)/)
  assert.doesNotMatch(resetSnippet, /syrianaProcess\.kill\('SIGTERM'\)/)
  ...
  assert.match(quitSnippet, /stopBackendChild\(syrianaProcess\)/)
  assert.doesNotMatch(quitSnippet, /syrianaProcess\.kill\('SIGTERM'\)/)
})
