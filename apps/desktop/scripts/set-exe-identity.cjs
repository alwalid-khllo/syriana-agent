#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Syriana icon + version metadata onto the
// built Syriana.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Syriana.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `syriana desktop`, the installer's
...
// path (which rebuilds via `syriana desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Syriana.exe>
//
...
// Stamp the Syriana icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Syriana',
      FileDescription: 'Syriana',
      CompanyName: 'syriana ai Research',
      LegalCopyright: 'Copyright (c) 2026 syriana ai Research'
    }
  })

  console.log('[set-exe-identity] done — Syriana icon + identity stamped')
}
