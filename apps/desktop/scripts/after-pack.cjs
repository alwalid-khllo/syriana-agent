 ...
 *
 * Stamps the Syriana icon + identity onto the packed Windows Syriana.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `syriana desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Syriana')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Syriana'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Syriana.exe keeps the stock Electron icon`)
  }
}
