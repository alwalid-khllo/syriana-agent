 ...
 * product name (`Syriana`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Syriana'
 *
 * This is a hard failure with no obvious cause for the user — `syriana desktop`
 ...
 * (the mac-unpacked Syriana.app bundle) and Windows (win-unpacked), so we
