...
declare global {
  interface Window {
    syrianaDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<SyrianaConnection>
      ...
      profile: {
        ...
        // backend under the new SYRIANA_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: SyrianaApiRequest) => Promise<T>
      notify: (payload: SyrianaNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<SyrianaReadFileTextResult>
      selectPaths: (options?: SyrianaSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<SyrianaPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<SyrianaPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: SyrianaTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<SyrianaReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<SyrianaGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<SyrianaGitBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<SyrianaRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: SyrianaReviewScope, baseRef?: null | string) => Promise<SyrianaReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: SyrianaReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<SyrianaReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: SyrianaTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<SyrianaTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: SyrianaWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: SyrianaPreviewFileChanged) => void) => () => void
...
export interface SyrianaTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface SyrianaTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `syriana update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  syrianaRoot?: string
...
export interface SyrianaConnection {
  baseUrl: string
...
export interface SyrianaTitleBarTheme {
  background: string
  foreground: string
}

export interface SyrianaWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface SyrianaApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface SyrianaNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface SyrianaPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface SyrianaReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface SyrianaPreviewWatch {
  id: string
  path: string
}
...
export interface SyrianaGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface SyrianaGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
export interface SyrianaRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface SyrianaRepoStatus {
  branch: null | string
...
// turn began.
export type SyrianaReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface SyrianaReviewFile {
  path: string
  added: number
  removed: number
...
export interface SyrianaReviewList {
  files: SyrianaReviewFile[]
...
export interface SyrianaReviewPr {
  url: string
  state: string
  number: number
}
...
export interface SyrianaReviewShipInfo {
  ghReady: boolean
  pr: SyrianaReviewPr | null
}

export interface SyrianaReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface SyrianaReadDirResult {
  entries: SyrianaReadDirEntry[]
  error?: string
}

export interface SyrianaPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface SyrianaSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
