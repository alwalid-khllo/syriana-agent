...
import { formatRefValue, syrianaDirectiveFormatter } from './directive-text'
...
describe('syrianaDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = syrianaDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = syrianaDirectiveFormatter.parse('@file:src/main.tsx the entry point')
