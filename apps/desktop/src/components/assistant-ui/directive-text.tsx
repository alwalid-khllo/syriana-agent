...
const SYRIANA_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type SyrianaRefType = (typeof SYRIANA_REF_TYPES)[number]
...
const ICON_PATHS: Record<SyrianaRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as SyrianaRefType] ?? ICON_FALLBACK
...
const SYRIANA_DIRECTIVE_RE = new RegExp(
...
export const syrianaDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(SYRIANA_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as SyrianaRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: SyrianaRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...syrianaDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Syriana directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.syrianaDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.syrianaDesktop?.readFileDataUrl(id)
