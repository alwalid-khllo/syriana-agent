...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.syrianaDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.syrianaDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.syrianaDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.syrianaDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.syrianaDesktop && text && normalizeExternalUrl(text) === target) {
