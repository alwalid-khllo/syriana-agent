...
import { getSyrianaConfigRecord, type SyrianaConfigRecord, saveSyrianaConfig } from '@/syriana'
...
export interface I18nConfigClient {
  getConfig: () => Promise<SyrianaConfigRecord>
  saveConfig: (config: SyrianaConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.syrianaDesktop?.api) {
      return Promise.resolve({})
    }

    return getSyrianaConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.syrianaDesktop?.api) {
    ...
    return saveSyrianaConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: SyrianaConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: SyrianaConfigRecord, locale: Locale): SyrianaConfigRecord {
