...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'syriana:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.syrianaDesktop?.saveImageFromUrl) {
        if (await window.syrianaDesktop.saveImageFromUrl(src)) {
