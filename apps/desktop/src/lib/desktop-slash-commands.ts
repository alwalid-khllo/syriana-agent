...
/** Why a known Syriana command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Syriana profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Syriana Agent version', surface: exec() },
...
function isKnownSyrianaSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Syriana' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownSyrianaSlashCommand(normalized)
}
