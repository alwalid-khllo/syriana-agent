import type {
  SyrianaConnection,
  SyrianaReadDirResult,
  SyrianaReadFileTextResult,
  SyrianaSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: SyrianaConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<SyrianaReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<SyrianaReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<SyrianaReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<SyrianaReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: SyrianaSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
