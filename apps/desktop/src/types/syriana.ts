...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Syriana doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in syriana_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `syriana model` uses — instead of
...
export interface SyrianaConfig {
  agent?: {
...
export type SyrianaConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `syriana model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors syriana_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Syriana, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `syriana tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Syriana); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/syriana/update/check` — the backend's own update state.
