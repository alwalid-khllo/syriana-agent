...
const SKIN_KEY = 'syriana-desktop-theme-v2'
const MODE_KEY = 'syriana-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'syriana-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'syriana-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'syriana-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.syrianaTheme = skinName
  root.dataset.syrianaMode = rendered
  ...
  window.syrianaDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('syriana-boot-background', chromeBg)
    window.localStorage.setItem('syriana-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.syrianaThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.syrianaDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Syriana backend theme on connect. */
