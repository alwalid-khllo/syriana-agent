...
import { getSyrianaConfig } from '@/syriana'
import { persistString } from '@/lib/storage'
import { $currentCwd, setCurrentCwd } from '@/store/session'

import { useSyrianaConfig } from './use-syriana-config'

vi.mock('@/syriana', () => ({
  getSyrianaConfig: vi.fn(),
  getSyrianaConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'syriana.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getSyrianaConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getSyrianaConfig>>)

describe('useSyrianaConfig refreshSyrianaConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshSyrianaConfig()
    })
