...
import { getSyrianaConfig, getSyrianaConfigDefaults } from '@/syriana'
...
interface SyrianaConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useSyrianaConfig({ activeSessionIdRef, refreshProjectBranch }: SyrianaConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshSyrianaConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getSyrianaConfig(), getSyrianaConfigDefaults().catch(() => ({}))])
  ...
  return { refreshSyrianaConfig, sttEnabled, voiceMaxRecordingSeconds }
}
