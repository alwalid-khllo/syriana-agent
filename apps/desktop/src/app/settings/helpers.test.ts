...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: SyrianaConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: SyrianaConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // SYRIANA_QWEN_ shares the SYRIANA_ stem with other integrations.
      expect(providerGroup('SYRIANA_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
