...
import { invalidateSyrianaConfig, setSyrianaConfigCache, useSyrianaConfigRecord } from '../hooks/use-config-record'
...
// Syriana' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Syriana default (medium), shown as Medium.
...
// backend now surfaces the full `syriana model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in syriana_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useSyrianaConfigRecord()
  const setConfig = setSyrianaConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `syriana model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Syriana runs the flow for you.`}
          </p>
        )}
