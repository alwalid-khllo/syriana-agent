...
import { getElevenLabsVoices, getSyrianaConfigSchema, saveSyrianaConfig } from '@/syriana'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, SyrianaConfigRecord } from '@/types/syriana'

import { setSyrianaConfigCache, useSyrianaConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: SyrianaConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<SyrianaConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useSyrianaConfigRecord()
  ...
  const {
    ...
    queryKey: ['syriana-config-schema'],
    queryFn: getSyrianaConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: SyrianaConfigRecord) => {
