...
import { disconnectOAuthProvider, listOAuthProviders } from '@/syriana'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/syriana'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.syrianaDesktop?.terminal)
...
//      in syriana_cli/provider_catalog.py) — the SAME provider identity
//      `syriana model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Syriana can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Syriana can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Syriana never deletes creds another tool owns behind a silent API call.
