...
export const EMPTY_SELECT_VALUE = '__syriana_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Syriana & Nous-trained models',
    docsUrl: 'https://portal.syriana.ai',
    priority: 0
  },
  {
    prefix: 'OPENROUTER_',
  ...
  { prefix: 'SYRIANA_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // syriana_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Syriana needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Syriana stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Syriana can read from one file request.',
  approvals: {
    mode: 'How Syriana handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Syriana updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
