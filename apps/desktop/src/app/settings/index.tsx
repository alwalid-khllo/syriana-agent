...
import { getSyrianaConfigDefaults, getSyrianaConfigRecord, saveSyrianaConfig } from '@/syriana'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getSyrianaConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'syriana-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveSyrianaConfig(await getSyrianaConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
