...
import type { SyrianaReadDirResult } from '@/global'
import { $connection } from '@/store/session'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<SyrianaReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { syrianaDesktop: { readDir: typeof readDir } }).syrianaDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { syrianaDesktop?: unknown }).syrianaDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): SyrianaReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { syrianaDesktop: unknown }).syrianaDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: SyrianaReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SyrianaReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { syrianaDesktop: unknown }).syrianaDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.syrianaDesktop is missing', async () => {
    delete (window as unknown as { syrianaDesktop?: unknown }).syrianaDesktop
