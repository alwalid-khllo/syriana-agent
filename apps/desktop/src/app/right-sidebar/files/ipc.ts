...
import type { SyrianaReadDirEntry, SyrianaReadDirResult } from '@/global'
...
export type ProjectTreeEntry = SyrianaReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: SyrianaReadDirEntry) {
...
async function filterIgnored(entries: SyrianaReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<SyrianaReadDirResult> {
  if (!window.syrianaDesktop) {
