...
import { getSessionMessages, type SessionMessage, triggerCronJob } from '../syriana'
...
import { useSyrianaConfig } from './session/hooks/use-syriana-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.syrianaDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.syrianaDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.syrianaDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.syrianaDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // syriana:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.syrianaDesktop?.onDeepLink?.(payload => {
    ...
    void window.syrianaDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.syrianaDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshSyrianaConfig, sttEnabled, voiceMaxRecordingSeconds } = useSyrianaConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshSyrianaConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshSyrianaConfig])
