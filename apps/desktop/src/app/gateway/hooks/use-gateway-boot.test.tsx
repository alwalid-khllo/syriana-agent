...
// bug that drives the REAL useGatewayBoot hook + REAL SyrianaGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshSyrianaConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { syrianaDesktop?: unknown }).syrianaDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { syrianaDesktop?: unknown }).syrianaDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForSyriana) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startSyriana()'s remote branch awaits waitForSyriana() for 45s before it
    ...
    ;(window as { syrianaDesktop?: unknown }).syrianaDesktop = desktop
    ...
    // After ~45s waitForSyriana gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Syriana backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
