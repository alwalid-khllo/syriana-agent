import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@syriana/shared'
import { useEffect, useRef } from 'react'

import type { SyrianaConnection } from '@/global'
import { SyrianaGateway } from '@/syriana'
...
import type { RpcEvent } from '@/types/syriana'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.syrianaDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: SyrianaGateway | null) => void
  refreshSyrianaConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.syrianaDesktop

    const publish = (next: SyrianaConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Syriana...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Syriana…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Syriana gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshSyrianaConfig().catch(() => undefined)
    ...
    const gateway = new SyrianaGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshSyrianaConfig()
