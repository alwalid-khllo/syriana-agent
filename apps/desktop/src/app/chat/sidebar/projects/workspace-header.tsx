...
const branchActionLabel = (branch: SyrianaGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `syriana/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<SyrianaGitBranch[]>([])
  ...
  const convert = async (branch: SyrianaGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
