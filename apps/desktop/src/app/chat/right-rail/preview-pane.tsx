        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.syrianaDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Syriana to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.syrianaDesktop?.watchPreviewFile ||
      !window.syrianaDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.syrianaDesktop.onPreviewFileChanged(payload => {
    ...
    void window.syrianaDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.syrianaDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.syrianaDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:syriana-preview')
