...
const { getProfiles } = await import('@/syriana')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/syriana/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<SyrianaConnection> = {}): SyrianaConnection =>
  ({ baseUrl: 'https://syriana-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as SyrianaConnection

const localConn = (over: Partial<SyrianaConnection> = {}): SyrianaConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as SyrianaConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<SyrianaConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { syrianaDesktop: { getConnection } })
