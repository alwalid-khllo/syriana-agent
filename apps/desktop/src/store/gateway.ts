import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@syriana/shared'
...
const isOpen = (gateway: SyrianaGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<SyrianaGateway | null>(null)
...
let primaryGateway: SyrianaGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: SyrianaGateway | null, profile = 'default'): void {
...
export function activeGateway(): SyrianaGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<SyrianaGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
