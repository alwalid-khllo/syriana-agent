...
const STORAGE_KEY = 'syriana:native-notifications'
...
// "Backgrounded" = the user isn't on Syriana. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.syrianaDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.syrianaDesktop
