...
import { getSyrianaConfigRecord, saveSyrianaConfig } from '@/syriana'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getSyrianaConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveSyrianaConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
