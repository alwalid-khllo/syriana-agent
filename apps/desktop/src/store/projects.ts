...
import type { SyrianaGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/syriana'
...
const PROJECT_SCOPE_KEY = 'syriana.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Syriana gateway is not connected')
  }
...
// repo, returning where Syriana should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<SyrianaGitBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.syrianaDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.syrianaDesktop?.writeClipboard?.(path)
  }
}
