...
import type { SyrianaRepoStatus } from '@/global'

import { $repoStatus, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: SyrianaRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<SyrianaRepoStatus | null>) {
  ;(window as unknown as { syrianaDesktop?: unknown }).syrianaDesktop = { git: { repoStatus: impl } }
}
