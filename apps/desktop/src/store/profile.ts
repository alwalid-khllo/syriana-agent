...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/syriana'
...
import type { ProfileInfo } from '@/types/syriana'
...
// /api/profiles/active `current`). "default" is the root ~/.syriana. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'syriana.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'syriana.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.syrianaDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new SYRIANA_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.syrianaDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.syrianaDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'syriana.desktop.showAllProfiles'
...
// Switch to the default (root ~/.syriana) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.syrianaDesktop?.touchBackend?.(target).catch(() => undefined)
}
