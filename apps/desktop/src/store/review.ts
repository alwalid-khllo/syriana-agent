...
import type { SyrianaReviewFile, SyrianaReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Syriana' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'syriana.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'syriana.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'syriana.desktop.reviewTreeMode'
const SELECTED_KEY = 'syriana.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<SyrianaReviewFile[]>([])
...
export const $reviewShipInfo = atom<SyrianaReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['syrianaDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: SyrianaReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.syrianaDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.syrianaDesktop?.openExternal?.(url)
    }
