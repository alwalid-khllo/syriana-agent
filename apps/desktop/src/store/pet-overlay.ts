 ...
 * app's bounds and stays visible while Syriana is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'syriana.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'syriana.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.syrianaDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.syrianaDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.syrianaDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.syrianaDesktop?.petOverlay?.close()
}
