...
import type { OAuthProvider } from '@/types/syriana'
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('syriana-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('syriana-desktop-onboarded-v1')).toBe('1')
  })
