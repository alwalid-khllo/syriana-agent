...
import type { SessionInfo, UsageStats } from '@/types/syriana'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'syriana.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'syriana.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'syriana.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'syriana.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'syriana.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'syriana.desktop.lastSessionId'
...
function workspaceCwdKey(connection: SyrianaConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.syrianaDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.syrianaDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<SyrianaConnection | null>(null)
...
export const setConnection = (next: Updater<SyrianaConnection | null>) => updateAtom($connection, next)
