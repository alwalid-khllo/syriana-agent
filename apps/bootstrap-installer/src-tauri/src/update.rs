...
/// install tree before giving up and letting `syriana update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    // async on Windows. If it still holds the venv shim, `syriana update`
    ...
    // ---- stage 2: syriana update -----------------------------------------
    // Pass --branch so `syriana update` targets the branch this installer was
    ...
    // without --branch, `syriana update` switches the checkout to `main` (a
    ...
    // --force skips `syriana update`'s Windows running-exe guard (which would
    ...
    // Retry-once for the update-boundary crash. `syriana update` lazily imports
    ...
    // ---- stage 3: syriana desktop --build-only ----------------------------
    // `syriana update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `syriana update` already does above, and `syriana update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `syriana desktop` \
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // force-kill every Syriana.exe except ourselves, then give the OS a
...
/// Spawn `syriana <args>` from `cwd`, stream stdout/stderr as Log events on the
