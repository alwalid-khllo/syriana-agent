...
/// desktop app handing off via `Syriana-Setup.exe --update`.
...
export const $syrianaHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.syriana/logs/bootstrap-installer.log')
    $syrianaHome.set('~/.syriana')
  ...
  try {
    const [logPath, syrianaHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_syriana_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $syrianaHome.set(syrianaHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Syriana" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Syriana-Setup.exe --update);
...
export async function launchSyrianaDesktop(): Promise<void> {
  ...
  await invoke('launch_syriana_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Syriana repository' },
