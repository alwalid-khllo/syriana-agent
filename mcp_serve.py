"""
Syriana MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Syriana-specific extra)

Usage:
    syriana mcp serve
    syriana mcp serve --verbose
...
logger = logging.getLogger("syriana.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SYRIANA_HOME."""
    try:
        from syriana_constants import get_syriana_home
        return get_syriana_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SYRIANA_HOME", Path.home() / ".syriana")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from syriana_constants import get_syriana_home
        directory_file = get_syriana_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SYRIANA_HOME", Path.home() / ".syriana")
...
class EventBridge:
    ...
    This is the Syriana equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from syriana_constants import get_syriana_home
            db_file = get_syriana_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SYRIANA_HOME", Path.home() / ".syriana")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Syriana MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "syriana",
        instructions=(
            "Syriana Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Syriana MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
