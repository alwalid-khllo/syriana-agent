...
``syriana gateway restart`` (or ``launchctl kickstart ai.syriana.gateway``
or ``systemctl restart syriana-gateway``).  When the cron fires, the
...
``syriana cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``syriana gateway``, ``launchctl ... syriana-gateway``,
``systemctl ... syriana-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_SYRIANA_GATEWAY=1``, and
``syriana gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `syriana gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:syriana\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a syriana-gateway label. macOS launchd
    # labels look like `ai.syriana.gateway` / `syriana-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated syriana services (e.g.
    # `launchctl unload ai.syriana.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bsyriana[.\-]?gateway)"
    # Branch C: systemctl ops on a syriana-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bsyriana[.\-]?gateway)"
    # Branch D: pkill / kill targeting the syriana gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<SYRIANA_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.syriana/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_syriana_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `syriana gateway restart` from a shell outside "
