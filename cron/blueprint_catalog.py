...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``syriana://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``syriana://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"syriana://blueprint/{quote(blueprint.key)}{qs}"
