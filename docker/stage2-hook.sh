...
#
# Wired into the image as /etc/cont-init.d/01-syriana-setup by the
...
SYRIANA_HOME="${SYRIANA_HOME:-/opt/data}"
INSTALL_DIR="/opt/syriana"

# Drop to syriana via s6-setuidgid, but skip it when already non-root.
as_syriana() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid syriana "$@"; }
...
# starts non-root. The baked install tree under /opt/syriana is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $SYRIANA_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass SYRIANA_UID/SYRIANA_GID — or the PUID/PGID aliases — which the
...
# `id -u syriana` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# syriana UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u syriana)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-syriana UID).
...
and the baked /opt/syriana install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e SYRIANA_UID=\$(id -u) -e SYRIANA_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the syriana user to that UID/GID at boot and chowns the data
...
# --- Bootstrap SYRIANA_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid syriana mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom SYRIANA_HOME paths whose parents only
# root can create (e.g. `SYRIANA_HOME=/home/syriana/.syriana` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for SYRIANA_UID/SYRIANA_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  SYRIANA_UID/
# SYRIANA_GID still win when both are set.  See #15290, salvages #25872.
SYRIANA_UID="${SYRIANA_UID:-${PUID:-}}"
SYRIANA_GID="${SYRIANA_GID:-${PGID:-}}"

if [ -n "${SYRIANA_UID:-}" ] && validate_uid_gid "$SYRIANA_UID" && [ "$SYRIANA_UID" != "$(id -u syriana)" ]; then
    echo "[stage2] Changing syriana UID to $SYRIANA_UID"
    usermod -u "$SYRIANA_UID" syriana
fi
if [ -n "${SYRIANA_GID:-}" ] && validate_uid_gid "$SYRIANA_GID" && [ "$SYRIANA_GID" != "$(id -g syriana)" ]; then
    echo "[stage2] Changing syriana GID to $SYRIANA_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$SYRIANA_GID" syriana 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised syriana user (UID 10000)
...
# syriana process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes syriana. Idempotent across container
...
#   - syriana is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G syriana 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] syriana already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" syriana 2>/dev/null; then
        echo "[stage2] Added syriana to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group syriana failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When SYRIANA_UID is remapped or the top-level $SYRIANA_HOME isn't owned by
# the runtime syriana UID, restore ownership to syriana — but ONLY for the
# directories syriana actually writes to. The full $SYRIANA_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of syriana-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_syriana_uid=$(id -u syriana)
...
chown_syriana_tree() {
    ...
    chown -R syriana:syriana "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$SYRIANA_HOME" 2>/dev/null)" != "$actual_syriana_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $SYRIANA_HOME (targeted) to syriana ($actual_syriana_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $SYRIANA_HOME: chown the directory itself (not its contents)
    # so syriana can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$SYRIANA_HOME"; then
        :
    else
        chown syriana:syriana "$SYRIANA_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $SYRIANA_HOME failed (rootless container?) — continuing"
    fi
    # Syriana-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by syriana (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$SYRIANA_HOME/$sub" ]; then
            chown_syriana_tree "$SYRIANA_HOME/$sub"
        fi
    done
fi
...
# syriana user. Hosted/container instances keep mutable state under
# $SYRIANA_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# SYRIANA_DISABLE_LAZY_INSTALLS=1. Keeping /opt/syriana root-owned and
...
# $SYRIANA_HOME/lazy-packages dir on the data volume (Dockerfile sets
# SYRIANA_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to syriana
...
# Always reset ownership of $SYRIANA_HOME/profiles to syriana on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> syriana …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as syriana and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$SYRIANA_HOME/profiles" ]; then
    chown_syriana_tree "$SYRIANA_HOME/profiles"
fi

# Always reset ownership of $SYRIANA_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged syriana runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$SYRIANA_HOME/cron" ]; then
    chown_syriana_tree "$SYRIANA_HOME/cron"
fi

# Reset ownership of syriana-owned top-level state files on every boot.
# The targeted data-volume chown above only covers syriana-owned
# *subdirectories*; loose state files living directly under $SYRIANA_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> syriana …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged syriana runtime then hits
...
# sweep so host-owned files in a bind-mounted $SYRIANA_HOME are never
...
# entries of syriana_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$SYRIANA_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$SYRIANA_HOME/$f"; then
            :
        else
            chown syriana:syriana "$SYRIANA_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the syriana runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$SYRIANA_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$SYRIANA_HOME/config.yaml"; then
        :
    else
        chown syriana:syriana "$SYRIANA_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$SYRIANA_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as syriana user ---
# Run as syriana via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $SYRIANA_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_syriana mkdir -p \
    "$SYRIANA_HOME/backups" \
    "$SYRIANA_HOME/cron" \
    "$SYRIANA_HOME/sessions" \
    "$SYRIANA_HOME/logs" \
    "$SYRIANA_HOME/logs/gateways" \
    "$SYRIANA_HOME/hooks" \
    "$SYRIANA_HOME/memories" \
    "$SYRIANA_HOME/skills" \
    "$SYRIANA_HOME/skins" \
    "$SYRIANA_HOME/plans" \
    "$SYRIANA_HOME/workspace" \
    "$SYRIANA_HOME/home" \
    "$SYRIANA_HOME/pairing" \
    "$SYRIANA_HOME/platforms/pairing" \
    "$SYRIANA_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/syriana/.install_method (see Dockerfile), NOT written here into
# $SYRIANA_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $SYRIANA_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.syriana:/opt/data) and sometimes shared with a
...
# 'syriana update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $SYRIANA_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/syriana stamp, so deleting the data-dir copy is safe).
if [ -f "$SYRIANA_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$SYRIANA_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$SYRIANA_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$SYRIANA_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$SYRIANA_HOME/$dest"; then
            :
        else
            as_syriana cp "$INSTALL_DIR/$src" "$SYRIANA_HOME/$dest"
        fi
    fi
}
...
if [ -f "$SYRIANA_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$SYRIANA_HOME/.env"; then
        :
    else
        chown syriana:syriana "$SYRIANA_HOME/.env" 2>/dev/null || true
        chmod 600 "$SYRIANA_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $SYRIANA_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `syriana update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set SYRIANA_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$SYRIANA_HOME/config.yaml" ]; then
    s6-setuidgid syriana "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$SYRIANA_HOME/auth.json" ] && [ -n "${SYRIANA_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$SYRIANA_HOME/auth.json"; then
        :
    else
        printf '%s' "$SYRIANA_AUTH_JSON_BOOTSTRAP" > "$SYRIANA_HOME/auth.json"
        chown syriana:syriana "$SYRIANA_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$SYRIANA_HOME/auth.json"
    fi
fi
...
# can set SYRIANA_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# SYRIANA_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$SYRIANA_HOME/gateway_state.json" ] && \
        [ "${SYRIANA_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$SYRIANA_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$SYRIANA_HOME/gateway_state.json"
        chown syriana:syriana "$SYRIANA_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$SYRIANA_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_syriana "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/syriana/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Syriana spawns for the
...
# main-wrapper.sh propagates it into the supervised ``syriana`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-syriana, dashboard, etc.).
