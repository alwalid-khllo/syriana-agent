#!/bin/sh
# shellcheck shell=sh
# /opt/syriana/bin/syriana — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `syriana` user (UID 10000). When an operator runs `docker exec <c> syriana ...`
# the default UID is root (0), and any file the command writes under
# $SYRIANA_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> syriana login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Syriana is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> syriana
...
# Fix
# ---
# This shim sits at /opt/syriana/bin/syriana and is placed earliest on PATH.
# When invoked as root, it drops to the syriana user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $SYRIANA_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user syriana`, kanban subagents, etc. — it short-circuits
...
# (/opt/syriana/.venv/bin/syriana), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set SYRIANA_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the syriana CLI. Default is to drop.

set -e

REAL=/opt/syriana/.venv/bin/syriana

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "syriana-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user syriana`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${SYRIANA_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the syriana user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "syriana-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "syriana-shim: re-run with --user syriana or set SYRIANA_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the syriana user's home before dropping privileges. Without
