...
Every Syriana request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Syriana Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=syriana-agent",
        "client=syriana-client-v<__version__>",
    ]

The version is sourced live from ``syriana_cli.__version__`` so it auto-aligns
...
``syriana_cli.__version__`` is the canonical source of truth.
"""
...
def _syriana_version() -> str:
    """Return the current Syriana release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``syriana_cli`` cannot be imported (should
...
def syriana_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=syriana-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=syriana-client-v{_syriana_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=syriana-agent", syriana_client_tag()]
