...
as ``kind: backend``) or ``~/.syriana/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``syriana tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$SYRIANA_HOME/cache/videos/``, creating parents as needed."""
    from syriana_constants import get_syriana_home

    path = get_syriana_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$SYRIANA_HOME/cache/videos/``.
