...
Storage
-------
Bundles live in ``~/.syriana/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SYRIANA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``syriana bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under SYRIANA_HOME.

    Honors ``SYRIANA_BUNDLES_DIR`` for tests; falls back to
    ``<SYRIANA_HOME>/skill-bundles``.
    """
    override = os.environ.get("SYRIANA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_syriana_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `syriana bundles` CLI subcommand.
# ---------------------------------------------------------------------------
