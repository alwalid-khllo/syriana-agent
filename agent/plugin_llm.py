...
Plugins built on Syriana Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Syriana
...
:class:`~syriana_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Syriana' main config
...
handles every provider, fallback chain, and per-task override Syriana
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`syriana_cli.plugins.PluginContext`
