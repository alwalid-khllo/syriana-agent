...
Owns one Codex thread per Syriana session. Drives `turn/start`, consumes
...
# Syriana' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_SYRIANA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Syriana/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Syriana session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _SYRIANA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("SYRIANA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="syriana",
            client_title="Syriana Agent",
            client_version=_get_syriana_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Syriana' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Syriana' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own syriana-tools callback we
            # auto-accept — the user already approved Syriana' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Syriana approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Syriana returns 'once', 'session', 'always', or 'deny'.
...
def _get_syriana_version() -> str:
    """Best-effort Syriana version string for codex's userAgent line."""
    try:
