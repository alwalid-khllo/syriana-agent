...
projection into Syriana' display, approval bridging, transcript projection into
...
"codex_app_server"`. Syriana' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Syriana secret — gateway
        ...
        spawn_env = syriana_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("SYRIANA_KANBAN_TASK"):
            kanban_db = spawn_env.get("SYRIANA_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "SYRIANA_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("SYRIANA_HOME", os.path.expanduser("~/.syriana")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "syriana",
        client_title: str = "Syriana Agent",
