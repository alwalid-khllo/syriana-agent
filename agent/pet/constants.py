...
row taxonomy is inferred from the atlas shape so Syriana can render both legacy
...
# rows (1536x1872), while older Syriana/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Syriana' activity state names. They are not always identical to the
...
# Legacy Syriana/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Syriana activity names -> accepted row-name aliases in descending
