...
upstream prefix cache warm.  See ``syriana-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Syriana never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want SYRIANA_HOME persona while keeping
    ...
    # Pointer to the syriana-agent skill + docs for user questions about Syriana itself.
    stable_parts.append(SYRIANA_AGENT_HELP_GUIDANCE)
    ...
    # SYRIANA_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Syriana, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Syriana profile the agent is running
    # under so it doesn't conflate ~/.syriana/skills/ (default profile) with
    # ~/.syriana/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Syriana profile: default. Other profiles (if any) live "
            "under ~/.syriana/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Syriana profile: {active_profile}. This session reads "
            f"and writes ~/.syriana/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.syriana/skills/, ~/.syriana/plugins/, "
            f"~/.syriana/cron/, ~/.syriana/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Syriana never rebuilds or reinjects parts of it
