...
``auth.json``.  They intentionally have no dependency on ``syriana_cli.auth`` so
...
# Sources Syriana owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Syriana-owned OAuth/device-code state)
