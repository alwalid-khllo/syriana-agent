...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `syriana backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - syriana_home (str): The active SYRIANA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.syriana``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "syriana").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'syriana memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], syriana_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'syriana memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``syriana_home`` is the active SYRIANA_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE SYRIANA_HOME.

        ``syriana backup`` only walks SYRIANA_HOME, so any provider state kept
        ...
        of the archive, and ``syriana import`` restores them to their original
