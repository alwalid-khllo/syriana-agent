...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Syriana starts and finds ``~/.openclaw/``.

    Points users at ``syriana claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``syriana claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Syriana, run "
        "`syriana claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `syriana claw cleanup` (renames it to ~/.openclaw.pre-migration — "
