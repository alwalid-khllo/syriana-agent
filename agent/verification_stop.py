...
def _session_is_messaging_surface() -> bool:
    ...
    ``SYRIANA_SESSION_PLATFORM``; the CLI and TUI set ``SYRIANA_SESSION_SOURCE``
    (e.g. ``cli``, ``tui``) instead. Both are consulted via the session-context
    helper (with an ``os.environ`` fallback), alongside the ``SYRIANA_PLATFORM``
    ...
    try:
        ...
        platform = os.getenv("SYRIANA_PLATFORM", "") or os.environ.get(
...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``SYRIANA_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("SYRIANA_VERIFY_ON_STOP")
    if env is not None:
    ...
    *,
    ...
    if verify_commands:
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `syriana-verify-` filename prefix, run it "
