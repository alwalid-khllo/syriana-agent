...
Syriana pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.syriana/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<syriana_home>/bin/bws`` on
  first use.  Syriana pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.syriana/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``syriana`` invocations don't
  hammer the API.
* Failures NEVER block Syriana startup.  Missing binary, no network,
...
# In-process cache so repeated load_syriana_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `syriana chat -q ...`
...
# <syriana_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.syriana/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under syriana_home/cache/.

    `home_path` is what `load_syriana_dotenv()` already resolved; falling back
    to `$SYRIANA_HOME` / `~/.syriana` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("SYRIANA_HOME", Path.home() / ".syriana"))
...
def _syriana_bin_dir() -> Path:
    """Where Syriana stores its managed binaries.  Profile-aware."""
    from syriana_constants import get_syriana_home

    return get_syriana_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<syriana_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _syriana_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``syriana secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _syriana_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="syriana-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "syriana-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<syriana_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from syriana_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_syriana_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `syriana secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `syriana secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `syriana secrets bitwarden setup` to install."
        )
        return result
