...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the SYRIANA_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Syriana runtime / deployment
    "SYRIANA_HOME", "SYRIANA_PROFILE", "SYRIANA_GATEWAY_LOCK_DIR",
    "SYRIANA_MAX_ITERATIONS", "SYRIANA_MAX_TOKENS", "SYRIANA_API_TIMEOUT",
    "SYRIANA_REDACT_SECRETS", "SYRIANA_NOUS_TIMEOUT_SECONDS",
    "_SYRIANA_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "SYRIANA_KANBAN_DB", "SYRIANA_KANBAN_WORKSPACES_ROOT", "SYRIANA_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "SYRIANA_KANBAN_",
    "SYRIANA_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(syriana_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(syriana_home) / ".env")
