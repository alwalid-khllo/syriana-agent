...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_SYRIANA_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Syriana expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``syriana_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_syriana_home()`` themselves.
        """
        if "syriana_home" not in kwargs:
            from syriana_constants import get_syriana_home
            kwargs["syriana_home"] = str(get_syriana_home())
        for provider in self._providers:
            try:
