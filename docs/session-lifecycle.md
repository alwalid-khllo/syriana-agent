...
The canonical transcript store is SQLite via `SessionDB` (from `syriana_state`). The
...
Counts consecutive restarts via a JSON file (`{SYRIANA_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `syriana update`, `syriana gateway restart`,
or `/restart`.
