...
Syriana middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Syriana'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Syriana calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Syriana retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Syriana guardrails,
...
- `telemetry_schema_version`: currently `syriana.observer.v1`
- `middleware_schema_version`: currently `syriana.middleware.v1`
...
Syriana stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Syriana runs
them as a nested chain in registration order. Middleware failures are fail-open:
Syriana logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Syriana applies middleware in this order:
...
For each tool call, Syriana applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Syriana pre-execution path against the effective arguments:
...
```bash
syriana plugins enable <plugin-name>
```

For isolated local testing, use one `SYRIANA_HOME` for plugin enablement and the
agent run:

```bash
export SYRIANA_HOME=/tmp/syriana-middleware-test
mkdir -p "$SYRIANA_HOME"
syriana plugins enable <plugin-name>
syriana chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run syriana plugins enable <plugin-name>
uv run syriana chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["syriana_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/syriana-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Syriana expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Syriana treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Syriana preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Syriana does not convert downstream
