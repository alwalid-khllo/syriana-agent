...
When running Syriana inside Docker, the default `network_mode: host` gives the
...
The Syriana [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ syriana-agent │   │ syriana-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   SYRIANA_UID=$(id -u) SYRIANA_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=syriana,syriana-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://syriana-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Syriana trust model and vulnerability reporting
