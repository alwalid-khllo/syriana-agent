...
This document is the formal interface between the **Syriana gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`SyrianaAIResearch/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `syriana gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`syriana gateway enroll` → connector `/relay/enroll`, or
...
(`SyrianaAIResearch/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `syriana` CLI.
