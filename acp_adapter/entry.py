"""CLI entry point for the syriana-agent ACP adapter.

Loads environment variables from ``~/.syriana/.env``, configures logging
...
try:
    ...
    # shadowing Syriana's own modules — ``syriana acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    syriana_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from SYRIANA_HOME (default ``~/.syriana``)."""
    from syriana_cli.env_loader import load_syriana_dotenv

    syriana_home = get_syriana_home()
    loaded = load_syriana_dotenv(syriana_home=syriana_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", syriana_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="syriana-acp",
        description="Run Syriana Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Syriana version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Syriana provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.syriana/node/ "
...
def _print_version() -> None:
    ...
    print(syriana_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import SyrianaACPAgent  # noqa: F401

    print("Syriana ACP check OK")


def _run_setup() -> None:
    from syriana_cli.main import main as syriana_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "syriana", "model"]
        syriana_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``syriana postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting syriana-agent ACP adapter")
    ...
    import acp
    from .server import SyrianaACPAgent
