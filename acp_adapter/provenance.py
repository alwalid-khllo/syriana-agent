...
This is an additive Syriana extension surfaced under ACP ``_meta.syriana`` so
...
compression rotates the internal Syriana head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.syriana.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_syriana_session_id: The live internal Syriana DB session id
            (``state.agent.session_id``).
        previous_syriana_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"syriana": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_syriana_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_syriana_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_syriana_session_id}
    ...
    rotated = bool(
        ...
        and previous_syriana_session_id != current_syriana_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentSyrianaSessionId": current_syriana_session_id,
        "rootSyrianaSessionId": root_id,
        "parentSyrianaSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_syriana_session_id:
        provenance["previousSyrianaSessionId"] = previous_syriana_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"syriana": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"syriana": {"sessionProvenance": prov}}
