"""ACP auth helpers — detect and advertise Syriana authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "syriana-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Syriana runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Syriana can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Syriana.
    ...
    not have Syriana provider credentials configured yet, so Syriana always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Syriana using the currently configured "
