...
Pure display functions with no SyrianaCLI state dependency.
"""
...
from syriana_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built syriana — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/fixology/syriana-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/fixology/syriana-agent"
...
def _fetch_pypi_latest(package: str = "syriana-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Syriana update is available.

    Two paths: if ``SYRIANA_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = syriana_home / ".update_check"
    embedded_rev = os.environ.get("SYRIANA_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # SYRIANA_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `syriana update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/syriana/update/check`
    ...
    if embedded_rev:
        ...
        # $SYRIANA_HOME/syriana-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = syriana_home / "syriana-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Syriana git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$SYRIANA_HOME/syriana-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``syriana_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from syriana_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/fixology/syriana-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Syriana checkout. Cached per-process. Release URL always points at the
    canonical SyrianaAIResearch/syriana-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Syriana Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else SYRIANA_CADUCEUS
    except Exception:
        _bskin = None
        _hero = SYRIANA_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]syriana ai Research[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]syriana ai Research[/]")

    if os.getenv("SYRIANA_YOLO_MODE"):
    ...
    try:
        from syriana_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from syriana_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built syriana; we know an update
    ...
    # Pip-install warning — `pip install syriana-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else SYRIANA_AGENT_LOGO
        try:
            import time
            import sys
            ...
            raw_text = Text.from_markup(_logo).plain
            
            # Animate if terminal supports it and not in YOLO mode
            if sys.stdout.isatty() and not os.getenv("SYRIANA_YOLO_MODE"):
                duration = 1.8
                fps = 30
                frames = int(duration * fps)
                
                # Gold -> Orange -> Red -> Gold
                colors = [
                    (255, 215, 0), (255, 140, 0), (255, 69, 0), (255, 215, 0)
                ]
                ...
                def get_settled_color(x, max_x):
                    pos = (x / float(max_x or 1)) % 1.0
                    idx = int(pos * (len(colors) - 1))
                    t = (pos * (len(colors) - 1)) - idx
                    c = interpolate_color(colors[idx], colors[idx + 1], t)
                    return Color.from_rgb(c[0], c[1], c[2])

                with Live(auto_refresh=False, console=console, transient=False) as live:
                    lines = raw_text.split('\n')
                    max_x = max(len(line) for line in lines) if lines else 1
                    import random
                    import string
                    glitch_chars = "01!<>*&^%$#@~`|\\/?[]{}()-" + string.ascii_letters
                    
                    for frame in range(frames):
                        progress = frame / float(max(frames - 1, 1))
                        glitch_window = 15
                        wipe_pos = int(progress * (max_x + glitch_window))
                        ...
                        for y, line in enumerate(lines):
                            for x, char in enumerate(line):
                                ...
                                elif x < wipe_pos - glitch_window:
                                    animated_text.append(char, style=Style(color=get_settled_color(x, max_x), bold=True))
                                elif x < wipe_pos:
                                    dist_to_front = wipe_pos - x
                                    if random.random() < (dist_to_front / float(glitch_window)):
                                        rchar = random.choice(glitch_chars)
                                        animated_text.append(rchar, style=Style(color="bright_white", bold=True))
                                    else:
                                        rcolor = random.choice(["bright_yellow", "bright_red", "gold3"])
                                        animated_text.append(char, style=Style(color=rcolor, bold=True))
                                else:
                        ...
                        live.update(animated_text, refresh=True)
                        time.sleep(1.0 / fps)
                    
                    # Snap to the exact original colors
                    live.update(Text.from_markup(_logo), refresh=True)
            else:
                console.print(_logo)
        except Exception:
            console.print(_logo)
