...
on Japanese locales, etc.).  Syriana's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``syriana_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Syriana is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SYRIANA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SYRIANA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Syriana-managed Git install directories so
    ...
    # already-running shells don't see the change — which means syriana
    ...
    # install without Syriana-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Syriana's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Syriana's
    ``syriana config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Syriana (User env var in Windows Settings, or
    export in a PowerShell profile) and Syriana picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Syriana-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\syriana\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``syriana`` from right after install) retain their old PATH.

    Any subprocess Syriana spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Syriana-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "syriana", "git", "cmd"),
        os.path.join(local_appdata, "syriana", "git", "bin"),
        os.path.join(local_appdata, "syriana", "git", "usr", "bin"),
        # Syriana venv Scripts directory — host of the syriana.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes syriana, but harmless to include.
        os.path.join(local_appdata, "syriana", "syriana-agent", "venv", "Scripts"),
