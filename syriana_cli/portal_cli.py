"""``syriana portal`` — the human-readable entry point for Nous Portal.

Running ``syriana portal`` with no subcommand performs the one-shot Portal
...
``syriana auth add nous --type oauth`` (which still works), is identical to
``syriana setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``syriana auth`` or ``syriana tools``. It's the onboarding + discovery
...
DEFAULT_PORTAL_URL = "https://portal.fixology.dev"
SUBSCRIPTION_URL = "https://portal.fixology.dev/manage-subscription"
DOCS_URL = "https://syriana-agent.fixology.dev/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `syriana model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `syriana portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `syriana auth add nous --type
    oauth`. It reuses the exact wiring behind `syriana setup --portal` (which in
    ...
    """
    from syriana_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `syriana portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `syriana portal` is the
        # human-readable alias for `syriana auth add nous --type oauth` /
    ...
    print("Run `syriana portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `syriana portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `syriana portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `syriana auth add "
            "nous --type oauth`, identical to `syriana setup --portal`). "
