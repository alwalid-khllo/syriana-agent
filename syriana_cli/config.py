"""
Configuration management for Syriana Agent.

Config files are stored in ~/.syriana/ for easy access:
- ~/.syriana/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.syriana/.env         - API keys and secrets

This module provides:
- syriana config          - Show current configuration
- syriana config edit     - Open config in editor
- syriana config set      - Set a specific value
- syriana config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``syriana config set`` (which rewrites
    ...
    ``config.yaml`` in place: syriana never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.syriana/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``syriana logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  syriana config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Syriana makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Syriana itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``syriana update``, the TUI build.
...
#   on every plugin install / ``syriana update``.
...
# * ``SYRIANA_HOME`` / ``SYRIANA_PROFILE`` / ``SYRIANA_CONFIG`` /
#   ``SYRIANA_ENV`` — Syriana runtime location flags. Writing these into
...
#
# IMPORTANT: ``SYRIANA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (SYRIANA_LANGFUSE_PUBLIC_KEY,
# SYRIANA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Syriana runtime location — never via dashboard env writer.
    # NOT a SYRIANA_* blanket: integration credentials (SYRIANA_GEMINI_*,
    # SYRIANA_LANGFUSE_*, SYRIANA_SPOTIFY_*, ...) ARE allowed.
    "SYRIANA_HOME", "SYRIANA_PROFILE", "SYRIANA_CONFIG", "SYRIANA_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Syriana runtime location "
            "(SYRIANA_HOME, SYRIANA_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.syriana/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "SYRIANA_TOOL_PROGRESS", "SYRIANA_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `syriana plugins enable
    # observability/langfuse` or `syriana tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "SYRIANA_LANGFUSE_ENV",
    "SYRIANA_LANGFUSE_RELEASE",
    "SYRIANA_LANGFUSE_SAMPLE_RATE",
    "SYRIANA_LANGFUSE_MAX_CHARS",
    "SYRIANA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from syriana_cli.colors import Colors, color
from syriana_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SYRIANA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_syriana_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Syriana is running in package-manager-managed mode.

    Two signals: the SYRIANA_MANAGED env var (set by the systemd service),
    or a .managed marker file in SYRIANA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade syriana-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``syriana_cli/`` — i.e. the git checkout for source
    installs, ``/opt/syriana`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$SYRIANA_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Syriana was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$SYRIANA_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. SYRIANA_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.syriana``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$SYRIANA_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.syriana:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$SYRIANA_HOME``, a
    ...
    ``docker`` and ``syriana update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``fixology/syriana-agent`` image bakes a ``docker``
        stamp into ``/opt/syriana`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $SYRIANA_HOME.
    try:
    ...
    #    host install whose shared $SYRIANA_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``syriana update``.
    try:
        method = (
            (get_syriana_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``syriana_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$SYRIANA_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/syriana`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Syriana lives in a ``uv tool`` layout.

    ``uv tool install syriana-agent`` places the install at
    ``.../uv/tools/syriana-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``syriana-agent`` happens to be uv-tool-installed on the machine while
    the *active* Syriana is a regular pip/venv install, causing
    ``syriana update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/syriana-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade syriana-agent"
    if method == "docker":
        return "docker pull fixology/syriana-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade syriana-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade syriana-agent"
        return "pip install --upgrade syriana-agent"
    return "syriana update"
...
# Long-form text for ``syriana update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# syriana_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Syriana, it doesn't update
...
✗ ``syriana update`` doesn't apply inside the Docker container.

Syriana Agent runs as a published image (fixology/syriana-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull fixology/syriana-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate syriana-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm fixology/syriana-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/fixology/syriana-agent/tags
  • Your config and session history live under ``$SYRIANA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``syriana update`` inside Docker.
...
def format_managed_message(action: str = "modify this Syriana installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("SYRIANA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Syriana installation is managed by NixOS "
            f"(SYRIANA_MANAGED={env_hint}).\n"
            "Edit services.syriana-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Syriana installation is managed by Homebrew "
            f"(SYRIANA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade syriana-agent"
        )

    return (
        f"Cannot {action}: this Syriana installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Syriana."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SYRIANA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, syriana_bin
    or None if container mode is not active, we're already inside the
    container, or SYRIANA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SYRIANA_DEV") == "1":
        return None
    ...
    container_mode_file = get_syriana_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "syriana-agent")
    exec_user = info.get("exec_user", "syriana")
    syriana_bin = info.get("syriana_bin", "/data/current-package/bin/syriana")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "syriana_bin": syriana_bin,
    }
...
# Re-export from syriana_constants — canonical definition lives there.
from syriana_constants import get_syriana_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_syriana_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_syriana_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_syriana_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the SYRIANA_UID / SYRIANA_GID env vars set by Docker deployments.

    Docker containers running Syriana commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level SYRIANA_HOME once, but
    subdirectories created at runtime by ``ensure_syriana_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("SYRIANA_UID", "").strip()
    gid_str = os.environ.get("SYRIANA_GID", "").strip()
    try:
...
def _chown_to_syriana_uid(path) -> None:
    """Chown ``path`` to ``SYRIANA_UID:SYRIANA_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_syriana_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_syriana_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the syriana group can
    share state with the gateway service.

    The mode can be overridden via the SYRIANA_HOME_MODE environment variable
    (e.g. SYRIANA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SYRIANA_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``SYRIANA_UID``/``SYRIANA_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SYRIANA_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_syriana_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Syriana runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SYRIANA_CONTAINER") or os.environ.get("SYRIANA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SYRIANA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SYRIANA_HOME, upgrading legacy empty templates.
...
def ensure_syriana_home():
    """Ensure ~/.syriana directory structure exists with secure permissions.
    ...
    # Named profiles must be created explicitly (e.g. ``syriana profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old SYRIANA_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_syriana_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_syriana_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SYRIANA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Syriana-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Syriana
        ...
        # identity slot (SOUL.md). Empty by default. The SYRIANA_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Syriana adds a coding operating brief
    ...
    "terminal": {
        ...
        #             SYRIANA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force SYRIANA_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Syriana auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Syriana sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Syriana
        ...
        # bundled Syriana image, which drops to the `syriana` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Syriana sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Syriana should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``syriana chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.syriana/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: syriana sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .syriana.md, or .cursorrules before Syriana applies
    ...
    # single read_file page exceeds these limits, Syriana truncates the
    ...
    # See tools/kanban_tools.py and syriana_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``syriana dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``syriana kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``syriana kanban decompose`` and the kanban
        ...
        # ``syriana profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `syriana model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `syriana` (and `syriana chat`) launches by default:
        ...
        # REPL and `--tui` (or SYRIANA_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `syriana --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `syriana -c` muscle memory.  Default off so existing
        # users aren't surprised.  SYRIANA_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `syriana --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/syriana`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `syriana pets`. Disabled until
        ...
        "pet": {
            ...
            # get_syriana_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``SYRIANA_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``SYRIANA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.fixology.dev).
        "oauth": {
        ...
        # (``SYRIANA_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``SYRIANA_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``SYRIANA_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/syriana``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Syriana appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.syriana/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.syriana/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.syriana/cache/delegation/
    ...
    # response. If not, Syriana feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Syriana auto-pauses the goal and
    ...
    "moa": {
        ...
        # <syriana_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.syriana/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${SYRIANA_SKILL_DIR} and ${SYRIANA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `syriana curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `syriana curator run --consolidate` overrides
        ...
        # normally restored on every `syriana update`, so pruning them only
        ...
        # skipped), snapshot ~/.syriana/skills/ into
        # ~/.syriana/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `syriana curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for syriana-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Syriana Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (SYRIANA_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Syriana' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.syriana/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SYRIANA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `syriana doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``syriana_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Syriana to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $SYRIANA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        "chronos": {
            ...
            "portal_url": "https://portal.fixology.dev",
        ...
        # Also overridable via SYRIANA_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `syriana -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `syriana` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `syriana kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     syriana-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Syriana tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.syriana/logs/.
    ...
    # update model picker lists without shipping a syriana-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://syriana-agent.fixology.dev/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `syriana model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal SYRIANA_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # the SYRIANA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.syriana.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.syriana/.env,
        ...
        # behavior: files must live under the Syriana cache, under
        ...
        # user. Bridged to SYRIANA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Syriana
        # cache (~/.syriana/cache/{documents,images,audio,video,screenshots})
        ...
        # to SYRIANA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # SYRIANA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to SYRIANA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.syriana/state.db.
    ...
    "sessions": {
        ...
        # default of ``syriana sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.syriana/sessions/session_{sid}.json`` on every turn
    ...
    # ``syriana update`` behaviour.
    "updates": {
        # Run a full ``syriana backup``-style zip of SYRIANA_HOME before every
        # ``syriana update``.  Backups land in ``<SYRIANA_HOME>/backups/`` and
        # can be restored with ``syriana import <path>``.  Off by default:
        # zipping a large SYRIANA_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``syriana update
        ...
        "backup_keep": 5,
        # What `syriana update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<SYRIANA_HOME>/lsp/bin/`` on first use.
    ...
    # `syriana tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.syriana/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.syriana/.env (or your shell) and never in config.yaml.
            ...
            # ~/.syriana/bin/ on first use.  When False you must install
            ...
            # `syriana secrets bitwarden setup`.
            "server_url": "",
        },
    },
    ...
    "computer_use": {
        ...
        # by default upstream. Syriana disables it for our users unless they
        # explicitly opt in here. When false (default), Syriana sets
    ...
    # Syriana Desktop (Electron app) launch options. These only affect
    # `syriana desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the SYRIANA_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Syriana mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'syriana model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. fixology.dev -> firecrawl-gateway.fixology.dev",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Syriana auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Syriana, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @syriana:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SYRIANA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #syriana)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: syriana-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'syriana-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Syriana API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Syriana API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Syriana API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SYRIANA_TOOL_PROGRESS and SYRIANA_TOOL_PROGRESS_MODE are deprecated —
    ...
    "SYRIANA_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.syriana.config`` entries, then checks
    ...
    try:
        ...
        # should never break `syriana update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Syriana versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``syriana_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'syriana setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Syriana won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'syriana doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("SYRIANA_HOME", "~/.syriana")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "syriana update / syriana -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("SYRIANA_TOOL_PROGRESS")
            old_mode = get_env_value("SYRIANA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from SYRIANA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from SYRIANA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("SYRIANA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from SYRIANA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$SYRIANA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_syriana_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `syriana plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `syriana curator status` has no
    ...
    #   3. Creates `~/.syriana/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_syriana_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_syriana_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `syriana config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `syriana config set`"
                    )
    ...
    # behavior set it to true explicitly via `syriana config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from syriana_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from syriana_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: syriana config set <key> <value>")
    ...
    # `syriana update`, but only the version bump is persisted.
    ...
    # metadata.syriana.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``syriana config set``
    ...
    backends) — while display paths (``syriana status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _syriana_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.syriana/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.syriana/config.yaml.
    ...
    (which change ``SYRIANA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/syriana/config.yaml invalidates the
...
#   openai-codex (OAuth — syriana auth) — OpenAI Codex
#   nous         (OAuth — syriana auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.syriana/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Syriana writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.syriana/.env.
    ...
    interactive menu render (`syriana tools`, `syriana setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `syriana tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Syriana env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.syriana/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.syriana/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.syriana/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.syriana/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Syriana — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Syriana vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.syriana/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.syriana/.env`` over ``os.environ``.

    Used for Syriana-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Syriana Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from syriana_cli.auth import get_anthropic_key
    ...
    # Warn on stale SYRIANA_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("SYRIANA_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale SYRIANA_MAX_ITERATIONS={_env_ghost} "
                f"(run 'syriana doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  syriana config edit     # Edit config file", Colors.DIM))
    print(color("  syriana config set <key> <value>", Colors.DIM))
    print(color("  syriana setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `syriana config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `syriana config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: syriana config set <key> <value>")
            print()
            print("Examples:")
            print("  syriana config set model anthropic/claude-sonnet-4")
            print("  syriana config set terminal.backend docker")
            print("  syriana config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'syriana config migrate' to add them")
        ...
        print("  syriana config           Show current configuration")
        print("  syriana config edit      Open config in editor")
        print("  syriana config set <key> <value>   Set a config value")
        print("  syriana config check     Check for missing/outdated config")
        print("  syriana config migrate   Update config with new options")
        print("  syriana config path      Show config file path")
        print("  syriana config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``syriana config`` UI so users
