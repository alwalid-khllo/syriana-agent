"""On-demand supply-chain audit for Syriana Agent installs.

Scans three surfaces a Syriana user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Syriana venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.syriana/plugins``
...
def _discover_plugins(syriana_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.syriana/plugins``.
...
def run_audit(
    *,
    ...
    home = syriana_home or Path(get_syriana_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, syriana_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(syriana_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `syriana security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, syriana_home=home
    )
    if total == 0:
