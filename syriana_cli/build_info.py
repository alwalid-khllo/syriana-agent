"""
Baked-in build metadata for Syriana Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``syriana_cli/dump.py`` and ``syriana_cli/banner.py``).  That doesn't work inside
...
To make ``syriana dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$SYRIANA_GIT_SHA``
arg into ``<project_root>/.syriana_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``SYRIANA_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".syriana_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.syriana_build_sha`` if present.  The file is
    written by the Dockerfile's ``SYRIANA_GIT_SHA`` build-arg and contains
