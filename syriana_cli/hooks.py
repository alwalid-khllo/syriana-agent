"""syriana hooks — inspect and manage shell-script hooks.

Usage::

    syriana hooks list
    syriana hooks test <event> [--for-tool X] [--payload-file F]
    syriana hooks revoke <command>
    syriana hooks doctor

Consent records live under ``~/.syriana/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.syriana/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``syriana hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: syriana hooks {list|test|revoke|doctor}")
        print("Run 'syriana hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.syriana/config.yaml.")
        print("See `syriana hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `syriana hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `syriana hooks test` and `syriana hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Syriana wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `syriana hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `syriana hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `syriana hooks doctor`.")
