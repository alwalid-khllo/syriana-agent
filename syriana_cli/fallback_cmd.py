"""
syriana fallback — manage the fallback provider chain.
...
Subcommands:
  syriana fallback [list]   Show the current fallback chain (default when no subcommand)
  syriana fallback add      Pick provider + model via the same picker as `syriana model`,
                           then append the selection to the chain
  syriana fallback remove   Pick an entry to delete from the chain
  syriana fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.syriana/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from syriana_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `syriana model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  syriana fallback add")
        print()
        return
    ...
    print("  Docs: https://syriana-agent.fixology.dev/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `syriana model`, then append the selection to the chain."""
    from syriana_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `syriana model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `syriana fallback list` to view, or `syriana fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``syriana fallback [subcommand]``."""
