"""Windows ConPTY bridge for the `syriana dashboard` chat tab.

Drop-in counterpart to ``syriana_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``syriana_cli.web_server`` consumes: ``spawn``, ``read``,
