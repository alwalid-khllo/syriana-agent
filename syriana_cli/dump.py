"""
Dump command for syriana CLI.

Outputs a compact, plain-text summary of the user's Syriana setup
...
from syriana_cli.config import get_syriana_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.syriana/.env.
    ...
    exports. ``syriana debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.syriana_build_sha`` by the Dockerfile's
    ``SYRIANA_GIT_SHA`` build-arg (see ``syriana_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``syriana dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from syriana_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(syriana_home: Path) -> int:
...
def _cron_summary(syriana_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = syriana_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"syriana_home:      {display_syriana_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.syriana/.env: a managed
        ...
        # A credential added via `syriana auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `syriana auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["syriana-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(syriana_home)}")
    lines.append(f"  skills:             {_count_skills(syriana_home)}")
