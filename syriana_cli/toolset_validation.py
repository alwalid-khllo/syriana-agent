...
the tool registry or launching Syriana (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``syriana-cli`` to the non-existent ``syriana``. ``resolve_toolset('syriana')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``syriana-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``syriana`` instead of ``syriana-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `syriana tools` to reconfigure."
        )
    return warnings
