"""Shell completion script generation for syriana CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_syriana_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Syriana Agent bash completion
# Add to ~/.bashrc:
#   eval "$(syriana completion bash)"

_syriana_profiles() {{
    local profiles_dir="$HOME/.syriana/profiles"
...
_syriana_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_syriana_profiles)" -- "$cur"))
        return
    fi
...
complete -F _syriana_completion syriana
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef syriana
# Syriana Agent zsh completion
# Add to ~/.zshrc:
...
_syriana_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.syriana/profiles" ]]; then
        profiles+=($HOME/.syriana/profiles/*(N/:t))
    fi
...
_syriana() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_syriana_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'syriana command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Syriana Agent fish completion",
        "# Add to your config:",
        "#   syriana completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __syriana_profiles",
        "    echo default",
        "    if test -d $HOME/.syriana/profiles",
        "        for d in $HOME/.syriana/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c syriana -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c syriana -f -s p -l profile"
        " -d 'Profile name' -xa '(__syriana_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__syriana_profiles)' -d 'Profile name'"
                )
