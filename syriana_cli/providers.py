"""
Single source of truth for provider identity in Syriana Agent.
...
2. **Syriana overlays** — transport type, auth patterns, aggregator flags,
...
# -- Syriana overlay ----------------------------------------------------------
# Syriana-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class SyrianaOverlay:
    """Syriana-specific provider metadata layered on top of models.dev."""
...
SYRIANA_OVERLAYS: Dict[str, SyrianaOverlay] = {
    "moa": SyrianaOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": SyrianaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nous": SyrianaOverlay(
        ...
        base_url_override="https://inference.fixology.dev/v1",
    ),
    "openai-codex": SyrianaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": SyrianaOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": SyrianaOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": SyrianaOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="SYRIANA_QWEN_BASE_URL",
    ),
    "lmstudio": SyrianaOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": SyrianaOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": SyrianaOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": SyrianaOverlay(
    ...
    "kimi-for-coding": SyrianaOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": SyrianaOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": SyrianaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": SyrianaOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": SyrianaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": SyrianaOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": SyrianaOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": SyrianaOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": SyrianaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": SyrianaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": SyrianaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": SyrianaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": SyrianaOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": SyrianaOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": SyrianaOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": SyrianaOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": SyrianaOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": SyrianaOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "syriana", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Syriana overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Syriana overlay
    ...
    overlay = SYRIANA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + syriana extra
    ...
    if overlay is not None:
        # Syriana-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in SYRIANA_OVERLAYS
    if provider == "bedrock":
