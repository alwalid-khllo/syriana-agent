...
Profile-aware location: ``$SYRIANA_HOME/logs/dashboard-auth.log``.
...
from ``syriana_constants`` or other syriana_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$SYRIANA_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``syriana_constants.get_syriana_home`` semantics: env var wins,
    else ``~/.syriana``. A local copy avoids an import cycle with the
    middleware which lives below ``syriana_cli``.
    """
    home = os.environ.get("SYRIANA_HOME") or str(Path.home() / ".syriana")
