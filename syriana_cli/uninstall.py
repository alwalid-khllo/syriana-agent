"""
Syriana Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.syriana/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Syriana PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing syriana-agent or syriana PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Syriana Agent" comment and following line
                if '# Syriana Agent' in line or '# syriana-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('syriana' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing syriana
                if 'syriana' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the syriana wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "syriana",
        Path("/usr/local/bin/syriana"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains syriana_cli reference)
                content = wrapper.read_text()
                if 'syriana_cli' in content or 'syriana-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(syriana_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``syriana`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Syriana home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (syriana_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``syriana gateway run`` processes
    ...
    try:
        from syriana_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from syriana_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SYRIANA_HOME`` and ``SYRIANA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\syriana\git\cmd``, ``%LOCALAPPDATA%\syriana\git\bin``,
#      ``%LOCALAPPDATA%\syriana\git\usr\bin``, ``%LOCALAPPDATA%\syriana\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\syriana\git\`` and Node to
#      ``%LOCALAPPDATA%\syriana\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``syriana dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\syriana\gateway-service\`` and sometimes
...
def _syriana_path_markers(syriana_home: Path) -> list[str]:
    """Path-entry substrings that identify Syriana-owned User-PATH entries."""
    root = str(syriana_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare syriana-agent install dir.
    markers = [root + "\\syriana-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if SYRIANA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "syriana".  We don't want to catch
    # unrelated entries like "csyriana-foo" or "ephermeral", so we look for
    # backslash-syriana as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(syriana_home: Path) -> list[str]:
    """Strip Syriana-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _syriana_path_markers(syriana_home)
            kept: list[str] = []
...
def remove_syriana_env_vars_windows() -> list[str]:
    """Delete SYRIANA_HOME and SYRIANA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SYRIANA_HOME", "SYRIANA_GIT_BASH_PATH"):
                try:
...
def _is_default_syriana_home(syriana_home: Path) -> bool:
    """Return True when ``syriana_home`` points at the default (non-profile) root."""
    try:
        from syriana_constants import get_default_syriana_root
        return syriana_home.resolve() == get_default_syriana_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SYRIANA_HOME directory.

    We shell out to ``syriana -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SYRIANA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m syriana_cli.main` so we don't depend on a `syriana`
    #    wrapper that may be half-removed mid-uninstall.
    syriana_invocation = [_sys.executable, "-m", "syriana_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                syriana_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's SYRIANA_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``syriana uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$SYRIANA_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from syriana_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(syriana_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Syriana Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Syriana Chat GUI installation was found.")
        print(f"  Checked: {syriana_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Syriana agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(syriana_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Syriana agent at {syriana_home / 'syriana-agent'}")
        print(f"  • Your config, sessions, and secrets under {syriana_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Syriana agent is still installed. Run 'syriana' to use the CLI,")
    print("or 'syriana uninstall'' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.syriana/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.syriana/ for future reinstall
    """
    project_root = get_project_root()
    syriana_home = get_syriana_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie SYRIANA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_syriana_home(syriana_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.syriana data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Syriana Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {syriana_home / 'config.yaml'}")
    print(f"  Secrets: {syriana_home / '.env'}")
    print(f"  Data:    {syriana_home / 'cron/'}, {syriana_home / 'sessions/'}, {syriana_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SYRIANA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Syriana data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Syriana code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``syriana`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$SYRIANA_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in syriana_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\syriana\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(syriana_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Syriana-owned PATH entries in User environment")

        log_info("Removing SYRIANA_HOME / SYRIANA_GIT_BASH_PATH User env vars...")
        removed_env = remove_syriana_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Syriana-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing syriana command...")
    ...
    #     (only when they still point into this Syriana home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Syriana-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(syriana_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Syriana-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(syriana_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE SYRIANA_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from syriana_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(syriana_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.syriana/, just remove the syriana-agent subdir
            if syriana_home in project_root.parents or project_root.parent == syriana_home:
    ...
    #     under SYRIANA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(syriana_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(syriana_home)
    ...
    # 5. Optionally remove ~/.syriana/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SYRIANA_HOME dirs live under
        ...
        try:
            if syriana_home.exists():
                shutil.rmtree(syriana_home)
                log_success(f"Removed {syriana_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {syriana_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {syriana_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {syriana_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://syriana-agent.fixology.dev/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://syriana-agent.fixology.dev/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Syriana Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m syriana_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import syriana_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``syriana_constants`` + ``syriana_cli.colors``
    (and lazily ``syriana_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m syriana_cli.uninstall")
    parser.add_argument(
        "--mode",
