...
Reads the user's Nous OAuth state from ``~/.syriana/auth.json`` through the
...
# Endpoints inference.fixology.dev actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `syriana auth add nous` first."
                )
    ...
    # to syriana_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
