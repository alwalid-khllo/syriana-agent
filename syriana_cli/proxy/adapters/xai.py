...
# xAI's public API is OpenAI-compatible for the endpoints Syriana commonly
# uses. The Responses endpoint is included because Syriana' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Syriana-managed OAuth credentials."""

    auth_hint = "syriana auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`syriana auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`syriana auth reset xai-oauth` or re-authenticate with "
                    "`syriana auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `syriana auth add xai-oauth --type oauth`."
            )
