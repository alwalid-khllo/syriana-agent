"""Implementation of the ``syriana bundles`` CLI subcommand.

Mirrors the structure of ``syriana_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `syriana bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  syriana bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``syriana bundles`` argparse tree.

    Called from ``syriana_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `syriana bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``syriana bundles <subcommand>`` to the right handler."""
