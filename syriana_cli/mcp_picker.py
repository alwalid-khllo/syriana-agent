"""MCP picker — interactive `syriana mcp picker` (also the default `syriana mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``syriana mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `syriana plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from syriana_cli.cli_output import prompt_yes_no
from syriana_cli.curses_ui import curses_single_select
from syriana_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Syriana session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from syriana_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `syriana mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: syriana mcp install <name>    Picker: syriana mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Syriana is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Syriana — run `syriana update` "
...
def show_catalog() -> None:
    """`syriana mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`syriana mcp picker` (and default `syriana mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`syriana mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `syriana mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
