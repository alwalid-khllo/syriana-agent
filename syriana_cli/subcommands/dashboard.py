"""``syriana dashboard`` / ``syriana serve`` subcommand parsers.
...
``syriana_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `syriana dashboard` / `syriana serve` cmdlines and SIGTERM them directly —
    # the same path `syriana update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Syriana web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Syriana web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Syriana Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Syriana desktop app shells (<= 0.15.x) spawn the
    # backend as `syriana dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Syriana couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Syriana Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Syriana backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Syriana backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # `syriana dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.syriana/.env.
    # Nested subparser so bare `syriana dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "SYRIANA_DASHBOARD_OAUTH_CLIENT_ID into ~/.syriana/.env, and prints "
            "how to engage the login gate. Requires being logged in (syriana setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://syriana.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via SYRIANA_DASHBOARD_PORTAL_URL. Mainly for "
