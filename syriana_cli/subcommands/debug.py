"""``syriana debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Syriana Agent. Use 'syriana debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    syriana debug share              Upload debug report (asks for confirmation)
    syriana debug share --yes        Skip confirmation (for scripts/CI)
    syriana debug share --lines 500  Include more log lines
    syriana debug share --expire 30  Keep paste for 30 days
    syriana debug share --local      Print report locally (no upload)
    syriana debug share --no-redact  Disable upload-time secret redaction
    syriana debug share --nous       Upload to Nous-internal storage (private)
    syriana debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'syriana debug share'",
    )
