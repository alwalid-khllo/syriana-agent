"""
Backup and import commands for syriana CLI.

`syriana backup` creates a zip archive of the entire ~/.syriana/ directory
(excluding the syriana-agent repo and transient files).

`syriana import` restores from a backup zip, overlaying onto the current
SYRIANA_HOME root.
"""
...
from syriana_constants import get_default_syriana_root, get_syriana_home, display_syriana_home
...
# ``syriana-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/syriana-agent/``
...
# SYRIANA_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "syriana-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under SYRIANA_HOME) —
...
# File names that ``syriana import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE SYRIANA_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside SYRIANA_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to syriana root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``syriana-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/syriana-agent/`` — must be preserved.
        if part == "syriana-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside SYRIANA_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Syriana home directory."""
    syriana_root = get_default_syriana_root()

    if not syriana_root.is_dir():
        print(f"Error: Syriana home directory not found at {syriana_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"syriana-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"syriana-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_syriana_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(syriana_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(syriana_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``syriana-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "syriana-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(syriana_root)
    ...
    # outside SYRIANA_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_syriana_home()}."
        )
    ...
    print(f"\nRestore with: syriana import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Syriana backup.
    ...
    # Look for telltale files that a syriana home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Syriana backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.syriana/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a syriana dir name
        if prefix in {".syriana", "syriana"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_syriana_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (syriana_root / "config.yaml").exists()
        has_env = (syriana_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Syriana configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under SYRIANA_HOME.
            ...
            target = syriana_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(syriana_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_syriana_home()}."
            )
        ...
        profiles_dir = syriana_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # syriana_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: syriana profile list  (after installing syriana)")

        # Guidance
        print()
        if not (syriana_root / "syriana-agent").is_dir():
            print("Note: The syriana-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: syriana update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  syriana -p {pname} gateway install")

        print("Done. Your Syriana configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and syriana backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SYRIANA_HOME).
...
# — `syriana update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $SYRIANA_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside SYRIANA_HOME and the entry is silently
...
def _quick_snapshot_root(syriana_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(syriana_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside SYRIANA_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``syriana update``.
    ...
    Args:
        ...
        syriana_home: Override for the Syriana home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(syriana_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for syriana backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_syriana_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, syriana_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``syriana_root`` to ``out_path``.
...
def _pre_update_backup_dir(syriana_home: Optional[Path] = None) -> Path:
    home = syriana_home or get_syriana_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SYRIANA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SYRIANA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``syriana update``) should continue even if the backup fails.
    """
    syriana_root = syriana_home or get_default_syriana_root()
    if not syriana_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(syriana_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, syriana_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `syriana claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of SYRIANA_HOME under ``backups/`` before a
    ``syriana claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``syriana import <archive>``.  Writes to
    ``<SYRIANA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `syriana import` and the
