"""`syriana checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.syriana/checkpoints/``.  Actions:

    syriana checkpoints               # same as `status`
    syriana checkpoints status        # total size, project count, breakdown
    syriana checkpoints list          # per-project checkpoint counts + workdir
    syriana checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    syriana checkpoints clear [-f]    # nuke the entire base (asks first)
    syriana checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    syriana checkpoints
    syriana checkpoints prune --retention-days 3 --max-size-mb 200
    syriana checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: syriana checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``syriana checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `syriana checkpoints` → status
