"""Slash command definitions and autocomplete for the Syriana CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Syriana config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Syriana works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Syriana is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.syriana/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Syriana Agent to the latest version", "Info"),
    CommandDef("version", "Show Syriana Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`syriana_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/syriana`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Syriana ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Syriana
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``syriana skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``syriana skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``syriana skills list`` and
...
# unrelated command landed. These claim their slots right after /syriana,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /syriana <command>).
...
# native slot, the alias spelling stays reachable via /syriana reset).
...
# ``/syriana <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/syriana" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /syriana credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /syriana billing.
#   - moa: high-cost slash mode, available through /syriana moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /syriana debug on Slack.
_SLACK_VIA_SYRIANA_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/syriana <verb>`` subcommand.
    ...
    can still reach them via ``/syriana <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/syriana`` is always reserved as the first entry so the
    legacy ``/syriana <subcommand>`` form keeps working for anything that
    ...
    # Reserve /syriana as the catch-all top-level command.
    entries.append(("syriana", "Talk to Syriana or run a subcommand", "[subcommand] [args]"))
    seen.add("syriana")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_SYRIANA_ONLY:
            # Intentionally Slack-via-/syriana only (see _SLACK_VIA_SYRIANA_ONLY).
            return
    ...
    # everything except /syriana, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://syriana-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /syriana handler.

    Maps both canonical names and aliases so /syriana bg do stuff works
    the same as /syriana background do stuff.

    Plugin-registered slash commands are included so ``/syriana <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from syriana_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
