...
Usage:
    syriana pairing list              # Show all pending + approved users
    syriana pairing approve <platform> <code>  # Approve a pairing code
    syriana pairing revoke <platform> <user_id> # Revoke user access
    syriana pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle syriana pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: syriana pairing {list|approve|revoke|clear-pending}")
        print("Run 'syriana pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.syriana/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'syriana pairing list' to see pending codes.\n")
