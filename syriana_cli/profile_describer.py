...
Used by ``syriana profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``syriana_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Syriana Agent kanban board.
...
Rules:
  ...
  - Never write "Syriana Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_syriana_home())
        else:
