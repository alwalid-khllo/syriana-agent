...
Pure logic: walks a Syriana config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `syriana doctor` and a future `syriana migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Syriana config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Syriana config and return retirement issues.
