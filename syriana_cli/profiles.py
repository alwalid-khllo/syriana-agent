"""
Profile management for multiple isolated Syriana instances.

Each profile is a fully independent SYRIANA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.syriana/profiles/<name>/`` by default.

The "default" profile is ``~/.syriana`` itself — backward compatible,
zero migration needed.

Usage::

    syriana profile create coder          # fresh profile + bundled skills
    syriana profile create coder --clone  # also copy config, .env, SOUL.md, skills
    syriana profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    syriana -p coder chat                 # or via flag
    syriana profile use coder             # set as sticky default
    syriana profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See syriana_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.syriana``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   syriana-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `syriana backup` archives
...
# Marker file written by `syriana profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `syriana
...
# `syriana skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Syriana infrastructure directories that only the default profile
         (``~/.syriana``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_syriana_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.syriana) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "syriana-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "syriana", "default", "test", "tmp", "root", "sudo",
})

# Syriana subcommands that cannot be used as profile names/aliases
_SYRIANA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the syriana root, NOT to the current SYRIANA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SYRIANA_HOME points outside
    ``~/.syriana``, profiles live under ``SYRIANA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_syriana_home() / "profiles"


def _get_default_syriana_home() -> Path:
    """Return the default (pre-profile) SYRIANA_HOME path.

    In standard deployments this is ``~/.syriana``.
    In Docker/custom deployments where SYRIANA_HOME is outside ``~/.syriana``
    (e.g. ``/opt/data``), returns SYRIANA_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_syriana_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Syriana installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SYRIANA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_syriana_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, syriana subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _SYRIANA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a syriana subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            syriana_exe = shutil.which("syriana") or "syriana"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(syriana_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Syriana. If we
    ...
    try:
        ...
        # not be migrated. The next `syriana doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``syriana -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``syriana profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``syriana -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``syriana profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``SYRIANA_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Syriana config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_syriana_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, syriana_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own SYRIANA_HOME.
    ...
    The returned ``syriana_home`` is the path to pass to
    ``set_syriana_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_syriana_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``syriana update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.syriana)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.syriana/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `syriana -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Syriana profile.\n"
    ...
    if not soul_path.exists():
        try:
            from syriana_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `syriana update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`syriana profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Syriana (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `syriana profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `syriana -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SYRIANA_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``syriana profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SYRIANA_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_syriana_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Syriana *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Syriana backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``SYRIANA_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `syriana -p <canon> profile
    ...
    syriana_markers = ("syriana_cli.main", "syriana-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Syriana process: either an entrypoint marker in argv, or
            # a resolved executable named `syriana`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_syriana = (
                any(marker in joined for marker in syriana_markers)
                or exe_name == "syriana"
                or exe_name.startswith("syriana")
            )
            if not is_syriana:
                continue
            ...
            # ...or by SYRIANA_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("SYRIANA_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.syriana).\n"
            "To remove everything, use: syriana uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_syriana_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``SYRIANA_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from syriana_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SYRIANA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SYRIANA_HOME")
    try:
        os.environ["SYRIANA_HOME"] = str(profile_dir)
        from syriana_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SYRIANA_HOME"] = old_home
        elif "SYRIANA_HOME" in os.environ:
            del os.environ["SYRIANA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.syriana/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: syriana profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SYRIANA_HOME.

    Returns ``"default"`` if SYRIANA_HOME is not set or points to ``~/.syriana``.
    Returns the profile name if SYRIANA_HOME points into ``~/.syriana/profiles/<name>``.
    Returns ``"custom"`` if SYRIANA_HOME is set to an unrecognized path.
    """
    from syriana_constants import get_syriana_home
    syriana_home = get_syriana_home()
    resolved = syriana_home.resolve()

    default_resolved = _get_default_syriana_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.syriana itself — its parent is ~/ and its
        # directory name is ".syriana", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: syriana profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.syriana itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.syriana). "
            "Specify a different name: syriana profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="syriana_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"syriana_{old_name}"
    legacy_old_host = f"syriana.{old_name}"
    new_host = f"syriana_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_syriana_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("syriana_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SYRIANA_HOME path string.

    Called early in the CLI entry point, before any syriana modules
    are imported, to set the SYRIANA_HOME environment variable.
    """
