"""``syriana slack ...`` CLI subcommands.

Today only ``syriana slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ syriana slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Syriana. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Syriana deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from syriana_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Syriana in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Syriana agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``syriana_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$SYRIANA_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Syriana")
    ...
    """
    name = getattr(args, "name", None) or "Syriana"
    description = getattr(args, "description", None) or "Your Syriana agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("SYRIANA_HOME") or str(Path.home() / ".syriana")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Syriana app\n"
