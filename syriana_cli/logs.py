"""``syriana logs`` — view and filter Syriana log files.
...
Usage examples::
    ...
    syriana logs                    # last 50 lines of agent.log
    syriana logs -f                 # follow agent.log in real time
    syriana logs errors             # last 50 lines of errors.log
    syriana logs gateway -n 100    # last 100 lines of gateway.log
    syriana logs gui -f            # follow gui.log (dashboard/pty/ws)
    syriana logs desktop -f        # follow desktop.log (Electron app boot/backend)
    syriana logs --level WARNING    # only WARNING+ lines
    syriana logs --session abc123   # filter by session ID substring
    syriana logs --component tools  # only tool-related lines
    syriana logs --since 1h         # lines from the last hour
    syriana logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_syriana_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Syriana runs — try 'syriana chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_syriana_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_syriana_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_syriana_home()}/logs/")
        return

    print(f"Log files in {display_syriana_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'syriana chat' to generate logs)")
