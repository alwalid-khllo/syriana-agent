...
const SIDEBAR_COLLAPSED_KEY = "syriana-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="syriana-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="syriana-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateSyriana = status?.can_update_syriana === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'syriana update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "syriana update";
    return (
      t.status.updateSyrianaConfirmMessage ??
      `This will run 'syriana update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateSyrianaConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateSyriana) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateSyriana,
      runningLabel: t.status.updatingSyriana,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Syriana gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateSyrianaConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateSyrianaConfirmTitle ?? `${t.status.updateSyriana}?`}
    />
    </>
  );
}
