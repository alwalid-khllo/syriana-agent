/**
 * ChatPage — embeds `syriana --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (syriana_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__SYRIANA_SESSION_TOKEN__ &&
    !window.__SYRIANA_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `syriana dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__SYRIANA_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Syriana TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[syriana-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__SYRIANA_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets SYRIANA_HOME pointed at the
    ...
    // forwarding them into Syriana. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="syriana-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
