...
interface SyrianaConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function SyrianaConsoleModal({ open, onClose }: SyrianaConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("syriana> ");
  const inputPromptRef = useRef("syriana> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "syriana> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Syriana Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="syriana-console-title"
    >
      <div
        className={cn(
          themedBody,
