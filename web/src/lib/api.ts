import { buildSyrianaWebSocketUrl } from "@syriana/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/syriana/). The Python backend
// injects ``window.__SYRIANA_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__SYRIANA_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const SYRIANA_BASE_PATH = readBasePath();
const BASE = SYRIANA_BASE_PATH;
...
declare global {
  interface Window {
    __SYRIANA_SESSION_TOKEN__?: string;
    __SYRIANA_BASE_PATH__?: string;
    ...
    __SYRIANA_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Syriana-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__SYRIANA_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``syriana update``, ``syriana gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__SYRIANA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__SYRIANA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("syriana.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("syriana.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__SYRIANA_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("syriana.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__SYRIANA_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Syriana dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``syriana_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__SYRIANA_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Syriana-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``syriana_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__SYRIANA_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildSyrianaWebSocketUrl({
...
export const api = {
  ...
  updateSyriana: () =>
    fetchJSON<ActionResponse>("/api/syriana/update", { method: "POST" }),
  checkSyrianaUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/syriana/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** syriana-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``syriana update``. */
  can_update_syriana?: boolean;
  config_path: string;
