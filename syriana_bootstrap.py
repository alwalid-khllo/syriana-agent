"""Windows UTF-8 bootstrap for Syriana entry points.
...
should be imported at the very top of every Syriana entry point
(``syriana``, ``syriana-agent``, ``syriana-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Syriana modules.

    Syriana ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Syriana source root
    ...
    """
    root = src_root or os.environ.get("SYRIANA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``SYRIANA_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("SYRIANA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import syriana_bootstrap``
# (or ``from syriana_bootstrap import apply_windows_utf8_bootstrap``) at
