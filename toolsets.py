...
_SYRIANA_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on SYRIANA_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (SYRIANA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `syriana tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``syriana tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SYRIANA_KANBAN_TASK env "
    ...
    # Coding posture (base Syriana — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in syriana_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Syriana toolsets (CLI + messaging platforms)
    #
    ...
    # the `syriana send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "syriana-cli": {
        ...
        "tools": _SYRIANA_CORE_TOOLS,
        "includes": []
    },

    "syriana-cron": {
        # Mirrors syriana-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `syriana tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as syriana-cli; gated by `syriana tools`",
        "tools": _SYRIANA_CORE_TOOLS,
        "includes": []
    },
    ...
    "syriana-email": {
        "description": "Email bot toolset - interact with Syriana via email (IMAP/SMTP)",
        "tools": _SYRIANA_CORE_TOOLS,
        "includes": []
    },
    ...
    "syriana-sms": {
        "description": "SMS bot toolset - interact with Syriana via SMS (Twilio)",
        "tools": _SYRIANA_CORE_TOOLS,
        "includes": []
    },
    ...
    "syriana-webhook": {
        ...
        "tools": _SYRIANA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "syriana-gateway": {
        ...
        "tools": [],
        "includes": ["syriana-telegram", "syriana-discord", "syriana-whatsapp", "syriana-slack", "syriana-signal", "syriana-bluebubbles", "syriana-homeassistant", "syriana-email", "syriana-sms", "syriana-mattermost", "syriana-matrix", "syriana-dingtalk", "syriana-feishu", "syriana-wecom", "syriana-wecom-callback", "syriana-weixin", "syriana-qqbot", "syriana-webhook", "syriana-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``syriana-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_SYRIANA_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``syriana-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (syriana-<name>).
        # Gives them _SYRIANA_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("syriana-"):
            platform_name = name[len("syriana-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SYRIANA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
