...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "syriana-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.syriana"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.syriana directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.syriana"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="syriana-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .syriana/ as a tar archive."""
        # Tar from / with the full path so archive entries preserve absolute
        # paths (e.g. home/user/.syriana/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.syriana".lstrip("/")
        ssh_cmd = self._build_ssh_command()
