...
def _load_syriana_env_vars() -> dict[str, str]:
    """Load ~/.syriana/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Syriana profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale syriana-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=syriana-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Syriana process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=syriana-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a syriana process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Syriana
    process leaves its container behind permanently because there's no
    subsequent Syriana process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=syriana-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=syriana-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``SYRIANA_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("SYRIANA_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using SYRIANA_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'syriana'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``syriana-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.syriana/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. syriana-agent:latest) already use /init as PID 1
        ...
        container_name = f"syriana-{uuid.uuid4().hex[:8]}"
        # Labels make syriana-created containers identifiable to:
        #   * the orphan reaper (`syriana-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`syriana-task-id`, `syriana-profile`)
        #   * operators running `docker ps --filter label=syriana-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Syriana profile is captured at
        ...
        label_args = [
            "--label", "syriana-agent=1",
            "--label", f"syriana-task-id={task_label}",
            "--label", f"syriana-profile={profile_name}",
        ]
        ...
        self._labels = {
            "syriana-agent": "1",
            "syriana-task-id": task_label,
            "syriana-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Syriana process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Syriana secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Syriana-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_syriana_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_syriana_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _SYRIANA_PROVIDER_ENV_BLOCKLIST)
        syriana_env = _load_syriana_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = syriana_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("syriana-task-id", "")
        profile_label = self._labels.get("syriana-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"syriana-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``syriana-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=syriana-agent=1",
                    "--filter", f"label=syriana-task-id={task_label}",
                    "--filter", f"label=syriana-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Syriana process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Syriana exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Syriana process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Syriana startup. That covers the
        ...
        # outlives Syriana processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"syriana-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``syriana /quit``
