...
One ``CDPSupervisor`` runs per Syriana ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Syriana logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "syriana-dialog-bridge.invalid"
...
(() => {
  if (window.__syrianaDialogBridgeInstalled) return;
  window.__syrianaDialogBridgeInstalled = true;
  const ENDPOINT = "http://syriana-dialog-bridge.invalid/";
