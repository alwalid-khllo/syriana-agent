...
Backend is selected during ``syriana tools`` setup (web.backend in config.yaml).
When available, Syriana can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Syriana config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Syriana' config/.env layer (``syriana config set``, ``syriana tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.syriana/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``syriana tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `syriana tools` repaint.
        try:
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_syriana_dir("cache/web", "web_cache")
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `syriana tools` to set one up."
                ),
            }
        else:
            logger.info(
