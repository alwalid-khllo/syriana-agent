...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``syriana_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from syriana_constants import get_subprocess_home
...
_syriana_config_resolved: str | None = None
_syriana_config_resolved_loaded = False


def _get_syriana_config_resolved() -> str | None:
    """Return the resolved absolute path of the Syriana config file (cached)."""
    global _syriana_config_resolved, _syriana_config_resolved_loaded
    if _syriana_config_resolved_loaded:
        return _syriana_config_resolved
    _syriana_config_resolved_loaded = True
    try:
        from syriana_cli.config import get_config_path
        _syriana_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _syriana_config_resolved = str(Path(_expand_tilde("~/.syriana/config.yaml")).resolve())
        except Exception:
            _syriana_config_resolved = None
    return _syriana_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Syriana config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    syriana_config = _get_syriana_config_resolved()
    if syriana_config and (resolved == syriana_config or normalized == syriana_config):
        return (
            f"Refusing to write to Syriana config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.syriana/config.yaml directly or use 'syriana config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Syriana mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Syriana
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Syriana state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.syriana/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Syriana process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.syriana/…`` path.

    Returns ``None`` when the write is in-scope or outside Syriana scope.
    ...
    # in a session that cd'd into ``~/.syriana/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Syriana internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under SYRIANA_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == SYRIANA_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Syriana-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Syriana-profile guard for
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Syriana profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with syriana_tools.write_file() for very "
            "large files."
        )
