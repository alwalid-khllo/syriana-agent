...
it is automatically downloaded from GitHub releases to $SYRIANA_HOME/bin/tirith.
...
def _get_syriana_home() -> str:
    """Return the Syriana home directory, respecting SYRIANA_HOME env var."""
    return str(get_syriana_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_syriana_home(), ".tirith-install-failed")
...
def _syriana_bin_dir() -> str:
    """Return $SYRIANA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_syriana_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SYRIANA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_syriana_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SYRIANA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SYRIANA_HOME/bin/tirith
    ...
    syriana_bin = os.path.join(_syriana_bin_dir(), "tirith")
    if os.path.isfile(syriana_bin) and os.access(syriana_bin, os.X_OK):
        _resolved_path = syriana_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return syriana_bin
