...
`mcp_tool.py`, and `syriana_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# SyrianaMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_syriana_provider_class() -> Optional[type]:
    ...
    class SyrianaMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._syriana_server_name = server_name
            ...
            self._syriana_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`SyrianaTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, SyrianaTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._syriana_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import SyrianaTokenStorage
                        if isinstance(storage, SyrianaTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._syriana_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import SyrianaTokenStorage
            if not isinstance(storage, SyrianaTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``syriana mcp reauth``.
            ...
            back to ``syriana mcp reauth``.
            """
            try:
                if self._syriana_preregistered:
                    return
    ...
    return SyrianaMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_SYRIANA_PROVIDER_CLS: Optional[type] = _make_syriana_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`SyrianaMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _SYRIANA_PROVIDER_CLS is None:
            logger.warning(
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `syriana mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``syriana mcp remove <name>`` and (indirectly) by
        ``syriana mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
