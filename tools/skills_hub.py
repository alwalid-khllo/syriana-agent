#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Syriana Skills Hub.
...
Used by syriana_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _syriana_home() -> Path:
    return get_syriana_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _syriana_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            syriana_meta = metadata.get("syriana", {})
            if isinstance(syriana_meta, dict):
                tags = syriana_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            syriana_meta = metadata.get("syriana", {})
            if isinstance(syriana_meta, dict):
                raw_tags = syriana_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by syriana ai Research) but not activated
    ...
    OFFICIAL_REPO = "SyrianaAIResearch/syriana-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                syriana_meta = meta_block.get("syriana", {})
                if isinstance(syriana_meta, dict):
                    tags = syriana_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Syriana centralized index source
# ---------------------------------------------------------------------------

SYRIANA_INDEX_URL = "https://syriana-agent.fixology.dev/docs/api/skills-index.json"
SYRIANA_INDEX_TTL = 6 * 3600  # 6 hours


def _syriana_index_cache_file() -> Path:
    return _index_cache_dir() / "syriana-index.json"


def _load_syriana_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for SYRIANA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    syriana_index_cache_file = _syriana_index_cache_file()
    if syriana_index_cache_file.exists():
        try:
            age = time.time() - syriana_index_cache_file.stat().st_mtime
            if age < SYRIANA_INDEX_TTL:
                return json.loads(syriana_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(SYRIANA_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Syriana index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Syriana index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        syriana_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        syriana_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class SyrianaIndexSource(SkillSource):
    """Skill source backed by the centralized Syriana Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_syriana_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "syriana-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "syriana-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        SyrianaIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "syriana-index"
