...
`syriana skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.syriana.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.syriana.blueprint`` is a mapping containing
    ...
    syriana = meta.get("syriana") if isinstance(meta, dict) else None
    blueprint = syriana.get("blueprint") if isinstance(syriana, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.syriana.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.syriana.blueprint`` block) they can hand
    to ``syriana skills publish`` to share. ``body`` is the plain-language
