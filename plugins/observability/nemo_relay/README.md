...
Optional Syriana observability plugin that maps Syriana observer hooks to
...
not replace Syriana Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Syriana emit NeMo Relay lifecycle events for
the work Syriana already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Syriana Agent can:

- Preserve Syriana execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
syriana plugins enable observability/nemo_relay
```

The `SYRIANA_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `SYRIANA_HOME` that the
agent run will use:

```bash
env SYRIANA_HOME=/tmp/syriana-nemo-relay-test \
  syriana plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`SYRIANA_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`SYRIANA_HOME` is the Syriana profile/config home used by both
`syriana plugins enable ...` and the later `syriana chat ...` run. If unset,
Syriana uses the user's default home, usually `~/.syriana`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export SYRIANA_HOME=/tmp/syriana-nemo-relay-test
syriana plugins enable observability/nemo_relay
syriana chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `syriana` command you run is built from the
...
uv run syriana plugins enable observability/nemo_relay
uv run syriana chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/syriana_agent-*.whl
...
The plugin can configure exporters directly from `SYRIANA_NEMO_RELAY_*`
...
```bash
export SYRIANA_NEMO_RELAY_ATOF_ENABLED=1
export SYRIANA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export SYRIANA_NEMO_RELAY_ATIF_ENABLED=1
export SYRIANA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `SYRIANA_NEMO_RELAY_ATOF_FILENAME`
- `SYRIANA_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `SYRIANA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `SYRIANA_NEMO_RELAY_ATIF_AGENT_NAME`
- `SYRIANA_NEMO_RELAY_ATIF_AGENT_VERSION`
- `SYRIANA_NEMO_RELAY_ATIF_MODEL_NAME`
- `SYRIANA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Syriana at it:

```bash
export SYRIANA_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `SYRIANA_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`SYRIANA_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`SYRIANA_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Syriana does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Syriana keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Syriana routes LLM and tool
...
For the full generic Syriana middleware contract, see
...
export SYRIANA_HOME=/tmp/syriana-nemo-relay-docs/syriana-home
mkdir -p "$SYRIANA_HOME"

cat > "$SYRIANA_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Syriana session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export SYRIANA_NEMO_RELAY_ATOF_ENABLED=1
export SYRIANA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/syriana-nemo-relay-docs/subagent/atof
export SYRIANA_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export SYRIANA_NEMO_RELAY_ATOF_MODE=overwrite
export SYRIANA_NEMO_RELAY_ATIF_ENABLED=1
export SYRIANA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/syriana-nemo-relay-docs/subagent/atif
export SYRIANA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export SYRIANA_NEMO_RELAY_ATIF_AGENT_NAME='Syriana Agent E2E'
export SYRIANA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export SYRIANA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"syriana.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Syriana Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Syriana dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/syriana-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/syriana-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/syriana-nemo-relay-docs/workdir/beta.txt
cd /tmp/syriana-nemo-relay-docs/workdir

export SYRIANA_NEMO_RELAY_ATOF_ENABLED=1
export SYRIANA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/syriana-nemo-relay-docs/parallel/atof
export SYRIANA_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export SYRIANA_NEMO_RELAY_ATOF_MODE=overwrite
export SYRIANA_NEMO_RELAY_ATIF_ENABLED=1
export SYRIANA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/syriana-nemo-relay-docs/parallel/atif
export SYRIANA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export SYRIANA_NEMO_RELAY_ATIF_AGENT_NAME='Syriana Agent E2E'
export SYRIANA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Syriana sessions map to `agent` scopes.
- Syriana API request hooks map to `llm` scope start/end events.
- Syriana tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Syriana execution middleware to hand
...
Enable it for Syriana:

```bash
export SYRIANA_NEMO_RELAY_PLUGINS_TOML=/tmp/syriana-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Syriana routes execution
through these boundaries:

```text
Syriana provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Syriana provider adapter next_call(...)

Syriana tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Syriana tool dispatcher next_call(...)
```
...
middleware for a local Syriana run. This path requires a NeMo Relay runtime that
...
```bash
export SYRIANA_HOME=/tmp/syriana-middleware-test/syriana-home
mkdir -p "$SYRIANA_HOME" /tmp/syriana-middleware-test/nemo-relay
...
cat > /tmp/syriana-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/syriana-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/syriana-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Syriana Middleware E2E"
agent_version = "local"
...
export SYRIANA_NEMO_RELAY_PLUGINS_TOML=/tmp/syriana-middleware-test/nemo-relay/plugins.toml
