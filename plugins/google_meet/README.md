...
Let the syriana agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where syriana runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `syriana meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `syriana meet` CLI |
...
| `cli.py` | `syriana meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$SYRIANA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `syriana meet node {run,list,approve,remove,status,ping}` |
...
syriana meet install                                      # pip + Chromium
syriana meet setup                                        # preflight
syriana meet auth                                         # optional
syriana meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
syriana meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.syriana/.env
syriana meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
syriana meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
syriana meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.syriana/.env
syriana meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, syriana will **not** switch your system audio input automatically — the
...
syriana plugins enable google_meet
syriana meet node run --display-name my-mac --host 0.0.0.0 --port 18789
