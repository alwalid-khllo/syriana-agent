"""`syriana meet node ...` subcommand tree.

Wired into the existing ``syriana meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``syriana meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="syriana-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``syriana meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             syriana meet node approve <name> ws://<host>:{args.port} {token}")
        try:
