...
session-scoped state file under ``$SYRIANA_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $SYRIANA_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_syriana_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this syriana install, leave it first —
    ...
    env = os.environ.copy()
    env["SYRIANA_MEET_URL"] = url
    env["SYRIANA_MEET_OUT_DIR"] = str(out)
    env["SYRIANA_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["SYRIANA_MEET_HEADED"] = "1"
    if auth_state:
        env["SYRIANA_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["SYRIANA_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if SYRIANA_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["SYRIANA_MEET_MODE"] = mode
    if realtime_model:
        env["SYRIANA_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["SYRIANA_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["SYRIANA_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["SYRIANA_MEET_REALTIME_KEY"] = realtime_api_key
