# Syriana Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: syriana-achievements plugin
...
### `GET /api/plugins/syriana-achievements/achievements`
...
### `POST /api/plugins/syriana-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/syriana-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/syriana-achievements/overview`
...
- Persist plugin data under:
  - `~/.syriana/plugins/syriana-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.syriana/plugins/syriana-achievements/scan_checkpoint.json`
...
- `~/.syriana/plugins/syriana-achievements/`
