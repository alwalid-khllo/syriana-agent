"""ntfy platform adapter (Syriana plugin).
...
a Syriana dependency.

This adapter ships as a Syriana platform plugin under
``plugins/platforms/ntfy/``. The Syriana plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "syriana-in"              # subscribe topic (incoming)
          publish_topic: "syriana-out"     # optional — defaults to topic
...
_ECHO_TAG = "syriana-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``syriana cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Syriana dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `syriana gateway status` without
