"""
LINE Messaging API platform adapter for Syriana Agent.
...
to Syriana Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SYRIANA_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            syriana_home = Path(get_syriana_home()).resolve()
        except Exception:
            syriana_home = Path.home().joinpath(".syriana").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``syriana status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``syriana status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``syriana setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.syriana/.env`` via ``syriana_cli.config``.
    """
    print()
    ...
    try:
        from syriana_cli.config import get_env_var, set_env_var
    except ImportError:
        print("syriana_cli.config not available; set LINE_* vars manually in ~/.syriana/.env")
        return
