...
# ``model_tools`` import time, so ``syriana status``, ``syriana chat``, etc.
...
# ``syriana_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Syriana is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a syriana MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SYRIANA_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _syriana_home = _get_syriana_home()
        except (ModuleNotFoundError, ImportError):
            _syriana_home = _Path.home() / ".syriana"
        self._thread_count_store = _ThreadCountStore(
            _syriana_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Syriana is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SYRIANA_HOME", str(_Path.home() / ".syriana"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SYRIANA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a syriana MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Syriana is thinking…' marker message.
        ...
        "Syriana is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Syriana is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Syriana is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Syriana is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Syriana is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``syriana setup``.

    The setup wizard at ``syriana_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.syriana/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. syriana-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.syriana/.env")
    print_info("Restart the gateway: syriana gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``syriana cron`` running as a
    separate process from ``syriana gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Syriana plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'syriana-agent[google_chat]'",
        ...
        # the "Syriana is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Syriana is thinking…' marker message "
