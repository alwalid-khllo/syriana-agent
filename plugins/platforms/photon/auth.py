...
Credential storage mirrors every other Syriana channel:

    * runtime SDK creds  -> ``~/.syriana/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.syriana/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a syriana dependency
...
# registers Syriana as its own client_id.
...
# Default name of the project Syriana provisions for the operator.
DEFAULT_PROJECT_NAME = "Syriana Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of syriana-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.syriana/auth.json`` honouring the active Syriana profile."""
    try:
        from syriana_constants import get_syriana_home
        return Path(get_syriana_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.syriana")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.syriana/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.syriana/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.syriana/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: syriana_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `syriana photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `syriana photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `syriana photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `syriana photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `syriana photon setup`)"
