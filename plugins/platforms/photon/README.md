...
This plugin connects Syriana Agent to iMessage (and other Spectrum
...
TypeScript-only, Syriana runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Syriana-Sidecar-Token`.
...
There is no separate `login` command; like every other Syriana channel,
...
Run `syriana photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.syriana/.env` (the same place every other
...
Management metadata lives in `~/.syriana/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `syriana photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `syriana photon status`, a DM and a group roundtrip,
