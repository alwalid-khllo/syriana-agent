...
Small Node helper that bridges Syriana Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Syriana is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Syriana-Sidecar-Token`)
...
The Syriana plugin's `syriana photon setup` command runs `npm install`
here automatically.
