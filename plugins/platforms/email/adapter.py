"""
Email platform adapter for the Syriana gateway.

Allows users to interact with Syriana by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "syriana-agent" "version" "{_syriana_version}" '
            '"vendor" "SyrianaAIResearch" '
            '"support-email" "noreply@fixology.dev")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `syriana config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `syriana gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Syriana Agent")
        ...
        msg_id = f"<syriana-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# syriana_cli/gateway.py, and the _send_email dispatch in
