"""SimpleX Chat platform adapter (Syriana plugin).
...
This adapter ships as a Syriana platform plugin under
``plugins/platforms/simplex/``. The Syriana plugin loader scans the
...
Optional environment variables:
    ...
    SYRIANA_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``syriana setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Syriana — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("SYRIANA_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``syriana setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.syriana/.env`` via
    ``syriana_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "syriana_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.syriana/.env"
        )
        return
