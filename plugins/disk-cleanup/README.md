...
Auto-tracks and cleans up ephemeral files created during Syriana Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SYRIANA_HOME` and `/tmp/syriana-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SYRIANA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SYRIANA_HOME` or `/tmp/syriana-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SYRIANA_HOME/disk-cleanup/` is itself excluded
- `$SYRIANA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
