"""disk_cleanup — ephemeral file cleanup for Syriana Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under SYRIANA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SYRIANA_HOME and /tmp/syriana-*
Never touches: ~/.syriana/logs/ or any system directory.
"""
...
try:
    ...
    def get_syriana_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("SYRIANA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".syriana").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SYRIANA_HOME/logs/``."""
    return get_syriana_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SYRIANA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SYRIANA_HOME or ``/tmp/syriana-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    syriana_home = get_syriana_home()
    try:
        path.resolve().relative_to(syriana_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/syriana-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("syriana-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "syriana-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $SYRIANA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so SYRIANA_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        syriana_home = get_syriana_home()
        for parent in ("cron", "cronjobs"):
            base = syriana_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SYRIANA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under SYRIANA_HOME, but never recurse into known
    # durable state trees.  Some installs place the Syriana checkout, venv,
    # and desktop build under SYRIANA_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    syriana_home = get_syriana_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in syriana_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    syriana_home = get_syriana_home()
    try:
        rel = path.resolve().relative_to(syriana_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "syriana-agent",
        }:
            return None
        ...
        # Path isn't under SYRIANA_HOME (e.g. /tmp/syriana-*) — fall through.
        pass
