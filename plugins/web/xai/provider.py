...
``{title, url, description, position}`` rows every other Syriana web
provider produces.
...
prefers Syriana-managed xAI Grok OAuth (via ``syriana auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.syriana/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``syriana tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `syriana auth` to sign in with "
