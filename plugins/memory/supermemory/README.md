...
- Supermemory API key from [app.supermemory.ai/integrations?connect=syriana](http://app.supermemory.ai/integrations?connect=syriana)
...
Config file: `$SYRIANA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `syriana` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `syriana-{identity}` → `syriana-coder`). |
...
All Supermemory API calls send `x-sm-source: syriana`, and document writes stamp
`metadata.sm_source: syriana`. This is a **functional routing key, not telemetry**:
it groups Syriana-written memories into a dedicated "Syriana" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Syriana profile:

```json
{
  "container_tag": "syriana-{identity}"
}
```

For a profile named `coder`, this resolves to `syriana-coder`. The default profile resolves to `syriana-default`. Without `{identity}`, all profiles share the same container.
