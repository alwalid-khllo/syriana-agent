...
Configuration
-------------
Secret (lives in $SYRIANA_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $SYRIANA_HOME/mem0.json, set via `syriana memory
setup`):
  ...
  agent_id           — Agent identifier (default: syriana)
...
def _load_config() -> dict:
    """Load config from env vars, with $SYRIANA_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "syriana"),
        "oss": {},
    }
    ...
    config_path = get_syriana_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, syriana_home):
        """Write config to $SYRIANA_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, syriana_home: str, config: dict) -> None:
        ...
        post_setup(syriana_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $SYRIANA_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "syriana")
