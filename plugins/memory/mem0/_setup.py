...
def _save_mem0_json(syriana_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(syriana_home) / "mem0.json"
    existing = {}
...
def _setup_platform(syriana_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "syriana-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "syriana"},
    ...
    provider.save_config(provider_config, syriana_home)

    if env_writes:
        _write_env(Path(syriana_home) / ".env", env_writes)
...
def _setup_oss(syriana_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(syriana_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "syriana-user")
    ...
    if env_writes:
        _write_env(Path(syriana_home) / ".env", env_writes)
    _save_mem0_json(syriana_home, {"mode": "oss", "user_id": user_id, "agent_id": "syriana", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, syriana_home: str) -> str:
...
_PGVECTOR_CONTAINER = "syriana-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "syriana"
...
def _setup_oss_interactive(syriana_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], syriana_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], syriana_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'syriana-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "syriana-user")

    agent_id = input("  Agent ID [syriana]: ").strip()
    agent_id = agent_id or "syriana"
    ...
    if env_writes:
        _write_env(Path(syriana_home) / ".env", env_writes)
    _save_mem0_json(syriana_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(syriana_home: str, config: dict) -> None:
    """Entry point called by syriana memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(syriana_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(syriana_home, config, flags)
        return
