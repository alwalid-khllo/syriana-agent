/**
 * Syriana Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.syriana/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SYRIANA_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SYRIANA_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "syriana-kanban-dot-triage",
    todo: "syriana-kanban-dot-todo",
    ready: "syriana-kanban-dot-ready",
    running: "syriana-kanban-dot-running",
    blocked: "syriana-kanban-dot-blocked",
    done: "syriana-kanban-dot-done",
    archived: "syriana-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-syriana-task";
  ...
  const DOCS_URL = "https://syriana-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://syriana-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "syriana.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="syriana-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])syriana-kanban-md-code\1/i.test(attrs)) {
      return ' class="syriana-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "syriana-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "syriana-kanban-md",
  ...
  // standard `drop` event and our `syriana-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("syriana-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("syriana-kanban-column--drop");
          if (target) target.classList.add("syriana-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("syriana-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("syriana-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("syriana-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__SYRIANA_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "syriana-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "syriana-kanban-attention",
        "syriana-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "syriana-kanban-attention-bar" },
        h("span", { className: "syriana-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "syriana-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "syriana-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "syriana-kanban-attention-row",
                  "syriana-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "syriana-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "syriana-kanban-attention-row-id" }, task.id),
                h("span", { className: "syriana-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "syriana-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "syriana-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "syriana-kanban-diag-action-btn",
      action.suggested ? "syriana-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " syriana-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".syriana-kanban-drawer-comment-row input, .syriana-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "syriana-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("syriana-kanban-diag", sevClass) },
      h("div", { className: "syriana-kanban-diag-header" },
        h("span", { className: "syriana-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "syriana-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "syriana-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "syriana-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "syriana-kanban-diag-data-row" },
                  h("span", { className: "syriana-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "syriana-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "syriana-kanban-diag-data-row" },
                h("span", { className: "syriana-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "syriana-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "syriana-kanban-diag-reassign-row" },
            h("span", { className: "syriana-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "syriana-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "syriana-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "syriana-kanban-diag-msg",
              msg.ok ? "syriana-kanban-diag-msg--ok" : "syriana-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "syriana-kanban-section" },
      h("div", { className: "syriana-kanban-section-head-row" },
        h("span", { className: "syriana-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "syriana-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "syriana-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "syriana-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "syriana-kanban-docs-link",
      title: "Open Syriana Kanban docs in a new tab",
      "aria-label": "Syriana Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `syriana kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "syriana-kanban-msg-ok" : "syriana-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "syriana-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "syriana-kanban-boardswitcher" },
      h("div", { className: "syriana-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "syriana-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "syriana-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "syriana-kanban-dialog-title" },
        ...
        h("div", { className: "syriana-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Syriana profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "syriana-kanban-bulk" },
      h("span", { className: "syriana-kanban-bulk-count" },
      ...
      h("div", { className: "syriana-kanban-bulk-priority",
      ...
      h("div", { className: "syriana-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Syriana profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "syriana-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("syriana-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("syriana-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "syriana-kanban-trash",
        dragOver ? "syriana-kanban-trash--drop" : "",
        props.draggingTaskId ? "syriana-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "syriana-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "syriana-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".syriana-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "syriana-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("syriana-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("syriana-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "syriana-kanban-column",
        dragOver ? "syriana-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "syriana-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "syriana-kanban-col-check",
        ...
        h("span", { className: cn("syriana-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "syriana-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "syriana-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "syriana-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "syriana-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "syriana-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "syriana-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "syriana-kanban-lane" },
                  h("div", { className: "syriana-kanban-lane-head" },
                    h("span", { className: "syriana-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "syriana-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "syriana-kanban-card--stale-red";
    if (age >= tier.amber) return "syriana-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".syriana-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "syriana-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "syriana-kanban-card",
        props.selected ? "syriana-kanban-card--selected" : "",
        props.failed ? "syriana-kanban-card--failed" : "",
        props.draggingSource ? "syriana-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "syriana-kanban-card-content" },
          h("div", { className: "syriana-kanban-card-row" },
            h("label", {
              className: "syriana-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "syriana-kanban-card-check",
            ...
            h("span", { className: "syriana-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or syriana kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "syriana-kanban-warning-badge",
                    "syriana-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "syriana-kanban-needs-assignee",
          ...
          h("div", { className: "syriana-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "syriana-kanban-card-row syriana-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "syriana-kanban-assignee",
                            title: `Assigned to Syriana profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "syriana-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "syriana-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "syriana-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "syriana-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Syriana profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Syriana profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "syriana-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "syriana-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "syriana-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "syriana-kanban-drawer-close",
        ...
        data ? h("div", { className: "syriana-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "syriana-kanban-section" },
      h("div", { className: "syriana-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "syriana-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "syriana-kanban-drawer-body" },
      h("div", { className: "syriana-kanban-drawer-title" },
        h("span", { className: cn("syriana-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "syriana-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "syriana-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "syriana-kanban-section" },
        h("div", { className: "syriana-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "syriana-kanban-section" },
        h("div", { className: "syriana-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "syriana-kanban-comment" },
                h("div", { className: "syriana-kanban-comment-head" },
                  h("span", { className: "syriana-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "syriana-kanban-comment-ago" },
      ...
      h("div", { className: "syriana-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "syriana-kanban-event",
              isDiag ? "syriana-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "syriana-kanban-event-header" },
                  h("span", { className: "syriana-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "syriana-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "syriana-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "syriana-kanban-event-header-plain" },
                  h("span", { className: "syriana-kanban-event-kind" }, e.kind),
                  h("span", { className: "syriana-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "syriana-kanban-event-phantom-row" },
                  h("span", { className: "syriana-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "syriana-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "syriana-kanban-event-payload" },
    ...
    return h("div", { className: "syriana-kanban-section" },
      h("div", { className: "syriana-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "syriana-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `syriana-kanban-run--${r.outcome || r.status || "ended"}`
          : "syriana-kanban-run--active";
        return h("div", { key: r.id, className: cn("syriana-kanban-run", outcomeClass) },
          h("div", { className: "syriana-kanban-run-head" },
            h("span", { className: "syriana-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "syriana-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "syriana-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "syriana-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "syriana-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "syriana-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "syriana-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "syriana-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "syriana-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "syriana-kanban-pre syriana-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "syriana-kanban-meta-row" },
      h("span", { className: "syriana-kanban-meta-label" }, props.label),
      h("span", { className: "syriana-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "syriana-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "syriana-kanban-meta-row" },
        h("span", { className: "syriana-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "syriana-kanban-meta-value syriana-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "syriana-kanban-meta-row" },
        h("span", { className: "syriana-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "syriana-kanban-meta-value syriana-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "syriana-kanban-section" },
      h("div", { className: "syriana-kanban-section-head-row" },
        h("span", { className: "syriana-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "syriana-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "syriana-kanban-section" },
      h("div", { className: "syriana-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "syriana-kanban-deps-row" },
        h("span", { className: "syriana-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "syriana-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "syriana-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "syriana-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "syriana-kanban-dep-chip-x",
      ...
      h("div", { className: "syriana-kanban-deps-row" },
        h("span", { className: "syriana-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "syriana-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "syriana-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "syriana-kanban-msg-ok"
          : "syriana-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "syriana-kanban-section" },
      ...
      h("div", { className: "syriana-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "syriana-kanban-home-sub syriana-kanban-home-sub--on"
              : "syriana-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__SYRIANA_PLUGINS__ && typeof window.__SYRIANA_PLUGINS__.register === "function") {
    window.__SYRIANA_PLUGINS__.register("kanban", KanbanPage);
  }
})();
