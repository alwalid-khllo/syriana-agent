"""Centralized logging setup for Syriana Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.syriana/logs/`` (profile-aware via ``get_syriana_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``syriana_cli.web_server``,
    ``syriana_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Syriana (TUI, gateway, ``hy_memory`` server, MCP
...
from syriana_constants import get_config_path, get_syriana_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_syriana_session_injector", False):
    ...
    _session_record_factory._syriana_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``syriana logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``syriana logs --component gateway``.
    "gateway": ("gateway", "syriana_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("syriana_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "syriana_cli.web_server",
        "syriana_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    syriana_home: Optional[Path] = None,
    ...
    """Configure the Syriana logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    syriana_home
        Override for the Syriana home directory.  Falls back to
        ``get_syriana_home()`` (profile-aware).
    log_level
    ...
    home = syriana_home or get_syriana_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_syriana_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._syriana_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the syriana group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from syriana_cli.config import is_managed
